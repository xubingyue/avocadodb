...
static ExecutionCondition const NotInCoordinator = [] {
  return !avocadodb::ServerState::instance()->isRunningInCluster() ||
  ...
  try {
    ...
    for (size_t i = 0; i < nCoords; ++i) {
      distances.emplace_back(geo_coordinate_distance_t(
          cors->distances[i],
          avocadodb::MMFilesGeoIndex::toDocumentIdentifierToken(
...
static avocadodb::MMFilesGeoIndex* getGeoIndex(
  ...
  avocadodb::MMFilesGeoIndex* index = nullptr;

  for (auto const& idx : document->getIndexes()) {
    if (idx->type() == avocadodb::Index::TRI_IDX_TYPE_GEO1_INDEX ||
        idx->type() == avocadodb::Index::TRI_IDX_TYPE_GEO2_INDEX) {
      index = static_cast<avocadodb::MMFilesGeoIndex*>(idx.get());
      break;
    }
  }
...
AqlValue MMFilesAqlFunctions::Fulltext(
  ...
  avocadodb::MMFilesFulltextIndex* fulltextIndex = nullptr;

  // split requested attribute name on '.' character to create a proper
  // vector of AttributeNames
  std::vector<std::vector<avocadodb::basics::AttributeName>> search;
  search.emplace_back();
  ...
  for (auto const& idx : collection->getIndexes()) {
    if (idx->type() == avocadodb::Index::TRI_IDX_TYPE_FULLTEXT_INDEX) {
      // test if index is on the correct field
      if (avocadodb::basics::AttributeName::isIdentical(idx->fields(), search,
                                                       false)) {
        // match!
        fulltextIndex = static_cast<avocadodb::MMFilesFulltextIndex*>(idx.get());
        break;
      }
    }
  }
...
/// @brief function NEAR
AqlValue MMFilesAqlFunctions::Near(avocadodb::aql::Query* query,
  ...
  avocadodb::MMFilesGeoIndex* index = getGeoIndex(trx, cid, collectionName);
