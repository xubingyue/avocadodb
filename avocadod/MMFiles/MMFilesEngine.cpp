...
void MMFilesEngine::recoveryDone(TRI_vocbase_t* vocbase) {
  ...
  for (auto& it : _deleted) {
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "collection/view '" << name
...
void MMFilesEngine::getDatabases(avocadodb::velocypack::Builder& result) {
  result.openArray();
  ...
  for (auto const& name : files) {
    ...
    try {
      builder = avocadodb::basics::VelocyPackHelper::velocyPackFromFile(file);
    } catch (...) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
          << "database directory '" << directory
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "database parameters: "
                                              << parametersString;
...
void MMFilesEngine::getCollectionInfo(TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
  ...
  std::shared_ptr<VPackBuilder> fileInfoBuilder =
      avocadodb::basics::VelocyPackHelper::velocyPackFromFile(
    ...
    for (auto const& file : files) {
      if (StringUtils::isPrefix(file, "index-") &&
        ...
        std::shared_ptr<VPackBuilder> indexVPack =
            avocadodb::basics::VelocyPackHelper::velocyPackFromFile(filename);
...
int MMFilesEngine::getCollectionsAndIndexes(
    TRI_vocbase_t* vocbase, avocadodb::velocypack::Builder& result,
  ...
  for (auto const& name : files) {
    ...
    if (!TRI_IsDirectory(directory.c_str())) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "ignoring non-directory '"
                                                << directory << "'";
      continue;
    }
    ...
    try {
      ...
      // add collection info
      result.add(info);
    } catch (avocadodb::basics::Exception const& e) {
...
TRI_vocbase_t* MMFilesEngine::openDatabase(
    avocadodb::velocypack::Slice const& args, bool isUpgrade, int& status) {
...
TRI_vocbase_t* MMFilesEngine::createDatabaseMMFiles(
    TRI_voc_tick_t id, avocadodb::velocypack::Slice const& data) {
...
void MMFilesEngine::waitUntilDeletion(TRI_voc_tick_t id, bool force,
  ...
  while (TRI_IsDirectory(path.c_str())) {
    ...
    if (iterations == 5 * 20) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME)
          << "waiting for deletion of database directory '" << path << "'";
    }
...
std::string MMFilesEngine::createCollection(
    ...
    avocadodb::LogicalCollection const* parameters) {
  ...
  std::string const tmpfile(
      avocadodb::basics::FileUtils::buildFilename(tmpname, ".tmp"));
...
avocadodb::Result MMFilesEngine::persistCollection(
    TRI_vocbase_t* vocbase, avocadodb::LogicalCollection const* collection) {
...
avocadodb::Result MMFilesEngine::dropCollection(
    TRI_vocbase_t* vocbase, avocadodb::LogicalCollection* collection) {
  if (inRecovery()) {
    // nothing to do here
    return {};
  }
...
void MMFilesEngine::destroyCollection(TRI_vocbase_t* vocbase,
  ...
  if (invalid) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      ...
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "wiping dropped collection '"
                                                << name << "' from disk";
...
void MMFilesEngine::changeCollection(
    TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
    avocadodb::LogicalCollection const* parameters, bool doSync) {
  saveCollectionInfo(vocbase, id, parameters, doSync);
}
...
void MMFilesEngine::createView(TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
                               avocadodb::LogicalView const* parameters) {
  std::string const path = databasePath(vocbase);
...
avocadodb::Result MMFilesEngine::persistView(TRI_vocbase_t* vocbase,
                                            avocadodb::LogicalView const* view) {
...
avocadodb::Result MMFilesEngine::dropView(TRI_vocbase_t* vocbase,
                                         avocadodb::LogicalView* view) {
  if (inRecovery()) {
    // nothing to do here
    return {};
  }
...
void MMFilesEngine::destroyView(TRI_vocbase_t* vocbase,
  ...
  if (invalid) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot rename dropped view '"
                                            << name << "': unknown path '"
                                            << physical->path() << "'";
  } else {
    // prefix the collection name with "deleted-"
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      ...
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "wiping dropped view '"
                                                << name << "' from disk";
...
void MMFilesEngine::changeView(TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
                               avocadodb::LogicalView const* view, bool doSync) {
...
void MMFilesEngine::createIndex(TRI_vocbase_t* vocbase,
  ...
  bool const doSync =
      application_features::ApplicationServer::getFeature<DatabaseFeature>(
          "Database")
          ->forceSyncProperties();
  bool ok = avocadodb::basics::VelocyPackHelper::velocyPackToFile(filename, data,
                                                                 doSync);
...
static bool UnloadCollectionCallback(LogicalCollection* collection) {
  ...
  auto ditches =
      avocadodb::MMFilesCollection::toMMFilesCollection(collection)->ditches();

  if (ditches->contains(avocadodb::MMFilesDitch::TRI_DITCH_DOCUMENT) ||
      ditches->contains(avocadodb::MMFilesDitch::TRI_DITCH_REPLICATION) ||
      ditches->contains(avocadodb::MMFilesDitch::TRI_DITCH_COMPACTION)) {
    locker.unlock();
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to close collection '"
...
void MMFilesEngine::unloadCollection(TRI_vocbase_t* vocbase,
  ...
  avocadodb::MMFilesCollection::toMMFilesCollection(collection)
      ->ditches()
...
std::vector<std::string> MMFilesEngine::getDatabaseNames() const {
  ...
  for (auto const& name : TRI_FilesDirectory(_databasePath.c_str())) {
    ...
    std::string const dname(
        avocadodb::basics::FileUtils::buildFilename(_databasePath, name));
...
int MMFilesEngine::saveDatabaseParameters(TRI_voc_tick_t id,
  ...
  if (!avocadodb::basics::VelocyPackHelper::velocyPackToFile(
...
TRI_vocbase_t* MMFilesEngine::openExistingDatabase(TRI_voc_tick_t id,
  ...
  try {
    ...
    for (auto const& it : VPackArrayIterator(slice)) {
      ...
      std::shared_ptr<LogicalView> view =
          std::make_shared<avocadodb::LogicalView>(vocbase.get(), it);
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error while opening database views: "
                                            << ex.what();
    throw;
  } catch (...) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
  ...
  try {
    ...
    for (auto const& it : VPackArrayIterator(slice)) {
      ...
      auto uniqCol =
          std::make_unique<avocadodb::LogicalCollection>(vocbase.get(), it);
      ...
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "added document collection '"
                                                << collection->name() << "'";
    }
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error while opening database collections: "
                                            << ex.what();
    throw;
  } catch (...) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
...
int MMFilesEngine::dropDatabaseDirectory(std::string const& path) {
  ...
  std::string const tmpfile(
      avocadodb::basics::FileUtils::buildFilename(path, ".tmp"));
...
bool MMFilesEngine::findMaxTickInJournals(std::string const& path) {
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "iterating ticks of journal '"
                                            << path << "'";
...
std::string MMFilesEngine::createViewDirectoryName(std::string const& basePath,
  ...
  return avocadodb::basics::FileUtils::buildFilename(basePath, filename);
}
...
void MMFilesEngine::saveCollectionInfo(
    TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
    avocadodb::LogicalCollection const* parameters, bool forceSync) const {
...
VPackBuilder MMFilesEngine::loadCollectionInfo(TRI_vocbase_t* vocbase,
  ...
  // find parameter file
  std::string filename =
      avocadodb::basics::FileUtils::buildFilename(path, parametersFilename());
  ...
  if (filename.substr(filename.size() - 4, 4) == ".tmp") {
    ...
    bool ok = avocadodb::basics::VelocyPackHelper::velocyPackToFile(original,
                                                                   slice, true);
  ...
  for (auto const& file : files) {
    ...
    if (next[0] == "index" && parts[1] == "json") {
      std::string filename =
          avocadodb::basics::FileUtils::buildFilename(path, file);
      std::shared_ptr<VPackBuilder> content =
          avocadodb::basics::VelocyPackHelper::velocyPackFromFile(filename);
...
int MMFilesEngine::openCollection(TRI_vocbase_t* vocbase,
  ...
  if (!stop) {
    for (auto& datafile : sealed) {
      ...
      std::string filename =
          avocadodb::basics::FileUtils::buildFilename(physical->path(), dname);
  ...
  if (stop) {
    for (auto& datafile : all) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "closing datafile '"
                                                << datafile->getName() << "'";
      delete datafile;
    }
...
char* MMFilesEngine::nextFreeMarkerPosition(LogicalCollection* collection,
  ...
  if (cache->lastFid != datafile->fid()) {
    ...
    auto ditch = avocadodb::MMFilesCollection::toMMFilesCollection(collection)
                     ->ditches()
...
std::shared_ptr<avocadodb::velocypack::Builder> MMFilesEngine::getReplicationApplierConfiguration(TRI_vocbase_t* vocbase, int& status) {
  std::string const filename = avocadodb::basics::FileUtils::buildFilename(databasePath(vocbase), "REPLICATION-APPLIER-CONFIG");
...
int MMFilesEngine::saveReplicationApplierConfiguration(TRI_vocbase_t* vocbase, avocadodb::velocypack::Slice slice, bool doSync) { 
  std::string const filename = avocadodb::basics::FileUtils::buildFilename(databasePath(vocbase), "REPLICATION-APPLIER-CONFIG");
...
int MMFilesEngine::handleSyncKeys(avocadodb::InitialSyncer& syncer,
