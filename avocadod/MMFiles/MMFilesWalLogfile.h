...
#ifndef AVOCADOD_MMFILES_WAL_LOGFILE_H
#define AVOCADOD_MMFILES_WAL_LOGFILE_H 1
...
class MMFilesWalLogfile {
 ...
 public:
  ...
  void setStatus(StatusType status) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "changing logfile status from " << statusText(_status) << " to " << statusText(status) << " for logfile " << id();
    _status = status;
  }
