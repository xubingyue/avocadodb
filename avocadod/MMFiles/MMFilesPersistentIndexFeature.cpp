...
void MMFilesPersistentIndexFeature::start() {
  ...
  auto* opts = ApplicationServer::getFeature<avocadodb::RocksDBOptionFeature>("RocksDBOption");
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "initializing rocksdb for persistent indexes, path: " << _path;
  ...
  if (! status.ok()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "unable to initialize RocksDB engine for persistent indexes: " << status.ToString();
    FATAL_ERROR_EXIT();
  }
}
...
void MMFilesPersistentIndexFeature::unprepare() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "shutting down RocksDB for persistent indexes";
  ...
  if (! status.ok()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error flushing data to RocksDB for persistent indexes: " << status.ToString();
  }
...
int MMFilesPersistentIndexFeature::syncWal() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "syncing RocksDB WAL for persistent indexes";

  rocksdb::Status status = Instance->db()->GetBaseDB()->SyncWAL();

  if (!status.ok()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error syncing RocksDB WAL for persistent indexes: " << status.ToString();
...
int MMFilesPersistentIndexFeature::dropDatabase(TRI_voc_tick_t databaseId) {
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "dropping RocksDB database: " << databaseId;
...
int MMFilesPersistentIndexFeature::dropCollection(TRI_voc_tick_t databaseId, TRI_voc_cid_t collectionId) {
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "dropping RocksDB database: " << databaseId << ", collection: " << collectionId;
...
int MMFilesPersistentIndexFeature::dropIndex(TRI_voc_tick_t databaseId, TRI_voc_cid_t collectionId, TRI_idx_iid_t indexId) {
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "dropping RocksDB database: " << databaseId << ", collection: " << collectionId << ", index: " << indexId;
...
int MMFilesPersistentIndexFeature::dropPrefix(std::string const& prefix) {
  ...
  try {
    ...
    for (size_t i = 0; i < prefix.size(); i += sizeof(TRI_idx_iid_t)) {
      ...
      if (q != nullptr) {
        LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "RocksDB prefix part: " << q;
        TRI_FreeString(TRI_CORE_MEM_ZONE, q);
      }
    }

    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "dropping RocksDB range: " << VPackSlice(l.c_str() + MMFilesPersistentIndex::keyPrefixSize()).toJson() << " - " << VPackSlice(u.c_str() + MMFilesPersistentIndex::keyPrefixSize()).toJson();
#endif
...

    {
      ...
      if (!status.ok()) {
        ...
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "RocksDB file deletion failed";
      }
    }
    ...
    if (!status.ok()) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "RocksDB key deletion failed: " << status.ToString();
      return TRI_ERROR_INTERNAL;
    }

    return TRI_ERROR_NO_ERROR;
  } catch (avocadodb::basics::Exception const& ex) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception during RocksDB key prefix deletion: " << ex.what();
    return ex.code();
  } catch (std::exception const& ex) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception during RocksDB key prefix deletion: " << ex.what();
    return TRI_ERROR_INTERNAL;
  } catch (...) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught unknown exception during RocksDB key prefix deletion";
    return TRI_ERROR_INTERNAL;
  }
}
