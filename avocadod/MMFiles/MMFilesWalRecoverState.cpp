...
void MMFilesWalRecoverState::releaseResources() {
  ...
  for (auto it = openedCollections.begin(); it != openedCollections.end();
       ++it) {
    avocadodb::LogicalCollection* collection = it->second;
...
TRI_vocbase_t* MMFilesWalRecoverState::releaseDatabase(
  ...
  while (it2 != openedCollections.end()) {
    avocadodb::LogicalCollection* collection = it2->second;
...
avocadodb::LogicalCollection* MMFilesWalRecoverState::releaseCollection(
...
avocadodb::LogicalCollection* MMFilesWalRecoverState::useCollection(
...
LogicalCollection* MMFilesWalRecoverState::getCollection(
  ...
  if (collection == nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "collection " << collectionId
...
int MMFilesWalRecoverState::executeSingleOperation(
  ...
  try {
    SingleCollectionTransaction trx(
        avocadodb::transaction::StandaloneContext::Create(vocbase), collectionId,
...
bool MMFilesWalRecoverState::ReplayMarker(MMFilesMarker const* marker,
  ...
  try {
    switch (type) {
      ...
      case TRI_DF_MARKER_VPACK_DOCUMENT: {
        ...
        int res = state->executeSingleOperation(
            ...
            [&](SingleCollectionTransaction* trx,
              ...
              if (avocadodb::MMFilesCollection::toMMFilesCollection(
      ...
      case TRI_DF_MARKER_VPACK_RENAME_COLLECTION: {
        ...
        if (vocbase == nullptr) {
          ...
          LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "cannot open database "
                                                    << databaseId;
          return true;
        }
        ...
        if (collection == nullptr) {
          ...
          LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "cannot open collection "
                                                    << collectionId;
          return true;
        }
        ...
        avocadodb::LogicalCollection* other = vocbase->lookupCollection(name);
      ...
      case TRI_DF_MARKER_VPACK_CREATE_INDEX: {
        ...
        std::string const indexName("index-" + std::to_string(indexId) +
                                    ".json");
        std::string const filename(avocadodb::basics::FileUtils::buildFilename(
            physical->path(), indexName));

        bool const forceSync = state->willBeDropped(databaseId, collectionId);
        bool ok = avocadodb::basics::VelocyPackHelper::velocyPackToFile(
        ...
        if (!ok) {
          ...
          std::shared_ptr<avocadodb::Index> unused;
      ...
      case TRI_DF_MARKER_VPACK_CREATE_DATABASE: {
        ...
        */
        ...
        if (res != TRI_ERROR_NO_ERROR) {
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "cannot create database "
                                                   << databaseId << ": "
                                                   << TRI_errno_string(res);
          ++state->errorCount;
          return state->canContinue();
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "cannot replay marker: "
                                             << ex.what();
    ++state->errorCount;
    return state->canContinue();
  } catch (...) {
...
int MMFilesWalRecoverState::removeEmptyLogfiles() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "removing empty WAL logfiles";
...
int MMFilesWalRecoverState::fillIndexes() {
  ...
  for (auto it = openedCollections.begin(); it != openedCollections.end();
    ...
    avocadodb::SingleCollectionTransaction trx(
        avocadodb::transaction::StandaloneContext::Create(collection->vocbase()),
