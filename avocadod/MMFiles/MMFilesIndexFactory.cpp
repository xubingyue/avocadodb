...
std::shared_ptr<Index> MMFilesIndexFactory::prepareIndexFromSlice(
  ...
  switch (type) {
    ...
    case avocadodb::Index::TRI_IDX_TYPE_GEO2_INDEX: {
      newIdx.reset(new avocadodb::MMFilesGeoIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_HASH_INDEX: {
      newIdx.reset(new avocadodb::MMFilesHashIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_SKIPLIST_INDEX: {
      newIdx.reset(new avocadodb::MMFilesSkiplistIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_PERSISTENT_INDEX: {
      newIdx.reset(new avocadodb::MMFilesPersistentIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_FULLTEXT_INDEX: {
      newIdx.reset(new avocadodb::MMFilesFulltextIndex(iid, col, info));
      break;
    }
...
void MMFilesIndexFactory::fillSystemIndexes(
  ...
  systemIndexes.emplace_back(
      std::make_shared<avocadodb::MMFilesPrimaryIndex>(col));

  // create edges index
  if (col->type() == TRI_COL_TYPE_EDGE) {
    systemIndexes.emplace_back(
        std::make_shared<avocadodb::MMFilesEdgeIndex>(1, col));
  }
}
