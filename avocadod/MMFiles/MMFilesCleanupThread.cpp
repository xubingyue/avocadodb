...
void MMFilesCleanupThread::run() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "shutting down cleanup thread";
}
...
void MMFilesCleanupThread::cleanupCursors(bool force) {
  ...
  try {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "caught exception during cursor cleanup";
  }
}

/// @brief checks all datafiles of a collection
void MMFilesCleanupThread::cleanupCollection(avocadodb::LogicalCollection* collection) {
  ...
  while (true) {
    ...
    auto callback = [&](avocadodb::MMFilesDitch const* ditch) -> bool {
      if (ditch->type() == avocadodb::MMFilesDitch::TRI_DITCH_COLLECTION_UNLOAD) {
    ...
    if (!popped) {
      ...
      if (gotStatus && !isUnloading) {
        popped = false;
        auto unloader =
            ditches->process(popped, [](avocadodb::MMFilesDitch const* ditch) -> bool {
              return (ditch->type() ==
                      avocadodb::MMFilesDitch::TRI_DITCH_COLLECTION_UNLOAD);
            });
        if (popped) {
    ...
    if (type == avocadodb::MMFilesDitch::TRI_DITCH_DATAFILE_DROP) {
      static_cast<avocadodb::MMFilesDropDatafileDitch*>(ditch)->executeCallback();
      delete ditch;
      // next iteration
    } else if (type == avocadodb::MMFilesDitch::TRI_DITCH_DATAFILE_RENAME) {
      static_cast<avocadodb::MMFilesRenameDatafileDitch*>(ditch)->executeCallback();
      delete ditch;
      // next iteration
    } else if (type == avocadodb::MMFilesDitch::TRI_DITCH_COLLECTION_UNLOAD) {
      // collection will be unloaded
      bool hasUnloaded = static_cast<avocadodb::MMFilesUnloadCollectionDitch*>(ditch)
                             ->executeCallback();
      delete ditch;
    ...
    } else if (type == avocadodb::MMFilesDitch::TRI_DITCH_COLLECTION_DROP) {
      // collection will be dropped
      bool hasDropped = static_cast<avocadodb::MMFilesDropCollectionDitch*>(ditch)
                            ->executeCallback();
      delete ditch;
      ...
      // unknown type
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "unknown ditch type '" << type << "'"; FATAL_ERROR_EXIT();
    }
