...
MMFilesPersistentIndexIterator::MMFilesPersistentIndexIterator(
    ...
    ManagedDocumentResult* mmdr, avocadodb::MMFilesPersistentIndex const* index,
    avocadodb::MMFilesPrimaryIndex* primaryIndex,
  ...
  _leftEndpoint.reset(new avocadodb::velocypack::Buffer<char>());
  ...
  _rightEndpoint.reset(new avocadodb::velocypack::Buffer<char>());
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "prefix: " <<
  // fasthash64(prefix.c_str(), prefix.size(), 0);
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "iterator left key: " <<
  // left.toJson();
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "iterator right key: " <<
  // right.toJson();
...
bool MMFilesPersistentIndexIterator::next(TokenCallback const& cb,
  ...
  while (limit > 0) {
    ...
    // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "cursor key: " <<
    // VPackSlice(key.data() +
    ...
    // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "comparing: " <<
    ...
    if (res <= 0) {
      ...
      // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "looking up document with
      // key: " << keySlice.toJson();
      // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "looking up document with
...
Result MMFilesPersistentIndex::remove(transaction::Methods* trx,
  ...
  for (size_t i = 0; i < count; ++i) {
    // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "removing key: " <<
...
avocadodb::aql::AstNode* MMFilesPersistentIndex::specializeCondition(
