...
/// @brief calculate a CRC value the same way as AvocadoDB 2.8 did
...
/// @brief check if a marker appears to be created by AvocadoDB 2.8
...
// nothing to do
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot memory map anonymous region: " << TRI_last_error();
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "The database directory might reside on a shared folder "
...
static MMFilesDatafile* CreatePhysicalDatafile(std::string const& filename,
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot memory map file '" << filename << "': '" << TRI_errno_string(res) << "'";
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "The database directory might reside on a shared folder "
...
MMFilesDatafile* MMFilesDatafile::create(std::string const& filename, TRI_voc_fid_t fid,
  ...
  if (withInitialMarkers) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot write header to datafile '" << datafile->getName() << "'";
  ...
  LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "created datafile '" << datafile->getName() << "' of size " << maximalSize << " and page-size " << pageSize;
...
int MMFilesDatafile::writeElement(void* position, MMFilesMarker const* marker, bool forceSync) {
  ...
  if (forceSync) {
    ...
    if (!ok) {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "msync failed with: " << TRI_last_error();

      return _lastError;
    } else {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "msync succeeded " << (void*) position << ", size " << marker->getSize();
    }
  }
...
bool TRI_IterateDatafile(MMFilesDatafile* datafile,
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "iterating over datafile '" << datafile->getName() << "', fid: " << datafile->fid();
...
bool MMFilesDatafile::tryRepair(std::string const& path) {
  ...
  if (!datafile->readWrite()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to change file protection for datafile '" << datafile->getName() << "'";
    return false;
  }
...
int MMFilesDatafile::close() {
  if (_state == TRI_DF_STATE_READ ||
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "munmap failed with: " << res;
      _state = TRI_DF_STATE_WRITE_ERROR;
      _lastError = res;
      return res;
    }
    ...
    if (isPhysical()) {
      ...
      if (res != TRI_ERROR_NO_ERROR) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to close datafile '" << getName() << "': " << res;
      }
    }
  ...
  if (_state == TRI_DF_STATE_CLOSED) {
    TRI_ASSERT(_fd == -1);
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "closing an already closed datafile '" << getName() << "'";
    return TRI_ERROR_NO_ERROR;
  } 
...
int MMFilesDatafile::truncateAndSeal(TRI_voc_size_t position) {
  ...
  // open the file
  std::string filename = avocadodb::basics::FileUtils::buildFilename(getName(), ".new");
  ...
  if (fd < 0) {
    TRI_SYSTEM_ERROR();
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot create new datafile '" << filename << "': " << TRI_GET_ERRORBUF;
  ...
  if (offset == (TRI_lseek_t)-1) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot seek in new datafile '" << filename << "': " << TRI_GET_ERRORBUF;
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot memory map file '" << filename << "': " << TRI_GET_ERRORBUF;
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "The database directory might reside on a shared folder "
                "(VirtualBox, VMWare) or an NFS "
                "mounted volume which does not allow memory mapped files.";
  ...
  // rename files
  std::string oldname = avocadodb::basics::FileUtils::buildFilename(_filename, ".corrupted");
...
bool MMFilesDatafile::check(bool ignoreFailures) {
  // this function must not be called for non-physical datafiles
  TRI_ASSERT(isPhysical());
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "checking markers in datafile '" << getName() << "'";
  ...
  if (_currentSize == 0) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "current size is 0 in read-only datafile '" << getName() << "', trying to fix";
  ...
  while (ptr < end) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "MARKER: size " << size << ", tick " << tick << ", crc " << marker->getCrc() << ", type " << type;
#endif

    if (size == 0) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "reached end of datafile '" << getName() << "' data, current size " << currentSize;
    ...
    if (size < sizeof(MMFilesMarker)) {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "marker in datafile '" << getName() << "' too small, size " << size << ", should be at least " << sizeof(MMFilesMarker);
    ...
    if (ptr + size > end) {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "marker in datafile '" << getName() << "' points with size " << size << " beyond end of file";
      if (lastGood != nullptr) {
        LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "last good marker found at: " << hexValue(static_cast<uint64_t>(static_cast<uintptr_t>(lastGood - _data)));
      }
    ...
    if (!TRI_IsValidMarkerDatafile(marker)) {
      if (type == 0 && size < 128) {
        ...
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "ignoring suspicious marker in datafile '" << getName() << "': type: " << type << ", size: " << size;
      } else {
        if (ignoreFailures) {
          return fix(currentSize);
        }
        ...
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "marker in datafile '" << getName() << "' is corrupt: type: " << type << ", size: " << size;
        if (lastGood != nullptr) {
          LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "last good marker found at: " << hexValue(static_cast<uint64_t>(static_cast<uintptr_t>(lastGood - _data)));
        }
    ...
    if (type != 0) {
      ...
      if (!ok) {
        ...
        if (size > 0) {
          ...
          if (p < end) {
            ...
            if (isFollowedByNullBytes) {
              ...
              // place
              LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "datafile '" << getName() << "' automatically truncated at last marker";
              ignoreFailures = true;
            } else {
        ...
        if (!ignoreFailures) {
          ...
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "crc mismatch found in datafile '" << getName() << "' of size "
                    << _maximalSize << ", at position " << currentSize;

          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "crc mismatch found inside marker of type '" << TRI_NameMarkerDatafile(marker) 
                    << "' and size " << size
          ...
          if (nextMarkerOk) {
            LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "data directly following this marker looks ok so repairing the marker manually may recover it...";
            LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "to truncate the file at this marker, please restart the server with the parameter '--wal.ignore-logfile-errors true' if the error happening during WAL recovery, or with parameter '--database.ignore-datafile-errors true' if it happened after WAL recovery";
          } else {
            LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "data directly following this marker cannot be analyzed";
          }
    ...
    if (marker->getType() == TRI_DF_MARKER_FOOTER) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "found footer, reached end of datafile '" << getName() << "', current size " << currentSize;
...
void MMFilesDatafile::printMarker(MMFilesMarker const* marker, TRI_voc_size_t size, char const* begin, char const* end) const {
  LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "raw marker data following:";
  LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "type: " << TRI_NameMarkerDatafile(marker) << ", size: " << marker->getSize() << ", crc: " << marker->getCrc();
  LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "(expected layout: size (4 bytes), crc (4 bytes), type and tick (8 bytes), payload following)";
  ...
  while (p < e) {
    ...
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << line << "  " << raw;
    line.clear();
    raw.clear();

    if (printed >= 2048) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "(output truncated due to excessive length)";
      break;
    }
  }
}
...
bool MMFilesDatafile::fix(TRI_voc_size_t currentSize) {
  LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "datafile '" << getName() << "' is corrupted at position " << currentSize;

  LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "setting datafile '" << getName() << "' to read-only and ignoring all data from this file beyond this position";
      ...
      if (!CheckCrcMarker(marker, end)) {
        ...
        if (p < end) {
          ...
          if (isFollowedByNullBytes) {
            // only last marker in datafile was corrupt. fix the datafile in
            // place
            LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "truncating datafile '" << getName() << "' at position " << currentSize;
          ...
          if (next < end) {
            ...
            if (nextMarker->getType() != 0 &&
              ...
              if (ok) {
                LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "zeroed single invalid marker in datafile '" << getName() << "' at position " << currentSize;
              } else {
                LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not zero single invalid marker in datafile '" << getName() << "' at position " << currentSize;
                return false;
              }
            } else {
...
MMFilesDatafile* MMFilesDatafile::open(std::string const& filename, bool ignoreFailures) {
  ...
  if (!ok) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "datafile '" << datafile->getName() << "' is corrupt";
  ...
  if (!datafile->_isSealed) {
    if (!datafile->readWrite()) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to change memory protection for memory backed by datafile '" << datafile->getName() << "'. please check file permissions and mount options.";
      return nullptr;
    }
    datafile->setState(TRI_DF_STATE_WRITE); 
  } 
...
MMFilesDatafile* MMFilesDatafile::openHelper(std::string const& filename, bool ignoreErrors) {
  ...
  if (fd < 0) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot open datafile '" << filename << "': '" << TRI_GET_ERRORBUF << "'";
  ...
  if (res < 0) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot get status of datafile '" << filename << "': " << TRI_GET_ERRORBUF;
  ...
  if (!ok) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot read datafile header from '" << filename << "': " << TRI_last_error();
  ...
  if (!ok) {
    if (IsMarker28(ptr)) {
      TRI_TRACKED_CLOSE_FILE(fd);
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "datafile found from older version of AvocadoDB. "
               << "Please dump data from that version with avocadodump "
               << "and reload it into this AvocadoDB instance with avocadorestore";
      FATAL_ERROR_EXIT();
    }
  ...
  if (size > header->_maximalSize) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "datafile '" << filename << "' has size " << size << ", but maximal size is " << header->_maximalSize;
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot memory map datafile '" << filename << "': " << TRI_errno_string(res);
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "The database directory might reside on a shared folder "
                "(VirtualBox, VMWare) or an NFS-mounted volume which does not allow memory mapped files.";
    return nullptr;
  }
