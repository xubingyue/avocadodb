...
MMFilesWalLogfile* MMFilesWalLogfile::createNew(std::string const& filename, MMFilesWalLogfile::IdType id,
  ...
  if (df == nullptr) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to create logfile '" << filename << "': " << TRI_errno_string(res);
      return nullptr;
    }
  }
...
MMFilesWalLogfile* MMFilesWalLogfile::openExisting(std::string const& filename, MMFilesWalLogfile::IdType id,
  ...
  if (df == nullptr) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to open logfile '" << filename << "': " << TRI_errno_string(res);
      return nullptr;
    }

    // cannot figure out the type of error
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to open logfile '" << filename << "'";
    return nullptr;
  }
