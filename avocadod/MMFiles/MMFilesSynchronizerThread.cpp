  ...
  while (true) {
    if (waitingWithoutSync > 0 || waitingWithSync > 0 || ++iterations == 10) {
      ...
      try {
        ...
        int res = ex.code();
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unexpected error in synchronizerThread: "
                 << TRI_errno_string(res);
      } catch (...) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unspecific error in synchronizerThread";
      }
    }
...
int MMFilesSynchronizerThread::doSync(bool& checkMore) {
  ...
  if (TRI_microtime() - startTime > 1.0) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "Long sync logfile " << id << ", region "
      << (void*) region.mem << ", size " << region.size;
  }

  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "syncing logfile " << id << ", region " << (void*) region.mem << " - "
