...
void MMFilesRemoverThread::run() {
  ...
  while (!isStopping()) {
    ...
    try {
      ...
      int res = ex.code();
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unexpected error in removerThread::run: "
               << TRI_errno_string(res);
    } catch (...) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unspecific error in removerThread::run";
    }
