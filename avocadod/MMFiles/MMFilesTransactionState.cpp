...
Result MMFilesTransactionState::commitTransaction(transaction::Methods* activeTrx) {
  ...
  if (_nestingLevel == 0) {
    ...
    if (AccessMode::isWriteOrExclusive(_type) &&
        !_collections.empty() &&
        !isSingleOperation() && 
        avocadodb::aql::QueryCache::instance()->mayBeActive()) {
      clearQueryCache();
    }
...
int MMFilesTransactionState::addOperation(TRI_voc_rid_t revisionId,
  ...
  if (marker->fid() == 0) {
    ...
    if (localWaitForSync) {
      ...
      if (collection->getPhysical()->hasIndexOfType(avocadodb::Index::TRI_IDX_TYPE_PERSISTENT_INDEX)) {
...
int MMFilesTransactionState::writeBeginMarker() {
  ...
  try {
    ...
    res = ex.code();
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction begin marker in log: " << ex.what();
  } catch (std::exception const& ex) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction begin marker in log: " << ex.what();
  } catch (...) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction begin marker in log: unknown exception";
  }
...
int MMFilesTransactionState::writeAbortMarker() {
  ...
  try {
    ...
    res = ex.code();
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction abort marker in log: " << ex.what();
  } catch (std::exception const& ex) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction abort marker in log: " << ex.what();
  } catch (...) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction abort marker in log: unknown exception";
  }
...
int MMFilesTransactionState::writeCommitMarker() {
  ...
  try {
    ...
    if (_options.waitForSync) {
      ...
      allCollections([&hasPersistentIndex](TransactionCollection* collection) {
        ...
        if (c->canAccess(AccessMode::Type::WRITE) && 
            c->collection()->getPhysical()->hasIndexOfType(avocadodb::Index::TRI_IDX_TYPE_PERSISTENT_INDEX)) {
    ...
    res = ex.code();
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction commit marker in log: " << ex.what();
  } catch (std::exception const& ex) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction commit marker in log: " << ex.what();
  } catch (...) {
    res = TRI_ERROR_INTERNAL;
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not save transaction commit marker in log: unknown exception";
  }
