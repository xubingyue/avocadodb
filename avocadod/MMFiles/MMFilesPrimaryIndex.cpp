...
static std::vector<std::vector<avocadodb::basics::AttributeName>> const
    IndexAttributes{{avocadodb::basics::AttributeName("_id", false)},
...
MMFilesPrimaryIndex::MMFilesPrimaryIndex(
    ...
    : Index(0, collection,
            std::vector<std::vector<avocadodb::basics::AttributeName>>(
                {{avocadodb::basics::AttributeName(StaticStrings::KeyString,
                                                  false)}}),
  ...
  if (collection != nullptr) {
    ...
    auto physical =
        static_cast<avocadodb::MMFilesCollection*>(collection->getPhysical());
...
MMFilesSimpleIndexElement MMFilesPrimaryIndex::lookupSequential(
    transaction::Methods* trx, avocadodb::basics::BucketPosition& position,
    uint64_t& total) {
  ManagedDocumentResult result;
  IndexLookupContext context(trx, _collection, &result, 1);
...
avocadodb::aql::AstNode* MMFilesPrimaryIndex::specializeCondition(
...
void MMFilesPrimaryIndex::handleValNode(transaction::Methods* trx,
                                        VPackBuilder* keys,
                                        avocadodb::aql::AstNode const* valNode,
                                        bool isId) const {
