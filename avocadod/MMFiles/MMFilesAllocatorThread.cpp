...
void MMFilesAllocatorThread::run() {
  while (!isStopping()) {
    ...
    try {
      ...
      res = ex.code();
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unexpected error in allocatorThread: "
               << TRI_errno_string(res);
    } catch (...) {
      res = TRI_ERROR_INTERNAL;
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "got unspecific error in allocatorThread";
    }

    if (worked) {
      if (res != TRI_ERROR_NO_ERROR) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to create new WAL reserve logfile: " << TRI_errno_string(res);
      }
