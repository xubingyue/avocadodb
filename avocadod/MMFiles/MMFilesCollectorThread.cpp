...
int MMFilesCollectorThread::processCollectionOperations(MMFilesCollectorCache* cache) {
  avocadodb::DatabaseGuard dbGuard(cache->databaseId);
  TRI_vocbase_t* vocbase = dbGuard.database();
  TRI_ASSERT(vocbase != nullptr);

  avocadodb::CollectionGuard collectionGuard(vocbase, cache->collectionId, true);
  avocadodb::LogicalCollection* collection = collectionGuard.collection();
  ...
  try {
    ...
    static_cast<avocadodb::MMFilesCollection*>(collection->getPhysical())->decreaseUncollectedLogfileEntries(cache->totalOperationsCount);
...
int MMFilesCollectorThread::transferMarkers(MMFilesWalLogfile* logfile,
  ...
  avocadodb::DatabaseGuard dbGuard(databaseId);
  TRI_vocbase_t* vocbase = dbGuard.database();
  TRI_ASSERT(vocbase != nullptr);

  avocadodb::CollectionGuard collectionGuard(vocbase, collectionId, true);
  avocadodb::LogicalCollection* collection = collectionGuard.collection();
  TRI_ASSERT(collection != nullptr);
...
int MMFilesCollectorThread::queueOperations(avocadodb::MMFilesWalLogfile* logfile,
