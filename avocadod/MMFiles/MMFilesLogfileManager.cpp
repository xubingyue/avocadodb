...
void MMFilesLogfileManager::validateOptions(std::shared_ptr<options::ProgramOptions> options) {
  if (_filesize < MinFileSize()) {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --wal.logfile-size. Please use a value of "
                  "at least "
               << MinFileSize();
    FATAL_ERROR_EXIT();
  }
  ...
  if (_numberOfSlots < MinSlots() || _numberOfSlots > MaxSlots()) {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --wal.slots. Please use a value between "
  ...
  if (_throttleWhenPending > 0 &&
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --wal.throttle-when-pending. Please use a "
                  "value of at least "
  ...
  if (_syncInterval < MinSyncInterval()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --wal.sync-interval. Please use a value "
                  "of at least "
               << MinSyncInterval();
    FATAL_ERROR_EXIT();
  }
...
void MMFilesLogfileManager::prepare() {
  ...
  if (shutdownFileExists) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "shutdown file found";

    int res = readShutdownInfo();

    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not open shutdown file '" << _shutdownFile
                 << "': " << TRI_errno_string(res);
      FATAL_ERROR_EXIT();
    }
  } else {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "no shutdown file found";
  }
}
...
void MMFilesLogfileManager::start() {
  ...
  if (_directory.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "no directory specified for WAL logfiles. Please use the "
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not create WAL logfile inventory: "
               << TRI_errno_string(res);
    FATAL_ERROR_EXIT();
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not inspect WAL logfiles: " << TRI_errno_string(res);
    FATAL_ERROR_EXIT();
  }

  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "WAL logfile manager configuration: historic logfiles: "
...
bool MMFilesLogfileManager::open() {
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start WAL allocator thread: "
               << TRI_errno_string(res);
    return false;
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start WAL synchronizer thread: "
               << TRI_errno_string(res);
    return false;
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not abort open transactions: " << TRI_errno_string(res);
    return false;
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start WAL collector thread: "
               << TRI_errno_string(res);
    return false;
  }
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start WAL remover thread: " << TRI_errno_string(res);
    return false;
  }
...
void MMFilesLogfileManager::unprepare() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "shutting down WAL";
  ...
  if (_allocatorThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping allocator thread";
  ...
  // stop other threads
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "sending shutdown request to WAL threads";
  ...
  if (_removerThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping remover thread";
...

  {
    ...
    if (_collectorThread != nullptr) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping collector thread";
  ...
  if (_synchronizerThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping synchronizer thread";
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "closing logfiles";
  closeLogfiles();
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not write WAL shutdown info: " << TRI_errno_string(res);
  }
}
...
int MMFilesLogfileManager::flush(bool waitForSync, bool waitForCollector,
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "about to flush active WAL logfile. currentLogfileId: "
  ...
  if (waitForCollector) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      ...
      // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "entering waitForCollector with lastOpenLogfileId " << lastOpenLogfileId;
      ...
      if (lastSealedLogfileId > 0) {
        ...
        if (res == TRI_ERROR_LOCK_TIMEOUT) {
          LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "got lock timeout when waiting for WAL flush. lastSealedLogfileId: " << lastSealedLogfileId;
        }
      }
    }
  }
...
int MMFilesLogfileManager::getLogfileDescriptor(MMFilesWalLogfile::IdType id) {
  ...
  if (it == _logfiles.end()) {
    // error
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not find logfile " << id;
    return -1;
  }
...
int MMFilesLogfileManager::getWriteableLogfile(uint32_t size,
  ...
  while (true) {
    {
      ...
      while (it != _logfiles.end()) {
        ...
        if (logfile->isWriteable(size)) {
          ...
          {
            // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "setting lastOpenedId " << logfile->id();
            MUTEX_LOCKER(mutexLocker, _idLock);
  ...
  TRI_ASSERT(result == nullptr);
  LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to acquire writeable WAL logfile after " << _flushTimeout << " ms";
...
void MMFilesLogfileManager::setCollectionDone(MMFilesWalLogfile* logfile) {
  ...
  // LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "setCollectionDone setting lastCollectedId to " << id
  {
    WRITE_LOCKER(writeLocker, _logfilesLock);
...
void MMFilesLogfileManager::removeLogfile(MMFilesWalLogfile* logfile) {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "removing logfile '" << filename << "'";
  ...
  if (!basics::FileUtils::remove(filename, &res)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to remove logfile '" << filename
             << "': " << TRI_errno_string(res);
  }
}
...
void MMFilesLogfileManager::waitForCollector() {
  while (true) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "waiting for WAL collector";
    usleep(50000);
  }
}
...
int MMFilesLogfileManager::waitForCollector(MMFilesWalLogfile::IdType logfileId,
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "waiting for collector thread to collect logfile " << logfileId;
  ...
  while (true) {
    ...
    // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "still waiting for collector. logfileId: " << logfileId <<
  ...
  LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "going into lock timeout. having waited for logfile: " << logfileId << ", maxWaitTime: " << maxWaitTime;
  logStatus();
...
void MMFilesLogfileManager::logStatus() {
  LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "logfile manager status report: lastCollectedId: " << _lastCollectedId.load() << ", lastSealedId: " << _lastSealedId.load();
  READ_LOCKER(locker, _logfilesLock);
  for (auto logfile : _logfiles) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "- logfile " << logfile.second->id() << ", filename '" << logfile.second->filename()
...
int MMFilesLogfileManager::runRecovery() {
  ...
  if (_ignoreRecoveryErrors) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "running WAL recovery ("
  ...
  if (_recoverState->errorCount == 0) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "WAL recovery finished successfully";
  } else {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "WAL recovery finished, some errors ignored due to settings";
  }
...
int MMFilesLogfileManager::readShutdownInfo() {
  ...
  try {
    builder = avocadodb::basics::VelocyPackHelper::velocyPackFromFile(_shutdownFile);
  } catch (...) {
  ...
  uint64_t lastTick =
      avocadodb::basics::VelocyPackHelper::stringUInt64(slice.get("tick"));
  ...
  uint64_t hlc =
      avocadodb::basics::VelocyPackHelper::stringUInt64(slice.get("hlc"));
  TRI_HybridLogicalClock(static_cast<TRI_voc_tick_t>(hlc));

  // read id of last collected logfile (maybe 0)
  uint64_t lastCollectedId = avocadodb::basics::VelocyPackHelper::stringUInt64(
      slice.get("lastCollected"));

  // read if of last sealed logfile (maybe 0)
  uint64_t lastSealedId =
      avocadodb::basics::VelocyPackHelper::stringUInt64(slice.get("lastSealed"));
  ...
  std::string const shutdownTime =
      avocadodb::basics::VelocyPackHelper::getStringValue(slice, "shutdownTime",
                                                         "");
  if (shutdownTime.empty()) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "no previous shutdown time found";
  } else {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "previous shutdown was at '" << shutdownTime << "'";
  }

  {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "initial values for WAL logfile manager: tick: " << lastTick
               << ", hlc: " << hlc 
...
int MMFilesLogfileManager::writeShutdownInfo(bool writeShutdownTime) {
  ...
  try {
    ...
    bool ok;
    {
      ...
      ok = avocadodb::basics::VelocyPackHelper::velocyPackToFile(
          _shutdownFile, builder.slice(), true);
    }

    if (!ok) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to write WAL state file '" << _shutdownFile << "'";
...
void MMFilesLogfileManager::stopMMFilesSynchronizerThread() {
  if (_synchronizerThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping WAL synchronizer thread";
...
void MMFilesLogfileManager::stopMMFilesAllocatorThread() {
  if (_allocatorThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping WAL allocator thread";
...
void MMFilesLogfileManager::stopMMFilesCollectorThread() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping WAL collector thread";
...
void MMFilesLogfileManager::stopMMFilesRemoverThread() {
  if (_removerThread != nullptr) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "stopping WAL remover thread";
...
int MMFilesLogfileManager::inventory() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "scanning WAL directory: '" << _directory << "'";
  ...
  for (auto it = files.begin(); it != files.end(); ++it) {
    ...
    if (StringUtils::isPrefix(file, "logfile-") &&
      ...
      if (id == 0) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "encountered invalid id for logfile '" << file
                  << "'. ids must be > 0";
      } else {
        // update global tick
...
int MMFilesLogfileManager::inspectLogfiles() {
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "inspecting WAL logfiles";
  ...
  for (auto it = _logfiles.begin(); it != _logfiles.end(); ++it) {
    ...
    if (logfile != nullptr) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "logfile " << logfile->id() << ", filename '" << logfile->filename()
  ...
  for (auto it = _logfiles.begin(); it != _logfiles.end(); /* no hoisting */) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "inspecting logfile " << logfile->id() << " ("
               << logfile->statusText() << ")";
  ...
  // should it be lower
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "setting max HLC value to " << _recoverState->maxRevisionId;
...
int MMFilesLogfileManager::createReserveLogfile(uint32_t size) {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "creating empty logfile '" << filename << "' with size "
             << size;
  ...
  if (logfile == nullptr) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to create logfile: " << TRI_errno_string(res);
    return res;
  }
...
int MMFilesLogfileManager::ensureDirectory() {
  ...
  if (!basics::FileUtils::isDirectory(directory)) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "WAL directory '" << directory
              << "' does not exist. creating it...";

    int res;
    if (!basics::FileUtils::createDirectory(directory, &res)) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not create WAL directory: '" << directory
               << "': " << TRI_last_error();
  ...
  if (!basics::FileUtils::isDirectory(directory)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "WAL directory '" << directory << "' does not exist";
