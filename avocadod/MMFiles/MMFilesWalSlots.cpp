  ...
  while (++iterations < 1000) {
    {
      ...
      if (slot->isUnused() && 
        ...
        if (mem == nullptr) {
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not find free WAL slot"; 
  ...
  while (true) {
    {
      ...
      if (slot->isUnused()) {
        ...
        if (_logfile != nullptr) {
          ...
          if (res != TRI_ERROR_NO_ERROR) {
            LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not write logfile footer: " << TRI_errno_string(res);
            return res;
          }
        ...
        if (res != TRI_ERROR_NO_ERROR) {
          ...
          if (status == MMFilesWalLogfile::StatusType::EMPTY) {
            ...
            if (res != TRI_ERROR_NO_ERROR) {
              LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not write logfile header: " << TRI_errno_string(res);
              return res;
            }
