...
struct MMFilesWalRecoverState {
  ...
  avocadodb::LogicalCollection* releaseCollection(TRI_voc_cid_t);

  /// @brief gets a collection (and inserts it into the cache if not in it)
  avocadodb::LogicalCollection* useCollection(TRI_vocbase_t*, TRI_voc_cid_t, int&);
  
  avocadodb::LogicalView* releaseView(TRI_voc_cid_t);
  ...
  avocadodb::LogicalCollection* getCollection(TRI_voc_tick_t, TRI_voc_cid_t);
  ...
  std::unordered_map<TRI_voc_cid_t, avocadodb::LogicalCollection*> openedCollections;
