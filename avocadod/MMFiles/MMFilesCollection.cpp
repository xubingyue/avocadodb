...
using Helper = avocadodb::basics::VelocyPackHelper;
...
avocadodb::Result MMFilesCollection::updateProperties(VPackSlice const& slice,
                                                     bool doSync) {
  // validation
  uint32_t tmp = avocadodb::basics::VelocyPackHelper::getNumericValue<uint32_t>(
      slice, "indexBuckets",
  ...
  int64_t count = avocadodb::basics::VelocyPackHelper::getNumericValue<int64_t>(
...
avocadodb::Result MMFilesCollection::persistProperties() {
  Result res;
  try {
...
bool MMFilesCollection::OpenIterator(MMFilesMarker const* marker,
  ...
  if (type == TRI_DF_MARKER_VPACK_DOCUMENT) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "skipping marker type "
...
MMFilesCollection::MMFilesCollection(LogicalCollection* collection,
    ...
    : PhysicalCollection(collection, info),
      ...
      _isVolatile(avocadodb::basics::VelocyPackHelper::readBooleanValue(
...
TRI_voc_rid_t MMFilesCollection::revision(
    avocadodb::transaction::Methods*) const {
...
int MMFilesCollection::sealDatafile(MMFilesDatafile* datafile,
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to seal journal '"
                                            << datafile->getName()
  ...
  if (!isCompactor && datafile->isPhysical()) {
    ...
    std::string filename =
        avocadodb::basics::FileUtils::buildFilename(path(), dname);

    res = datafile->rename(filename);

    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "closed file '"
                                                << datafile->getName() << "'";
    } else {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
...
MMFilesDatafile* MMFilesCollection::createDatafile(TRI_voc_fid_t fid,
  ...
  if (isVolatile()) {
    ...
    std::string filename =
        avocadodb::basics::FileUtils::buildFilename(path(), jname);
  ...
  if (isCompactor) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "created new compactor '"
                                              << datafile->getName() << "'";
  } else {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "created new journal '"
                                              << datafile->getName() << "'";
  }
...
void MMFilesCollection::figuresSpecific(
    std::shared_ptr<avocadodb::velocypack::Builder>& builder) {
...
void MMFilesCollection::fillIndex(
    std::shared_ptr<avocadodb::basics::LocalTaskQueue> queue, transaction::Methods* trx,
    avocadodb::Index* idx,
    std::vector<std::pair<TRI_voc_rid_t, VPackSlice>> const& documents,
    bool skipPersistent) {
...
avocadodb::MMFilesPrimaryIndex* MMFilesCollection::primaryIndex() const {
  ...
  return static_cast<avocadodb::MMFilesPrimaryIndex*>(primary.get());
}
...
int MMFilesCollection::fillIndexes(
    transaction::Methods* trx,
    std::vector<std::shared_ptr<avocadodb::Index>> const& indexes,
    bool skipPersistent) {
  ...
  auto queue = std::make_shared<avocadodb::basics::LocalTaskQueue>(ioService);
  ...
  try {
    ...
    if (nrUsed > 0) {
      avocadodb::basics::BucketPosition position;
      uint64_t total = 0;
...
int MMFilesCollection::openWorker(bool ignoreErrors) {
  ...
  try {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "cannot open '" << path()
                                                << "', check failed";
      return res;
    }
...
void MMFilesCollection::open(bool ignoreErrors) {
  ...
  avocadodb::SingleCollectionTransaction trx(
      avocadodb::transaction::StandaloneContext::Create(vocbase), cid,
  ...
  if (_logicalCollection->version() <= LogicalCollection::VERSION_30 &&
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME)
        ...
        << "' contains _rev values that are higher than expected for an "
           "AvocadoDB 3.1 database. If this collection was created or used with "
           "a pre-release or development version of AvocadoDB 3.1, please "
           "restart the server with option '--database.check-30-revisions "
           "false' to suppress this warning. If this collection was created "
           "with an AvocadoDB 3.0, please dump the 3.0 database with avocadodump "
           "and restore it in 3.1 with avocadorestore.";
...
void MMFilesCollection::prepareIndexes(VPackSlice indexesSlice) {
  ...
  for (auto const& v : VPackArrayIterator(indexesSlice)) {
    if (avocadodb::basics::VelocyPackHelper::getBooleanValue(v, "error",
                                                            false)) {
...
void MMFilesCollection::createInitialIndexes() {
  ...
  std::vector<std::shared_ptr<avocadodb::Index>> systemIndexes;
  StorageEngine* engine = EngineSelectorFeature::ENGINE;
  IndexFactory const* idxFactory = engine->indexFactory();
  TRI_ASSERT(idxFactory != nullptr);
...
std::shared_ptr<Index> MMFilesCollection::lookupIndex(
  ...
  avocadodb::Index::IndexType const type = avocadodb::Index::type(tmp.c_str());
...
int MMFilesCollection::saveIndex(transaction::Methods* trx,
  ...
  TRI_ASSERT(idx->type() != Index::IndexType::TRI_IDX_TYPE_PRIMARY_INDEX);
  std::vector<std::shared_ptr<avocadodb::Index>> indexListLocal;
...
bool MMFilesCollection::addIndex(std::shared_ptr<avocadodb::Index> idx) {
...
void MMFilesCollection::addIndexLocal(std::shared_ptr<avocadodb::Index> idx) {
  // primary index must be added at position 0
  TRI_ASSERT(idx->type() != avocadodb::Index::TRI_IDX_TYPE_PRIMARY_INDEX ||
             _indexes.empty());
...
Result MMFilesCollection::insert(transaction::Methods* trx,
  ...
  res = Result(TRI_ERROR_NO_ERROR);
  {
    ...
    bool const useDeadlockDetector =
      (lock && !trx->isSingleOperationTransaction() && !trx->state()->hasHint(transaction::Hints::Hint::NO_DLD));
    try {
      avocadodb::MMFilesCollectionWriteLocker collectionLocker(
...
Result MMFilesCollection::insertIndexes(avocadodb::transaction::Methods* trx,
                                        TRI_voc_rid_t revisionId,
                                        VPackSlice const& doc) {
...
Result MMFilesCollection::insertDocument(avocadodb::transaction::Methods* trx,
                                         TRI_voc_rid_t revisionId,
                                         VPackSlice const& doc,
...
Result MMFilesCollection::update(
    avocadodb::transaction::Methods* trx, VPackSlice const newSlice,
  ...
  if (options.recoveryData == nullptr) {
    ...
    if (trx->state()->isDBServer()) {
      ...
      if (avocadodb::shardKeysChanged(_logicalCollection->dbName(),
...
Result MMFilesCollection::remove(avocadodb::transaction::Methods* trx,
                                 VPackSlice const slice,
  ...
  VPackSlice oldDoc(vpack);
  TRI_voc_rid_t oldRevisionId =
      avocadodb::transaction::helpers::extractRevFromDocument(oldDoc);
  prevRev = oldRevisionId;
...
Result MMFilesCollection::removeFastPath(avocadodb::transaction::Methods* trx,
                                         TRI_voc_rid_t oldRevisionId,
                                         VPackSlice const oldDoc,
                                         OperationOptions& options,
                                         TRI_voc_rid_t const& revisionId,
  ...
  VPackSlice key =
      avocadodb::transaction::helpers::extractKeyFromDocument(oldDoc);
  TRI_ASSERT(!key.isNone());
