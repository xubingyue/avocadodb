...
MMFilesDropCollectionDitch::MMFilesDropCollectionDitch(
    MMFilesDitches* ditches, avocadodb::LogicalCollection* collection,
    std::function<bool(avocadodb::LogicalCollection*)> callback,
    char const* filename, int line)
    : MMFilesDitch(ditches, filename, line),
      _collection(collection),
      _callback(callback) {}
...
void MMFilesDitches::destroy() {
  ...
  while (ptr != nullptr) {
    ...
    if (type == MMFilesDitch::TRI_DITCH_COLLECTION_UNLOAD ||
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "logic error. shouldn't have document ditches on unload";
      TRI_ASSERT(false);
    } else {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unknown ditch type";
    }
