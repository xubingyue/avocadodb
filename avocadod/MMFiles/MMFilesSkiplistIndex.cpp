...
static int CompareKeyElement(void* userData, VPackSlice const* left,
  ...
  return avocadodb::basics::VelocyPackHelper::compare(
...
static int CompareElementElement(void* userData,
  ...
  return avocadodb::basics::VelocyPackHelper::compare(l, r, true);
}
...
MMFilesSkiplistLookupBuilder::MMFilesSkiplistLookupBuilder(
    ...
    std::vector<std::vector<avocadodb::aql::AstNode const*>>& ops,
    avocadodb::aql::Variable const* var, bool reverse)
  ...
  if (last[0]->type != avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ &&
      last[0]->type != avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN) {
    _isEquality = false;
    ...
    for (size_t i = 0; i < ops.size() - 1; ++i) {
      ...
      TRI_ASSERT(op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ ||
                 op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN);
    ...
    for (auto const& op : last) {
      ...
      switch (op->type) {
        case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LT:
          if (isReverseOrder) {
            _includeLower = false;
          } else {
            _includeUpper = false;
        ...
        case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LE:
          if (isReverseOrder) {
            value->toVelocyPackValue(*(_lowerBuilder.get()));
          } else {
            value->toVelocyPackValue(*(_upperBuilder.get()));
          ...
          break;
        case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GT:
          if (isReverseOrder) {
            _includeUpper = false;
          } else {
            _includeLower = false;
        ...
        // Fall through intentional
        case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE:
          if (isReverseOrder) {
            value->toVelocyPackValue(*(_upperBuilder.get()));
          } else {
            value->toVelocyPackValue(*(_lowerBuilder.get()));
...
MMFilesSkiplistInLookupBuilder::MMFilesSkiplistInLookupBuilder(
  ...
  std::set<VPackSlice, avocadodb::basics::VelocyPackHelper::VPackSorted<true>>
      unique_set(
          (avocadodb::basics::VelocyPackHelper::VPackSorted<true>(reverse)));
  std::pair<avocadodb::aql::Variable const*,
            std::vector<avocadodb::basics::AttributeName>>
      paramPair;
  ...
  for (size_t i = 0; i < ops.size() - 1; ++i) {
    ...
    if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN) {
      if (valueLeft) {
        // Case: value IN x.a
  ...
  auto const& last = ops.back();
  avocadodb::aql::AstNode const* lower = nullptr;
  avocadodb::aql::AstNode const* upper = nullptr;
  ...
  for (auto const& op : last) {
    ...
    switch (op->type) {
      ...
      case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE:
        ...
        break;
      case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN:
        TRI_ASSERT(upper == nullptr);
        TRI_ASSERT(lower == nullptr);
        TRI_ASSERT(value->numMembers() > 0);
        tmp->clear();
        ...
        buildSearchValues();
        return;
      case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ:
        TRI_ASSERT(upper == nullptr);
        TRI_ASSERT(lower == nullptr);
        value->toVelocyPackValue(*(_dataBuilder.get()));
        _isEquality = true;
  ...
  _dataBuilder->openArray();
  if (lower == nullptr) {
    _dataBuilder->add(avocadodb::basics::VelocyPackHelper::NullValue());
  } else {
...
MMFilesSkiplistIterator::MMFilesSkiplistIterator(
    ...
    ManagedDocumentResult* mmdr, avocadodb::MMFilesSkiplistIndex const* index,
...
bool MMFilesSkiplistIterator::intervalValid(void* userData, Node* left,
  ...
  if (_CmpElmElm(userData, left->document(), right->document(),
                 avocadodb::SKIPLIST_CMP_TOTORDER) > 0) {
    return false;
  }
  return true;
}
...
MMFilesSkiplistIndex::MMFilesSkiplistIndex(
    TRI_idx_iid_t iid, avocadodb::LogicalCollection* collection,
...
int MMFilesSkiplistIndex::ElementElementComparator::operator()(
  ...
  if (avocadodb::SKIPLIST_CMP_PREORDER == cmptype) {
    return 0;
  }
...
bool MMFilesSkiplistIndex::accessFitsIndex(
    avocadodb::aql::AstNode const* access, avocadodb::aql::AstNode const* other,
    avocadodb::aql::AstNode const* op, avocadodb::aql::Variable const* reference,
    std::unordered_map<size_t, std::vector<avocadodb::aql::AstNode const*>>&
        found,
  ...
  avocadodb::aql::AstNode const* what = access;
  std::pair<avocadodb::aql::Variable const*,
            std::vector<avocadodb::basics::AttributeName>>
      attributeData;
  ...
  if (op->type != avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN) {
    ...
    if (avocadodb::basics::TRI_AttributeNamesHaveExpansion(
    ...
    // doc.value[*]
    TRI_ASSERT(op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN);
    bool canUse = false;

    if (what->isAttributeAccessForVariable(attributeData) &&
        attributeData.first == reference &&
        !avocadodb::basics::TRI_AttributeNamesHaveExpansion(
            attributeData.second) &&
  ...
  std::vector<avocadodb::basics::AttributeName> const& fieldNames =
      attributeData.second;
  ...
  for (size_t i = 0; i < _fields.size(); ++i) {
    ...
    bool match = avocadodb::basics::AttributeName::isIdentical(_fields[i],
                                                              fieldNames, true);
    ...
    if (match) {
      ...
      if (it == found.end()) {
        found.emplace(i, std::vector<avocadodb::aql::AstNode const*>{op});
      } else {
...
bool MMFilesSkiplistIndex::accessFitsIndex(
    avocadodb::aql::AstNode const* access, avocadodb::aql::AstNode const* other,
    avocadodb::aql::AstNode const* op, avocadodb::aql::Variable const* reference,
    std::vector<std::vector<avocadodb::aql::AstNode const*>>& found,
...
bool MMFilesSkiplistIndex::findMatchingConditions(
    avocadodb::aql::AstNode const* node,
    avocadodb::aql::Variable const* reference,
    std::vector<std::vector<avocadodb::aql::AstNode const*>>& mapping,
    bool& usesIn) const {
  std::unordered_set<std::string> nonNullAttributes;
  usesIn = false;
...
IndexIterator* MMFilesSkiplistIndex::iteratorForCondition(
    ...
    avocadodb::aql::AstNode const* node,
    avocadodb::aql::Variable const* reference, bool reverse) {
  std::vector<std::vector<avocadodb::aql::AstNode const*>> mapping;
  bool usesIn = false;
  if (node != nullptr) {
...
bool MMFilesSkiplistIndex::supportsFilterCondition(
  ...
  std::unordered_map<size_t, std::vector<avocadodb::aql::AstNode const*>> found;
  std::unordered_set<std::string> nonNullAttributes;
  size_t values = 0;
  ...
  for (size_t i = 0; i < _fields.size(); ++i) {
    ...
    for (size_t j = 0; j < nodes.size(); ++j) {
      if (nodes[j]->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ ||
          nodes[j]->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN) {
...
avocadodb::aql::AstNode* MMFilesSkiplistIndex::specializeCondition(
  ...
  std::vector<avocadodb::aql::AstNode const*> children;
  bool lastContainsEquality = true;
  ...
  for (size_t i = 0; i < _fields.size(); ++i) {
    ...
    std::sort(nodes.begin(), nodes.end(),
              [this](avocadodb::aql::AstNode const* lhs,
                 avocadodb::aql::AstNode const* rhs) -> bool {
...
bool MMFilesSkiplistIndex::isDuplicateOperator(
  ...
  if (operatorsFound.find(
          static_cast<int>(avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ)) !=
          operatorsFound.end() ||
      operatorsFound.find(
          static_cast<int>(avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN)) !=
          operatorsFound.end()) {
    return true;
  }
  ...
  switch (type) {
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LT:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LE)) !=
                  operatorsFound.end();
      break;
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LE:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LT)) !=
                  operatorsFound.end();
      break;
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GT:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE)) !=
                  operatorsFound.end();
      break;
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GT)) !=
                  operatorsFound.end();
      break;
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN)) !=
                  operatorsFound.end();
      break;
    case avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN:
      duplicate = operatorsFound.find(static_cast<int>(
                      avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ)) !=
                  operatorsFound.end();
      break;
    default: {
      // ignore
