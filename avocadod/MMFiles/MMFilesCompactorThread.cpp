...
void MMFilesCompactorThread::DropDatafileCallback(MMFilesDatafile* df, LogicalCollection* collection) {
  ...
  std::string filename = avocadodb::basics::FileUtils::buildFilename(physical->path(), name);
...
void MMFilesCompactorThread::RenameDatafileCallback(MMFilesDatafile* datafile, 
  ...
  if (datafile->isPhysical()) {
    ...
    std::string tempFilename = avocadodb::basics::FileUtils::buildFilename(physical->path(), jname);
...
void MMFilesCompactorThread::compactDatafiles(LogicalCollection* collection,
  ...
  avocadodb::SingleCollectionTransaction trx(avocadodb::transaction::StandaloneContext::Create(collection->vocbase()), 
  ...
  if (context->_dfi.numberAlive == 0 && context->_dfi.numberDead == 0 &&
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      auto b = avocadodb::MMFilesCollection::toMMFilesCollection(collection)
                   ->ditches()
            ...
            if (collection->status() == TRI_VOC_COL_STATUS_LOADED && doCompact) {
              ...
              try {
                ...
                if (physical->lastCompactionStamp() + compactionCollectionInterval() <= now) {
                  auto ce = avocadodb::MMFilesCollection::toMMFilesCollection(
                                collection)
                                ->ditches()
