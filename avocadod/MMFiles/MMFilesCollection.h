...
#ifndef AVOCADOD_MMFILES_MMFILES_COLLECTION_H
#define AVOCADOD_MMFILES_MMFILES_COLLECTION_H 1
...
class MMFilesCollection final : public PhysicalCollection {
 ...
 public:
  ...
  int iterateMarkersOnLoad(avocadodb::transaction::Methods* trx);
 ...
 private:
  ...
  Result removeFastPath(avocadodb::transaction::Methods* trx,
                        TRI_voc_rid_t oldRevisionId,
                        avocadodb::velocypack::Slice const oldDoc,
                        OperationOptions& options,
                        TRI_voc_rid_t const& revisionId,
                        avocadodb::velocypack::Slice const toRemove);
  ...
  Result insertDocument(avocadodb::transaction::Methods* trx,
                        TRI_voc_rid_t revisionId,
                        avocadodb::velocypack::Slice const& doc,
 ...
 private:
  ...
  bool addIndex(std::shared_ptr<avocadodb::Index> idx);
  void addIndexLocal(std::shared_ptr<avocadodb::Index> idx);
 ...
 private:
  mutable avocadodb::MMFilesDitches _ditches;

  // lock protecting the indexes and data
  mutable basics::ReadWriteLock _dataLock;

  avocadodb::basics::ReadWriteLock _filesLock;
  ...
  avocadodb::basics::ReadWriteLock _compactionLock;
