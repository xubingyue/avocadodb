...
using namespace avocadodb::methods;

Result Indexes::getIndex(avocadodb::LogicalCollection const* collection,
...
avocadodb::Result Indexes::getAll(avocadodb::LogicalCollection const* collection,
  ...
  if (ServerState::instance()->isCoordinator()) {
    ...
    tmp.openArray(true);
    for (std::shared_ptr<avocadodb::Index> const& idx : indexes) {
...
static Result EnsureIndexLocal(avocadodb::LogicalCollection* collection,
...
Result Indexes::ensureIndexCoordinator(
    avocadodb::LogicalCollection const* collection, VPackSlice const& indexDef,
  ...
  int res = ClusterInfo::instance()->ensureIndexCoordinator(
      dbName, cid, indexDef, create, &avocadodb::Index::Compare, resultBuilder,
      errorMsg, 360.0);
  return Result(res, errorMsg);
}

Result Indexes::ensureIndex(avocadodb::LogicalCollection* collection,
                            VPackSlice const& definition, bool create,
                            VPackBuilder& output) {
...
Result Indexes::extractHandle(avocadodb::LogicalCollection const* collection,
...
avocadodb::Result Indexes::drop(avocadodb::LogicalCollection const* collection,
