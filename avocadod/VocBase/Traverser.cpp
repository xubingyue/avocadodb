...
bool Traverser::VertexGetter::getSingleVertex(avocadodb::velocypack::Slice edge, StringRef cmp,
...
bool Traverser::UniqueVertexGetter::getSingleVertex(avocadodb::velocypack::Slice edge, StringRef cmp,
                                              uint64_t depth, StringRef& result) {
  VPackSlice resSlice = edge;
  if (!resSlice.isString()) {
...
void Traverser::UniqueVertexGetter::reset(avocadodb::StringRef const& startVertex) {
  _returnedVertices.clear();
  // The startVertex always counts as visited!
  _returnedVertices.emplace(startVertex);
}

Traverser::Traverser(avocadodb::traverser::TraverserOptions* opts,
                     transaction::Methods* trx,
                     avocadodb::ManagedDocumentResult* mmdr)
    : _trx(trx),
      _mmdr(mmdr),
      _startIdBuilder(trx),
      _pruneNext(false),
...
bool avocadodb::traverser::Traverser::edgeMatchesConditions(VPackSlice e,
                                                           StringRef vid,
                                                           uint64_t depth,
                                                           size_t cursorId) {
...
bool avocadodb::traverser::Traverser::vertexMatchesConditions(VPackSlice v, uint64_t depth) {
...
bool avocadodb::traverser::Traverser::next() {
  TRI_ASSERT(!_done);
...
TraverserCache* avocadodb::traverser::Traverser::traverserCache() {
  return _opts->cache();
}

avocadodb::aql::AqlValue avocadodb::traverser::Traverser::lastVertexToAqlValue() {
  return _enumerator->lastVertexToAqlValue();
}

avocadodb::aql::AqlValue avocadodb::traverser::Traverser::lastEdgeToAqlValue() {
  return _enumerator->lastEdgeToAqlValue();
}

avocadodb::aql::AqlValue avocadodb::traverser::Traverser::pathToAqlValue(
    VPackBuilder& builder) {
  return _enumerator->pathToAqlValue(builder);
}

size_t avocadodb::traverser::Traverser::getAndResetReadDocuments() {
  return traverserCache()->getAndResetInsertedDocuments();
}

size_t avocadodb::traverser::Traverser::getAndResetFilteredPaths() {
  return traverserCache()->getAndResetFilteredDocuments();
}

void avocadodb::traverser::Traverser::allowOptimizedNeighbors() {
