...
void TRI_vocbase_t::registerCollection(
    bool doLock, avocadodb::LogicalCollection* collection) {
...
void TRI_vocbase_t::registerView(bool doLock,
                                 std::shared_ptr<avocadodb::LogicalView> view) {
...
avocadodb::LogicalCollection* TRI_vocbase_t::createCollectionWorker(
    VPackSlice parameters) {
  std::string name = avocadodb::basics::VelocyPackHelper::getStringValue(
      parameters, "name", "");
  TRI_ASSERT(!name.empty());

  // Try to create a new collection. This is not registered yet

  std::unique_ptr<avocadodb::LogicalCollection> collection =
      std::make_unique<avocadodb::LogicalCollection>(this, parameters);
  TRI_ASSERT(collection != nullptr);
...
int TRI_vocbase_t::loadCollection(avocadodb::LogicalCollection* collection,
...
int TRI_vocbase_t::dropCollectionWorker(avocadodb::LogicalCollection* collection,
  ...
  avocadodb::aql::PlanCache::instance()->invalidate(this);
  avocadodb::aql::QueryCache::instance()->invalidate(this);
  ...
  switch (collection->status()) {
    ...
    case TRI_VOC_COL_STATUS_UNLOADING: {
      ...
      avocadodb::Result res = collection->updateProperties(
...
void TRI_vocbase_t::shutdown() {
  ...
  std::vector<avocadodb::LogicalCollection*> collections;
...
std::shared_ptr<VPackBuilder> TRI_vocbase_t::inventory(
    TRI_voc_tick_t maxTick, bool (*filter)(avocadodb::LogicalCollection*, void*),
    void* data, bool shouldSort,
    std::function<bool(avocadodb::LogicalCollection*,
...
avocadodb::LogicalCollection* TRI_vocbase_t::createCollection(
    VPackSlice parameters) {
  ...
  avocadodb::LogicalCollection* collection = createCollectionWorker(parameters);
  ...
  avocadodb::Result res2 = engine->persistCollection(this, collection);
...
int TRI_vocbase_t::unloadCollection(avocadodb::LogicalCollection* collection,
                                    bool force) {
  {
    WRITE_LOCKER_EVENTUAL(locker, collection->_lock);
...
/// @brief drops a collection
int TRI_vocbase_t::dropCollection(avocadodb::LogicalCollection* collection,
...
int TRI_vocbase_t::renameCollection(avocadodb::LogicalCollection* collection,
...
void TRI_vocbase_t::releaseCollection(avocadodb::LogicalCollection* collection) {
  collection->_lock.unlock();
}

/// @brief creates a new view, worker function
std::shared_ptr<avocadodb::LogicalView> TRI_vocbase_t::createViewWorker(
  ...
  std::shared_ptr<avocadodb::LogicalView> view =
      std::make_shared<avocadodb::LogicalView>(this, parameters);
  TRI_ASSERT(view != nullptr);
...
/// using a cid of > 0 is supported to import dumps from other servers etc.
/// but the functionality is not advertised
std::shared_ptr<avocadodb::LogicalView> TRI_vocbase_t::createView(
    VPackSlice parameters, TRI_voc_cid_t id) {
  READ_LOCKER(readLocker, _inventoryLock);
  ...
  TRI_ASSERT(engine != nullptr);
  // TODO Review
  avocadodb::Result res2 = engine->persistView(this, view.get());
...
/// @brief drops a view
int TRI_vocbase_t::dropView(std::shared_ptr<avocadodb::LogicalView> view) {
  TRI_ASSERT(view != nullptr);
...
TRI_vocbase_t::TRI_vocbase_t(TRI_vocbase_type_e type, TRI_voc_tick_t id,
  ...
  _queries.reset(new avocadodb::aql::QueryList(this));
  _cursorRepository.reset(new avocadodb::CursorRepository(this));
  _collectionKeys.reset(new avocadodb::CollectionKeysRepository());
...
std::vector<std::shared_ptr<avocadodb::LogicalView>> TRI_vocbase_t::views() {
...
std::vector<avocadodb::LogicalCollection*> TRI_vocbase_t::collections(
    bool includeDeleted) {
  std::vector<avocadodb::LogicalCollection*> collections;
...
std::string TRI_RidToString(TRI_voc_rid_t rid) {
  if (rid <= tickLimit) {
    return avocadodb::basics::StringUtils::itoa(rid);
  }
...
TRI_voc_rid_t TRI_StringToRid(char const* p, size_t len, bool& isOld,
                              bool warn) {
  if (len > 0 && *p >= '1' && *p <= '9') {
    TRI_voc_rid_t r = avocadodb::basics::StringUtils::uint64_check(p, len);
    if (warn && r > tickLimit) {
