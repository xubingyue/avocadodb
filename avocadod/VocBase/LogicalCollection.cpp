...
static TRI_voc_cid_t ReadPlanId(VPackSlice info, TRI_voc_cid_t cid) {
  ...
  if (id.isString()) {
    ...
    return avocadodb::basics::StringUtils::uint64(id.copyString());
  } else if (id.isNumber()) {
...
avocadodb::Result LogicalCollection::updateProperties(VPackSlice const& slice,
                                                     bool doSync) {
...
std::shared_ptr<avocadodb::velocypack::Builder> LogicalCollection::figures() {
  if (ServerState::instance()->isCoordinator()) {
...
bool LogicalCollection::dropIndex(TRI_idx_iid_t iid) {
  TRI_ASSERT(!ServerState::instance()->isCoordinator());
  avocadodb::aql::PlanCache::instance()->invalidate(_vocbase);
  avocadodb::aql::QueryCache::instance()->invalidate(_vocbase, name());
...
#ifndef USE_ENTERPRISE
bool LogicalCollection::skipForAqlWrite(avocadodb::velocypack::Slice document,
...
VPackSlice LogicalCollection::keyOptions() const {
  if (_keyOptions == nullptr) {
    return avocadodb::basics::VelocyPackHelper::NullValue();
  }
