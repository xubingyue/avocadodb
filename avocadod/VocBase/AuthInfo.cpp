...
bool AuthInfo::parseUsers(VPackSlice const& slice) {
  ...
  for (VPackSlice const& authSlice : VPackArrayIterator(slice)) {
    ...
    if (s.hasKey("source") && s.get("source").isString() &&
      ...
      LOG_TOPIC(TRACE, avocadodb::Logger::CONFIG)
...
static std::shared_ptr<VPackBuilder> QueryAllUsers(
  ...
  avocadodb::aql::Query query(false, vocbase,
                             avocadodb::aql::QueryString(queryStr), emptyBuilder,
                             emptyBuilder, avocadodb::aql::PART_MAIN);
...
static VPackBuilder QueryUser(aql::QueryRegistry* queryRegistry,
  ...
  binds.close();  // obj
  avocadodb::aql::Query query(false, vocbase,
                             avocadodb::aql::QueryString(queryStr),
...
AuthResult AuthInfo::checkAuthenticationBasic(std::string const& secret) {
  ...
  if (n == std::string::npos || n == 0 || n + 1 > up.size()) {
    LOG_TOPIC(TRACE, avocadodb::Logger::AUTHENTICATION)
...
AuthResult AuthInfo::checkAuthenticationJWT(std::string const& jwt) {
  ...
  if (parts.size() != 3) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "Secret contains "
                                              << parts.size() << " parts";
    return AuthResult();
  }
...
std::shared_ptr<VPackBuilder> AuthInfo::parseJson(std::string const& str,
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Out of memory parsing " << hint
                                            << "!";
  } catch (VPackException const& ex) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "Couldn't parse " << hint
                                              << ": " << ex.what();
  } catch (...) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
...
AuthJwtResult AuthInfo::validateJwtBody(std::string const& body) {
  ...
  if (issSlice.copyString() != "avocadodb") {
    return authResult;
  }
...
std::string AuthInfo::generateJwt(VPackBuilder const& payload) {
  ...
  if (hasIss && hasIat) {
    ...
    if (!hasIss) {
      bodyBuilder.add("iss", VPackValue("avocadodb"));
    }
    if (!hasIat) {
