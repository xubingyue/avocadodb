...
static std::string GetStateFilename(TRI_vocbase_t* vocbase) {
  StorageEngine* engine = EngineSelectorFeature::ENGINE;
  return avocadodb::basics::FileUtils::buildFilename(engine->databasePath(vocbase), "REPLICATION-APPLIER-STATE");
}
...
static void ApplyThread(void* data) {
  auto syncer = static_cast<avocadodb::ContinuousSyncer*>(data);
...
int TRI_replication_applier_t::start(TRI_voc_tick_t initialTick, bool useTick,
  ...
  auto syncer = std::make_unique<avocadodb::ContinuousSyncer>(_vocbase, &_configuration, initialTick, useTick, barrierId);
