...
/// note: the NEW_BORN status is not used in AvocadoDB 1.3 anymore, but is left
...
struct TRI_vocbase_t {
  friend class avocadodb::CollectionNameResolver;
  friend class avocadodb::StorageEngine;
 ...
 private:
  ...
  avocadodb::basics::ReadWriteLock _collectionsLock;  // collection iterator lock
  std::vector<avocadodb::LogicalCollection*>
  ...
  std::unordered_map<std::string, avocadodb::LogicalCollection*>
      _collectionsByName;  // collections by name
  std::unordered_map<TRI_voc_cid_t, avocadodb::LogicalCollection*>
      _collectionsById;  // collections by id

  avocadodb::basics::ReadWriteLock _viewsLock;  // views management lock
  std::unordered_map<std::string, std::shared_ptr<avocadodb::LogicalView>>
      _viewsByName;  // views by name
  std::unordered_map<TRI_voc_cid_t, std::shared_ptr<avocadodb::LogicalView>>
      _viewsById;  // views by id

  std::unique_ptr<avocadodb::aql::QueryList> _queries;
  std::unique_ptr<avocadodb::CursorRepository> _cursorRepository;
  std::unique_ptr<avocadodb::CollectionKeysRepository> _collectionKeys;

  std::unique_ptr<TRI_replication_applier_t> _replicationApplier;

  avocadodb::basics::ReadWriteLock _replicationClientsLock;
  std::unordered_map<TRI_server_id_t, std::pair<double, TRI_voc_tick_t>>
      _replicationClients;

 public:
  avocadodb::basics::DeadlockDetector<avocadodb::LogicalCollection>
      _deadlockDetector;
  avocadodb::basics::ReadWriteLock _inventoryLock;  // object lock needed when
 ...
 public:
  ...
  avocadodb::aql::QueryList* queryList() const { return _queries.get(); }
  avocadodb::CursorRepository* cursorRepository() const {
    return _cursorRepository.get();
  }
  avocadodb::CollectionKeysRepository* collectionKeys() const {
  ...
  std::vector<std::shared_ptr<avocadodb::LogicalView>> views();

  /// @brief returns all known collections
  std::vector<avocadodb::LogicalCollection*> collections(bool includeDeleted);
  
  void processCollections(std::function<void(avocadodb::LogicalCollection*)> const& cb, bool includeDeleted);
  ...
  avocadodb::LogicalCollection* lookupCollection(std::string const& name);
  /// @brief looks up a collection by identifier
  avocadodb::LogicalCollection* lookupCollection(TRI_voc_cid_t id);

  /// @brief looks up a view by name
  std::shared_ptr<avocadodb::LogicalView> lookupView(std::string const& name);
  /// @brief looks up a view by identifier
  std::shared_ptr<avocadodb::LogicalView> lookupView(TRI_voc_cid_t id);

  /// @brief returns all known collections with their parameters
  /// and optionally indexes
  /// the result is sorted by type and name (vertices before edges)
  std::shared_ptr<avocadodb::velocypack::Builder> inventory(
      TRI_voc_tick_t, bool (*)(avocadodb::LogicalCollection*, void*), void*,
      bool, std::function<bool(avocadodb::LogicalCollection*,
                               avocadodb::LogicalCollection*)>);

  /// @brief renames a collection
  int renameCollection(avocadodb::LogicalCollection* collection,
  ...
  avocadodb::LogicalCollection* createCollection(
      avocadodb::velocypack::Slice parameters);
  ...
  int dropCollection(avocadodb::LogicalCollection* collection,
                     bool allowDropSystem, double timeout);

  /// @brief callback for collection dropping
  static bool DropCollectionCallback(avocadodb::LogicalCollection* collection);

  /// @brief unloads a collection
  int unloadCollection(avocadodb::LogicalCollection* collection, bool force);
  ...
  /// using a cid of > 0 is supported to import dumps from other servers etc.
  /// but the functionality is not advertised
  std::shared_ptr<avocadodb::LogicalView> createView(
      avocadodb::velocypack::Slice parameters, TRI_voc_cid_t id);

  /// @brief drops a view
  int dropView(std::string const& name);
  int dropView(std::shared_ptr<avocadodb::LogicalView> view);

  /// @brief locks a collection for usage, loading or manifesting it
  /// Note that this will READ lock the collection you have to release the
  /// collection lock by yourself.
  int useCollection(avocadodb::LogicalCollection* collection,
  ...
  avocadodb::LogicalCollection* useCollection(TRI_voc_cid_t cid,
                                             TRI_vocbase_col_status_e&);
  ...
  /// collection lock by yourself and call @ref TRI_ReleaseCollectionVocBase
  /// when you are done with the collection.
  avocadodb::LogicalCollection* useCollection(std::string const& name,
                                             TRI_vocbase_col_status_e&);

  /// @brief releases a collection from usage
  void releaseCollection(avocadodb::LogicalCollection* collection);

 private:
  /// @brief looks up a collection by name, without acquiring a lock
  avocadodb::LogicalCollection* lookupCollectionNoLock(std::string const& name);

  int loadCollection(avocadodb::LogicalCollection* collection,
                     TRI_vocbase_col_status_e& status, bool setStatus = true);

  /// @brief adds a new collection
  /// caller must hold _collectionsLock in write mode or set doLock
  void registerCollection(bool doLock, avocadodb::LogicalCollection* collection);

  /// @brief removes a collection from the global list of collections
  /// This function is called when a collection is dropped.
  bool unregisterCollection(avocadodb::LogicalCollection* collection);

  /// @brief creates a new collection, worker function
  avocadodb::LogicalCollection* createCollectionWorker(
      avocadodb::velocypack::Slice parameters);

  /// @brief drops a collection, worker function
  int dropCollectionWorker(avocadodb::LogicalCollection* collection,
                           DropState& state, double timeout);

  /// @brief creates a new view, worker function
  std::shared_ptr<avocadodb::LogicalView> createViewWorker(
      avocadodb::velocypack::Slice parameters, TRI_voc_cid_t& id);

  /// @brief adds a new view
  /// caller must hold _viewsLock in write mode or set doLock
  void registerView(bool doLock, std::shared_ptr<avocadodb::LogicalView> view);

  /// @brief removes a view from the global list of views
  /// This function is called when a view is dropped.
  bool unregisterView(std::shared_ptr<avocadodb::LogicalView> view);
};
