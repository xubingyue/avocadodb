...
static int HexHashFromData(std::string const& hashMethod,
  ...
  try {
    if (hashMethod == "sha1") {
      avocadodb::rest::SslInterface::sslSHA1(str.data(), str.size(), crypted,
                                            cryptedLength);
    } else if (hashMethod == "sha512") {
      avocadodb::rest::SslInterface::sslSHA512(str.data(), str.size(), crypted,
                                              cryptedLength);
    } else if (hashMethod == "sha384") {
      avocadodb::rest::SslInterface::sslSHA384(str.data(), str.size(), crypted,
                                              cryptedLength);
    } else if (hashMethod == "sha256") {
      avocadodb::rest::SslInterface::sslSHA256(str.data(), str.size(), crypted,
                                              cryptedLength);
    } else if (hashMethod == "sha224") {
      avocadodb::rest::SslInterface::sslSHA224(str.data(), str.size(), crypted,
                                              cryptedLength);
    } else if (hashMethod == "md5") {  // WFT?!!!
      avocadodb::rest::SslInterface::sslMD5(str.data(), str.size(), crypted,
                                           cryptedLength);
    } else {
      // invalid algorithm...
...
AuthUserEntry AuthUserEntry::fromDocument(VPackSlice const& slice) {
  ...
  if (!simpleSlice.isObject()) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "cannot extract simple";
  ...
  if (!activeSlice.isBoolean()) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "cannot extract active flag";
    return AuthUserEntry();
  }
  ...
  if (databasesSlice.isObject()) {
    for (auto const& obj : VPackObjectIterator(databasesSlice)) {
      ...
      if (obj.value.isObject()) {
        ...
        LOG_TOPIC(DEBUG, avocadodb::Logger::CONFIG)
