...
TRI_voc_cid_t CollectionNameResolver::getCollectionIdLocal(
  ...
  if (name[0] >= '0' && name[0] <= '9') {
    ...
    return static_cast<TRI_voc_cid_t>(
        avocadodb::basics::StringUtils::uint64(name));
  }

  avocadodb::LogicalCollection const* collection = getCollectionStruct(name);
...
TRI_voc_cid_t CollectionNameResolver::getCollectionIdCluster(
  ...
  if (name[0] >= '0' && name[0] <= '9') {
    // name is a numeric id
    TRI_voc_cid_t cid = static_cast<TRI_voc_cid_t>(avocadodb::basics::StringUtils::uint64(name));
...
avocadodb::LogicalCollection const* CollectionNameResolver::getCollectionStruct(
    std::string const& name) const {
  ...
  avocadodb::LogicalCollection const* collection = _vocbase->lookupCollection(name);
...
std::string CollectionNameResolver::getCollectionNameCluster(
  ...
  while (tries++ < 2) {
    auto ci = ClusterInfo::instance()->getCollection(
        _vocbase->name(), avocadodb::basics::StringUtils::itoa(cid));
  ...
  LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "CollectionNameResolver: was not able to resolve id " << cid;
  return "_unknown";
}
...
std::string CollectionNameResolver::getCollectionName(
  ...
  TRI_voc_cid_t tmp = avocadodb::basics::StringUtils::uint64(nameOrId);
  ...
  if (ServerState::isDBServer(_serverRole)) {
    ...
    if (it != _vocbase->_collectionsById.end()) {
      if ((*it).second->planId() == (*it).second->cid()) {
        ...
        name = avocadodb::basics::StringUtils::itoa((*it).second->planId());
