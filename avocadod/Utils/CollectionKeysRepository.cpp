...
CollectionKeysRepository::~CollectionKeysRepository() {
  ...
  while (true) {
    ...
    if (tries == 0) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "waiting for used keys to become unused";
    } else if (tries == 120) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "giving up waiting for unused keys";
    }
...
void CollectionKeysRepository::store(avocadodb::CollectionKeys* keys) {
  MUTEX_LOCKER(mutexLocker, _lock);
...
bool CollectionKeysRepository::remove(CollectionKeysId id) {
  avocadodb::CollectionKeys* collectionKeys = nullptr;
...
bool CollectionKeysRepository::garbageCollect(bool force) {
  std::vector<avocadodb::CollectionKeys*> found;
