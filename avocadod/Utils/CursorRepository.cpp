...
CursorRepository::~CursorRepository() {
  ...
  while (true) {
    ...
    if (tries == 0) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "waiting for used cursors to become unused";
    } else if (tries == 120) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "giving up waiting for unused cursors";
    }
...
bool CursorRepository::remove(CursorId id, Cursor::CursorType type) {
  avocadodb::Cursor* cursor = nullptr;
...
bool CursorRepository::garbageCollect(bool force) {
  ...
  std::vector<avocadodb::Cursor*> found;
