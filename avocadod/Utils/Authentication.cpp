...
static AuthLevel _convertToAuthLevel(avocadodb::StringRef ref) {
  if (ref.compare("rw") == 0) {
    return AuthLevel::RW;
...
AuthLevel avocadodb::convertToAuthLevel(velocypack::Slice grants) {
  return _convertToAuthLevel(StringRef(grants));
}

AuthLevel avocadodb::convertToAuthLevel(std::string const& grants) {
  return _convertToAuthLevel(StringRef(grants));
}

std::string avocadodb::convertFromAuthLevel(AuthLevel lvl) {
  if (lvl == AuthLevel::RW) {
    return "rw";
