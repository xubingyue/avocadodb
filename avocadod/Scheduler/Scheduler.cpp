...
Scheduler::~Scheduler() {
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to delete old scheduler threads";
  }
}
...
bool Scheduler::start(ConditionVariable* cv) {
  ...
  // done
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "all scheduler threads are up and running";
  return true;
}
...
// Windows does not support POSIX signal handling
  ...
  if (res < 0) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot initialize signal handlers for pipe";
  }
#endif
}
