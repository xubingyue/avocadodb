...
bool ListenTask::start() {
  ...
  try {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::COMMUNICATION) << "failed to open endpoint '" << _endpoint->specification()
              << "' with error: " << err.what();
    return false;
  ...
  _handler = [this](boost::system::error_code const& ec) {
    ...
    if (ec) {
      ...
      if (_acceptFailures < MAX_ACCEPT_ERRORS) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "accept failed: " << ec.message();
      } else if (_acceptFailures == MAX_ACCEPT_ERRORS) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "accept failed: " << ec.message();
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "too many accept failures, stopping to report";
      }
    }
