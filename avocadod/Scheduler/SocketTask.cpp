...
SocketTask::SocketTask(avocadodb::EventLoop loop,
                       std::unique_ptr<avocadodb::Socket> socket,
                       avocadodb::ConnectionInfo&& connectionInfo,
...
bool SocketTask::reserveMemory() {
  ...
  if (_readBuffer.reserve(READ_BLOCK_SIZE + 1) == TRI_ERROR_OUT_OF_MEMORY) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "out of memory while reading from client";
    closeStreamNoLock();
    return false;
  }
