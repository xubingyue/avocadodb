...
void SchedulerFeature::validateOptions(
  ...
  if (_nrServerThreads == 0) {
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "Detected number of processors: " << _nrServerThreads;
  }
...
void SchedulerFeature::start() {
  AvocadoGlobalContext::CONTEXT->maskAllSignals();
  buildScheduler();

  bool ok = _scheduler->start(nullptr);

  if (!ok) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "the scheduler cannot be started";
    FATAL_ERROR_EXIT();
  }
...
bool CtrlHandler(DWORD eventType) {
  ...
  if (shutdown == false) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Invalid CTRL HANDLER event received - ignoring event";
    return true;
  }
  ...
  if (!seen) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << shutdownMessage << ", beginning shut down sequence";
  ...
  LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << shutdownMessage << ", terminating";
...
void SchedulerFeature::buildControlCHandler() {
#ifdef WIN32
  {
    ...
    if (result == 0) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "unable to install control-c handler";
    }
  }
#else
  ...
  _signalHandler = [this](const boost::system::error_code& error, int number) {
    ...
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "control-c received, beginning shut down sequence";
  ...
  _exitHandler = [](const boost::system::error_code& error, int number) {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "control-c received (again!), terminating";
    FATAL_ERROR_EXIT();
  };
...
void SchedulerFeature::buildHangupHandler() {
  ...
  _hangupHandler = [this](const boost::system::error_code& error, int number) {
    ...
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "hangup received, about to reopen logfile";
    LogAppender::reopen();
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "hangup received, reopened logfile";
