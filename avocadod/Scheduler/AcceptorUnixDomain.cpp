...
void AcceptorUnixDomain::open() {
  ...
  if (basics::FileUtils::exists(path)) {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "socket file '" << path << "' already exists.";

    int error = 0;
    // delete previously existing socket file
    if (basics::FileUtils::remove(path, &error)) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "deleted previously existing socket file '" << path << "'";
    } else {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to delete previously existing socket file '" << path
               << "'";
    }
  }
...
void AcceptorUnixDomain::close() {
  ...
  std::string path = ((EndpointUnixDomain*) _endpoint)->path();
  if (!basics::FileUtils::remove(path, &error)) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "unable to remove socket file '" << path << "'";
  }
}
