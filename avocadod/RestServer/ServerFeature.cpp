...
void ServerFeature::validateOptions(std::shared_ptr<ProgramOptions>) {
  ...
  if (1 < count) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot combine '--console', '--javascript.unit-tests' and "
               << "'--javascript.script'";
    FATAL_ERROR_EXIT();
  }

  if (_operationMode == OperationMode::MODE_SERVER && !_restServer) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "need at least '--console', '--javascript.unit-tests' or"
...
void ServerFeature::beginShutdown() {
  std::string msg =
      AvocadoGlobalContext::CONTEXT->binaryName() + " [shutting down]";
