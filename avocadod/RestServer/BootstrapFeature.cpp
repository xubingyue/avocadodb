...
void BootstrapFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  options->addHiddenOption("hund", "make AvocadoDB bark on startup",
...
static void raceForClusterBootstrap() {
  ...
  while (true) {
    ...
    VPackBuilder b;
    b.add(VPackValue(avocadodb::ServerState::instance()->getId()));
    ...
    b.clear();
    b.add(VPackValue(avocadodb::ServerState::instance()->getId() + ": done"));
...
void BootstrapFeature::start() {
  ...
  if (_bark) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "The dog says: wau wau!";
  }
