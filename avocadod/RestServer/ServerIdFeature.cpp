...
int ServerIdFeature::readId() {
  ...
  try {
    std::shared_ptr<VPackBuilder> builder =
        avocadodb::basics::VelocyPackHelper::velocyPackFromFile(_idFilename);
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "using existing server id: " << foundId;
...
int ServerIdFeature::writeId() {
  ...
  try {
    ...
    // out of memory
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot save server id in file '" << _idFilename
             << "': out of memory";
    return TRI_ERROR_OUT_OF_MEMORY;
  }

  // save json info to file
  LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "Writing server id to file '" << _idFilename << "'";
  bool ok = avocadodb::basics::VelocyPackHelper::velocyPackToFile(
      _idFilename, builder.slice(), true);

  if (!ok) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not save server id in file '" << _idFilename << "': " << TRI_last_error();
