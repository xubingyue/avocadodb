...
void EndpointFeature::validateOptions(std::shared_ptr<ProgramOptions>) {
  if (_backlogSize > SOMAXCONN) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "value for --tcp.backlog-size exceeds default system "
...
void EndpointFeature::prepare() {
  ...
  if (_endpointList.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "no endpoints have been specified, giving up, please use the "
...
void EndpointFeature::buildEndpointLists() {
  for (std::vector<std::string>::const_iterator i = _endpoints.begin();
    ...
    if (!ok) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid endpoint '" << (*i) << "'";
      FATAL_ERROR_EXIT();
    }
  }
}
