...
void DatabasePathFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  if (1 == positionals.size()) {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "expected at most one database directory, got '"
               << StringUtils::join(positionals, ",") << "'";
    FATAL_ERROR_EXIT();
  }

  if (_directory.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "no database path has been supplied, giving up, please use "
...
void DatabasePathFeature::start() {
  ...
  if (!basics::FileUtils::isDirectory(_directory)) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "created database directory '" << _directory << "'";
    } else {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "unable to create database directory '" << _directory << "': " << systemErrorStr;
      FATAL_ERROR_EXIT();
    }
  }
}
