...
#include "Enterprise/RestServer/avocadodEE.h"
#endif
...
static int runServer(int argc, char** argv, AvocadoGlobalContext &context) {
  try {
    context.installSegv();
    ...
    try {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
          << "avocadod terminated because of an exception: "
          << ex.what();
      ret = EXIT_FAILURE;
    } catch (...) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
          << "avocadod terminated because of an exception of "
             "unknown type";
      ret = EXIT_FAILURE;
    }
    Logger::flush();
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
        << "avocadod terminated because of an xception of "
           "unknown type";
  }
  exit(EXIT_FAILURE);
}
...
static void WINAPI ServiceMain(DWORD dwArgc, LPSTR* lpszArgv) {
  ...
  AvocadoGlobalContext context(ARGC, ARGV, SBIN_DIRECTORY);
