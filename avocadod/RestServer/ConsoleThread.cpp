...
void ConsoleThread::run() {
  ...
  if (_context == nullptr) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot acquire V8 context";
    FATAL_ERROR_EXIT();
  }
  ...
  try {
    ...
    if (strcmp(error, USER_ABORTED) != 0) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << error;
    }
  } catch (...) {
...
void ConsoleThread::inner() {
  ...
  // run the shell
  std::cout << "avocadod console (" << rest::Version::getVerboseVersionString()
            << ")" << std::endl;
  std::cout << "Copyright (c) AvocadoDB GmbH" << std::endl;
  ...
  localContext->Enter();
  {
    ...
    // read and eval .avocadod.rc from home directory if it exists
    char const* startupScript = R"SCRIPT(
start_pretty_print();

(function () {
  var __fs__ = require("fs");
  var __rcf__ = __fs__.join(__fs__.home(), ".avocadod.rc");
  if (__fs__.exists(__rcf__)) {
    ...
    if (pthread_sigmask(SIG_UNBLOCK, &set, nullptr) < 0) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to install signal handler";
    }
#endif

    V8LineEditor console(isolate, localContext, ".avocadod.history");
    ...
    while (!isStopping() && !_userAborted.load()) {
      ...
      {
        MUTEX_LOCKER(mutexLocker, serverConsoleMutex);
        input = console.prompt("avocadod> ", "avocadod", eof);
      }
