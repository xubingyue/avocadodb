...
void UpgradeFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  if (_upgrade && !_upgradeCheck) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot specify both '--database.auto-upgrade true' and "
                  "'--database.upgrade-check false'";
    FATAL_ERROR_EXIT();
  }

  if (!_upgrade) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "executing upgrade check: not disabling server features";
    return;
  }

  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "executing upgrade procedure: disabling server features";
...
void UpgradeFeature::start() {
  ...
  if (init->restoreAdmin()) {
    ...
    if (res.fail()) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "failed to create clear users: "
                                               << res.errorMessage();
    ...
    if (res.fail()) {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "failed to create root user: "
                                               << res.errorMessage();
      *_result = EXIT_FAILURE;
      return;
    }
...
void UpgradeFeature::upgradeDatabase() {
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "starting database init/upgrade";
  ...
  // enter context and isolate
  {
    ...
    if (context == nullptr) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not enter context #0";
      FATAL_ERROR_EXIT();
    }
...

    {
...

      {
        ...
        // run upgrade script
        LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "running database init/upgrade";
  ...
  // 5 = no version found
              ...
              LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Database '" << vocbase->name()
                         << "' " << typeName << " failed. Please inspect the logs from "
                            "the " << typeName << " procedure";
            } else {
              LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "JavaScript error during server start";
            }
              
            FATAL_ERROR_EXIT();
          }
            
          LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "database '" << vocbase->name()
                     << "' init/upgrade done";
        }
      }
  ...
  if (_upgrade) {
    *_result = EXIT_SUCCESS;
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "database upgrade passed";
  }

  // and return from the context
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "finished database init/upgrade";
}
