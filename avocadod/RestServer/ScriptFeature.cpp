...

  {
    ...
    localContext->Enter();
    {
      ...
      for (auto script : scripts) {
        LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "executing script '" << script << "'";
        bool r = TRI_ExecuteGlobalJavaScriptFile(isolate, script.c_str(), true);

        if (!r) {
          LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot load script '" << script << "', giving up";
          FATAL_ERROR_EXIT();
        }
      }
      ...
      if (main.IsEmpty() || main->IsUndefined()) {
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "no main function defined, giving up";
        FATAL_ERROR_EXIT();
      } else {
        ...
        try {
          ...
          LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception " << TRI_errno_string(ex.code()) << ": "
                   << ex.what();
          ok = false;
        } catch (std::bad_alloc const&) {
          LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception "
                   << TRI_errno_string(TRI_ERROR_OUT_OF_MEMORY);
          ok = false;
        } catch (...) {
          LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught unknown exception";
          ok = false;
        }
      }
    }
