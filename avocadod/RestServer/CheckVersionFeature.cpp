...
void CheckVersionFeature::checkVersion() {
  *_result = 1;
  ...
  // run version check
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "starting version check";
  ...
  // enter context and isolate
  {
...

    {
...

      {
        ...
        LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "running database version check";
        ...
        for (auto& name : databaseFeature->getDatabaseNames()) {
          ...
          LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "version check return status " << status;

          if (status < 0) {
            LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Database version check failed for '"
                       << vocbase->name()
            ...
            *_result = 3;
            LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "Database version check failed for '"
  ...
  LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "final result of version check: " << *_result;

  if (*_result == 1) {
    *_result = EXIT_SUCCESS;
  } else if (*_result > 1) {
    *_result = EXIT_FAILURE;
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Database version check failed";
    FATAL_ERROR_EXIT();
  }
}
