      ...
      if (database != nullptr) {
        ...
        if (database->type() != TRI_VOCBASE_TYPE_COORDINATOR) {
          ...
          if (database->isOwnAppsDirectory() && !appPath.empty()) {
            std::string path = avocadodb::basics::FileUtils::buildFilename(
                avocadodb::basics::FileUtils::buildFilename(appPath, "_db"),
                database->name());

            if (TRI_IsDirectory(path.c_str())) {
              LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "removing app directory '" << path
        ...
        if (++cleanupCycles >= 10 &&
            avocadodb::ServerState::instance()->isCoordinator()) {
...
void DatabaseFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  if (_maximalJournalSize < TRI_JOURNAL_MINIMAL_SIZE) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for '--database.maximal-journal-size'. "
                  "expected at least "
  ...
  if (_checkVersion && _upgrade) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot specify both '--database.check-version' and "
...
void DatabaseFeature::start() {
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not iterate over all databases: "
  ...
  if (!_databaseManager->start()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start database manager thread";
    FATAL_ERROR_EXIT();
  }

  // activate deadlock detection in case we're not running in cluster mode
  if (!avocadodb::ServerState::instance()->isRunningInCluster()) {
  ...
  for (auto& p : theLists->_databases) {
    ...
    if (vocbase->replicationApplier()->_configuration._autoStart) {
      if (!_replicationApplier) {
        LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "replication applier explicitly deactivated for database '"
                  << vocbase->name() << "'";
      } else {
        int res = vocbase->replicationApplier()->start(0, false, 0);

        if (res != TRI_ERROR_NO_ERROR) {
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "unable to start replication applier for database '"
...
int DatabaseFeature::createDatabase(TRI_voc_tick_t id, std::string const& name,
  ...
  MUTEX_LOCKER(mutexLocker, _databaseCreateLock);
  {
    ...
    try {
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "initializing replication applier for database '"
    ...
    vocbase->_deadlockDetector.enabled(
        !avocadodb::ServerState::instance()->isRunningInCluster());
...

    {
      ...
      try {
        ...
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Out of memory for putting new database into list!";
...
int DatabaseFeature::dropDatabaseCoordinator(TRI_voc_tick_t id, bool force) {
  ...
  if (vocbase != nullptr) {
    ...
    if (vocbase->markAsDropped()) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "dropping coordinator database '" << vocbase->name() << "'";
...
int DatabaseFeature::dropDatabase(std::string const& name, bool waitForDeletion,
  ...
  int res;
  {
    ...
    avocadodb::aql::PlanCache::instance()->invalidate(vocbase);
    avocadodb::aql::QueryCache::instance()->invalidate(vocbase);
...
int DatabaseFeature::createBaseApplicationDirectory(std::string const& appPath,
  ...
  int res = TRI_ERROR_NO_ERROR;
  std::string path = avocadodb::basics::FileUtils::buildFilename(appPath, type);
  ...
  if (!TRI_IsDirectory(path.c_str())) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "created base application directory '" << path << "'";
    } else {
      if ((res != TRI_ERROR_FILE_EXISTS) || (!TRI_IsDirectory(path.c_str()))) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to create base application directory "
                 << errorMessage;
      } else {
        LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "someone else created base application directory '" << path
                  << "'";
        res = TRI_ERROR_NO_ERROR;
      }
    }
...
int DatabaseFeature::createApplicationDirectory(std::string const& name,
  ...
  if (!TRI_IsDirectory(path.c_str())) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "created application directory '" << path
                 << "' for database '" << name << "'";
    } else if (res == TRI_ERROR_FILE_EXISTS) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "unable to create application directory '" << path
                << "' for database '" << name << "': " << errorMessage;
      res = TRI_ERROR_NO_ERROR;
    } else {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unable to create application directory '" << path
               << "' for database '" << name << "': " << errorMessage;
    }
  }
...
int DatabaseFeature::iterateDatabases(VPackSlice const& databases) {
  ...
  try {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot start database: " << ex.what();
    FATAL_ERROR_EXIT();
  } catch (...) {
    delete newLists;

    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot start database: unknown exception";
    FATAL_ERROR_EXIT();
  }
...
void DatabaseFeature::closeDroppedDatabases() {
  ...
  for (TRI_vocbase_t* vocbase : oldList->_droppedDatabases) {
    ...
    if (vocbase->type() == TRI_VOCBASE_TYPE_NORMAL) {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unknown database type " << vocbase->type() << " "
...
void DatabaseFeature::verifyAppPaths() {
  ...
  if (!appPath.empty() && !TRI_IsDirectory(appPath.c_str())) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "created --javascript.app-path directory '" << appPath
                << "'";
    } else {
