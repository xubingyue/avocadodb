...
void LockfileFeature::start() {
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    ...
    if (otherPID.empty()) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "failed to read/write lockfile, please check the file permissions of the lockfile '" << _lockFilename << "'";
    } else {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "database is locked by process " <<
        otherPID << "; please stop it first and check that the lockfile '" << _lockFilename << "' goes away. If you are sure no other avocadod process is running, please remove the lockfile '" << _lockFilename << "' and try again";
    }
    FATAL_ERROR_EXIT();
  }
