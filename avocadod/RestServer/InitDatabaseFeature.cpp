  ...
  if (!_seenPassword) {
    while (true) {
      ...
      if (!password1.empty()) {
        ...
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "passwords do not match, please repeat";
      } else {
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "initialization aborted by user";
        FATAL_ERROR_EXIT();
      }
    }
  }
...
doexit:
  LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << message;
