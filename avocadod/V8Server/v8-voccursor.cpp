...
static void JS_CreateCursor(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  avocadodb::aql::QueryResult result(TRI_ERROR_NO_ERROR);
  result.result = builder;
  result.cached = false;
  result.context = std::make_shared<avocadodb::transaction::StandaloneContext>(vocbase);

  TRI_ASSERT(builder.get() != nullptr);

  try {
    avocadodb::Cursor* cursor = cursors->createFromQueryResult(
