...
static void JS_Transaction(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  // has caught and could not be converted to avocadoError
...
static void JS_EnableNativeBacktraces(
  ...
  avocadodb::basics::Exception::SetVerbose(TRI_ObjectToBoolean(args[0]));
...
static void JS_ParseAql(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  avocadodb::aql::Query query(true, vocbase, aql::QueryString(queryString),
                             nullptr, nullptr,
                             avocadodb::aql::PART_MAIN);
...
static void JS_WarningAql(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (v8g->_query != nullptr) {
    ...
    auto query = static_cast<avocadodb::aql::Query*>(v8g->_query);
...
static void JS_ExecuteAqlJson(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  TRI_GET_GLOBALS();
  avocadodb::aql::Query query(true, vocbase, queryBuilder, options,
                             avocadodb::aql::PART_MAIN);

  auto queryResult = query.execute(
      static_cast<avocadodb::aql::QueryRegistry*>(v8g->_queryRegistry));
...
static void JS_ExecuteAql(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  auto queryResult = query.executeV8(
      isolate, static_cast<avocadodb::aql::QueryRegistry*>(v8g->_queryRegistry));
...
static void JS_QueryIsKilledAql(
  ...
  TRI_GET_GLOBALS();
  if (v8g->_query != nullptr &&
      static_cast<avocadodb::aql::Query*>(v8g->_query)->killed()) {
    TRI_V8_RETURN_TRUE();
  }
...
static void JS_QueryCachePropertiesAql(
  ...
  auto queryCache = avocadodb::aql::QueryCache::instance();
...
static void JS_QuerySleepAql(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  TRI_GET_GLOBALS();
  avocadodb::aql::Query* query = static_cast<avocadodb::aql::Query*>(v8g->_query);
...
static void MapGetVocBase(v8::Local<v8::String> const name,
  ...
  if (!cacheObject.IsEmpty() && cacheObject->HasRealNamedProperty(cacheName)) {
    ...
    collection = TRI_UnwrapClass<avocadodb::LogicalCollection>(
...
void JS_AvocadoDBContext(v8::FunctionCallbackInfo<v8::Value> const& args) {
  TRI_V8_TRY_CATCH_BEGIN(isolate);
  v8::HandleScope scope(isolate);
...
void TRI_InitV8VocBridge(v8::Isolate* isolate, v8::Handle<v8::Context> context,
                         avocadodb::aql::QueryRegistry* queryRegistry,
  ...
  ft->SetClassName(TRI_V8_ASCII_STRING("AvocadoDatabase"));

  v8::Handle<v8::ObjectTemplate> AvocadoNS = ft->InstanceTemplate();
  AvocadoNS->SetInternalFieldCount(2);
  AvocadoNS->SetNamedPropertyHandler(MapGetVocBase);

  // for any database function added here, be sure to add it to in function
  // JS_CompletionsVocbase, too for the auto-completion

  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_engine"),
                       JS_Engine);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_engineStats"),
                       JS_EngineStats);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_version"),
                       JS_VersionServer);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_id"),
                       JS_IdDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_isSystem"),
                       JS_IsSystemDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_name"),
                       JS_NameDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_path"),
                       JS_PathDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_currentWalFiles"),
                       JS_CurrentWalFiles);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_versionFilename"),
                       JS_VersionFilenameDatabase, true);
  TRI_AddMethodVocbase(isolate, AvocadoNS,
                       TRI_V8_ASCII_STRING("_createDatabase"),
                       JS_CreateDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_dropDatabase"),
                       JS_DropDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_databases"),
                       JS_Databases);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_useDatabase"),
                       JS_UseDatabase);
  TRI_AddMethodVocbase(isolate, AvocadoNS, TRI_V8_ASCII_STRING("_flushCache"),
                       JS_FakeFlushCache, true);
  
  v8g->VocbaseTempl.Reset(isolate, AvocadoNS);
  TRI_AddGlobalFunctionVocbase(isolate,
                               TRI_V8_ASCII_STRING("AvocadoDatabase"),
                               ft->GetFunction());

  TRI_InitV8Statistics(isolate, context);

  TRI_InitV8IndexAvocadoDB(isolate, AvocadoNS);

  TRI_InitV8Collections(context, vocbase, v8g, isolate, AvocadoNS);
  TRI_InitV8Views(context, vocbase, v8g, isolate, AvocadoNS);
  ...
  if (v.IsEmpty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "out of memory when initializing VocBase";
    FATAL_ERROR_ABORT();
  }
