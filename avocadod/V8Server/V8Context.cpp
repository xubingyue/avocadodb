...
std::string const GlobalContextMethods::CodeReloadAql =
    "try { require(\"@avocadodb/aql\").reload(); } catch (err) { }";
...
std::string const GlobalContextMethods::CodeWarmupExports =
    "require(\"@avocadodb/actions\").warmupExports()";
...
void V8Context::handleGlobalContextMethods() {
  ...
  for (auto& type : copy) {
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "executing global context method '" << func
               << "' for context " << _id;
    ...
    try {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::V8) << "caught exception during global context method '" << func << "'";
    }
...
void V8Context::handleCancelationCleanup() {
  ...
  LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "executing cancelation cleanup context " << _id;
  ...
  try {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::V8) << "caught exception during cancelation cleanup";
