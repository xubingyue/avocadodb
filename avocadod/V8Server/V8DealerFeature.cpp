...
void V8DealerFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  if (_startupDirectory.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::V8)
  ...
  if (ctx == nullptr) {
    LOG_TOPIC(ERR, avocadodb::Logger::V8) << "failed to get global context";
    FATAL_ERROR_EXIT();
  }
  ...
  if (_appPath.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::V8) << "no value has been specified for --javascript.app-path";
    FATAL_ERROR_EXIT();
  }
...
void V8DealerFeature::start() {
  // dump paths
  {
    ...
    LOG_TOPIC(INFO, avocadodb::Logger::V8) << "JavaScript using " << StringUtils::join(paths, ", ");
  }
...
void V8DealerFeature::collectGarbage() {
  ...
  while (_stopping == 0) {
    try {
      ...
      if (context != nullptr) {
        avocadodb::CustomWorkStack custom("V8 GC", (uint64_t)context->_id);

        LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "collecting V8 garbage in context #" << context->_id
...
void V8DealerFeature::enterLockedContext(TRI_vocbase_t* vocbase,
...

  {
...

    {
      ...
      try {
        LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "entering V8 context " << context->_id;
...
V8Context* V8DealerFeature::enterContext(TRI_vocbase_t* vocbase,
  ...
  TimedAction exitWhenNoContext([](double waitTime) {
    LOG_TOPIC(WARN, avocadodb::Logger::V8) << "giving up waiting for unused V8 context after " << Logger::FIXED(waitTime) << " s";
  }, 60);
    ...
    while (!_stopping) {
      {
        ...
        if (!found) {
          vocbase->release();
          LOG_TOPIC(WARN, avocadodb::Logger::V8) << "specified context #" << id << " not found";
          return nullptr;
        }
      }

      LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "waiting for V8 context " << id << " to become available";
      usleep(50 * 1000);
    }
  ...
  // look for a free context
    ...
    while (_freeContexts.empty() && !_stopping) {
      ...
      LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "waiting for unused V8 context";
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "found unused V8 context";
...
void V8DealerFeature::exitLockedContext(V8Context* context) {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "leaving V8 context " << context->_id;
...
void V8DealerFeature::exitContext(V8Context* context) {
  ...
  if (gc != nullptr) {
    ...
    if (context->_lastGcStamp + _gcFrequency < lastGc) {
      LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "V8 context has reached GC timeout threshold and will be "
                    "scheduled for GC";
      performGarbageCollection = true;
    } else if (context->_numExecutions >= _gcInterval) {
      LOG_TOPIC(TRACE, avocadodb::Logger::V8)
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "returned dirty V8 context";
...
void V8DealerFeature::applyContextUpdate(V8Context* context) {
  for (auto& p : _contextUpdates) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "updated V8 context #" << context->_id;
  }
}
...
void V8DealerFeature::shutdownContexts() {
  ...
  // wait for all contexts to finish
  {
    ...
    for (size_t n = 0; n < 10 * 5; ++n) {
      if (_busyContexts.empty()) {
        LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "no busy V8 contexts";
        break;
      }

      LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "waiting for busy V8 contexts (" << _busyContexts.size()
                 << ") to finish ";
  ...
  // send all busy contexts a terminate signal
  {
    ...
    for (auto& it : _busyContexts) {
      LOG_TOPIC(WARN, avocadodb::Logger::V8) << "sending termination signal to V8 context";
  ...
  if (!_busyContexts.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::V8) << "cannot shutdown V8 contexts";
    FATAL_ERROR_EXIT();
  }

  // stop GC thread
  if (_gcThread != nullptr) {
    LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "waiting for GC Thread to finish action";
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "commanding GC Thread to terminate";
  }
  ...
  LOG_TOPIC(DEBUG, avocadodb::Logger::V8) << "V8 contexts are shut down";
}
...
V8Context* V8DealerFeature::buildContext(size_t id) {
  ...
  try {
...

    {
      ...
      if (context->_context.IsEmpty()) {
        LOG_TOPIC(FATAL, avocadodb::Logger::V8) << "cannot initialize V8 engine";
        FATAL_ERROR_EXIT();
      }
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "initialized V8 context #" << id;
...
void V8DealerFeature::loadJavaScriptFileInternal(std::string const& file, V8Context* context, VPackBuilder* builder) {
...

  {
    ...
    switch (
      ...
      case JSLoader::eSuccess:
        LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "loaded JavaScript file '" << file << "'";
        break;
      case JSLoader::eFailLoad:
        LOG_TOPIC(FATAL, avocadodb::Logger::V8) << "cannot load JavaScript file '" << file << "'";
        FATAL_ERROR_EXIT();
        break;
      case JSLoader::eFailExecute:
        LOG_TOPIC(FATAL, avocadodb::Logger::V8) << "error during execution of JavaScript file '" << file
                    << "'";
        FATAL_ERROR_EXIT();
        break;
    }
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "loaded Javascript file '" << file << "' for V8 context #" << context->_id;
}

void V8DealerFeature::shutdownContext(V8Context* context) {
  TRI_ASSERT(context != nullptr);
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "shutting down V8 context #" << context->_id;
...

    {
      ...
      if (RUNNING_ON_VALGRIND) {
        ...
        while (tries++ < 10 &&
               TRI_RunGarbageCollectionV8(isolate, availableTime)) {
          if (tries > 3) {
            LOG_TOPIC(WARN, avocadodb::Logger::V8) << "waiting for garbage v8 collection to end";
          }
        }
      } else {
        TRI_RunGarbageCollectionV8(isolate, availableTime);
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::V8) << "closed V8 context #" << context->_id;
