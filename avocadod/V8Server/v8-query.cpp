...
aql::QueryResultV8 AqlQuery(
    v8::Isolate* isolate, avocadodb::LogicalCollection const* col,
    std::string const& aql, std::shared_ptr<VPackBuilder> bindVars) {
  TRI_ASSERT(col != nullptr);

  TRI_GET_GLOBALS();
  avocadodb::aql::Query query(true, col->vocbase(), avocadodb::aql::QueryString(aql),
...
static void JS_AnyQuery(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  avocadodb::LogicalCollection const* col = TRI_UnwrapClass<avocadodb::LogicalCollection>(
...
static void JS_LookupByKeys(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  VPackBuilder strippedBuilder =
      avocadodb::aql::BindParameters::StripCollectionNames(keys.slice(), collection->name().c_str());
