...
class V8Task : public std::enable_shared_from_this<V8Task> {
 ...
 public:
  ...
  void setParameter(
      std::shared_ptr<avocadodb::velocypack::Builder> const& parameters);
 ...
 private:
  ...
  std::shared_ptr<avocadodb::velocypack::Builder> _parameters;
  ...
  // now execute the function within this context
  {
    ...
    if (!action.IsEmpty()) {
      ...
      try {
        ...
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception in V8 user task: "
                 << TRI_errno_string(ex.code()) << " " << ex.what();
      } catch (std::bad_alloc const&) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception in V8 user task: "
                 << TRI_errno_string(TRI_ERROR_OUT_OF_MEMORY);
      } catch (...) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught unknown exception in V8 user task";
      }
    }
  }
}
