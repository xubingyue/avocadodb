...
class v8_action_t final : public TRI_action_t {
 public:
  ...
  TRI_action_result_t execute(TRI_vocbase_t* vocbase, GeneralRequest* request,
    ...
    std::string const& c = request->header("x-avocado-v8-context", found);
...

    {
      ...
      if (it == _callbacks.end()) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "no callback function for JavaScript action '" << _url
                  << "'";
  ...
  if (res->Has(ContentTypeKey)) {
    ...
    switch (response->transportType()) {
      ...
      case Endpoint::TransportType::VST:
        response->setHeader(avocadodb::StaticStrings::ContentTypeHeader,
                            contentType);
        break;
...
static void JS_DefineAction(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (result != nullptr) {
    ...
    if (action != nullptr) {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "cannot create callback for V8 action";
    }
  } else {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot define V8 action";
  }
  ...
  if (current->IsObject()) {
    ...
    if (property->IsExternal()) {
      ...
      switch (request->transportType()) {
        case Endpoint::TransportType::HTTP: {
          auto httpRequest = static_cast<avocadodb::HttpRequest*>(e->Value());
          if (httpRequest != nullptr) {
            std::string bodyStr;
...
static void JS_RequestParts(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (current->IsObject()) {
    ...
    if (property->IsExternal()) {
      v8::Handle<v8::External> e = v8::Handle<v8::External>::Cast(property);
      auto request = static_cast<avocadodb::HttpRequest*>(e->Value());
...
static int clusterSendToAllServers(
    ...
    avocadodb::rest::RequestType const& method, std::string const& body) {
