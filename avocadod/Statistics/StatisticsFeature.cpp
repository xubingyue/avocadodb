...
class avocadodb::StatisticsThread final : public Thread {
 public:
...
void StatisticsFeature::prepare() {
  ...
  for (int i = 0; i < ((int)avocadodb::rest::RequestType::ILLEGAL) + 1; ++i) {
    StatisticsCounter c;
...
void StatisticsFeature::start() {
  ...
  if (!_statisticsThread->start()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not start statistics thread";
    FATAL_ERROR_EXIT();
  }
}
