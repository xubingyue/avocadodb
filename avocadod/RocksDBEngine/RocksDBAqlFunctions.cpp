...
AqlValue RocksDBAqlFunctions::Fulltext(
  ...
  avocadodb::RocksDBFulltextIndex* fulltextIndex = nullptr;
  ...
  for (auto const& idx : collection->getIndexes()) {
    if (idx->type() == avocadodb::Index::TRI_IDX_TYPE_FULLTEXT_INDEX) {
      ...
      if (avocadodb::basics::AttributeName::isIdentical(idx->fields(), search,
                                                       false)) {
        // match!
        fulltextIndex = static_cast<avocadodb::RocksDBFulltextIndex*>(idx.get());
        break;
      }
    }
  }
...
static avocadodb::RocksDBGeoIndex* getGeoIndex(
  ...
  avocadodb::RocksDBGeoIndex* index = nullptr;
  for (auto const& idx : document->getIndexes()) {
    if (idx->type() == avocadodb::Index::TRI_IDX_TYPE_GEO1_INDEX ||
      ...
      index = static_cast<avocadodb::RocksDBGeoIndex*>(idx.get());
      break;
    }
  }
...
/// @brief function NEAR
AqlValue RocksDBAqlFunctions::Near(avocadodb::aql::Query* query,
  ...
  avocadodb::RocksDBGeoIndex* index = getGeoIndex(trx, cid, collectionName);
