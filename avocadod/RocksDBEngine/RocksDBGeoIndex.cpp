...
void RocksDBGeoIndexIterator::evaluateCondition() {
  if (_condition) {
    ...
    TRI_ASSERT(fcall->type == avocadodb::aql::NODE_TYPE_FCALL);
...
RocksDBGeoIndex::RocksDBGeoIndex(TRI_idx_iid_t iid,
  ...
  if (_fields.size() == 1) {
    _geoJson = avocadodb::basics::VelocyPackHelper::getBooleanValue(
        info, "geoJson", false);
    auto& loc = _fields[0];
...
void RocksDBGeoIndex::toVelocyPack(VPackBuilder& builder, bool withFigures,
  ...
  // the "constraint" attribute has no meaning since AvocadoDB 2.5 and is only
...
bool RocksDBGeoIndex::matchesDefinition(VPackSlice const& info) const {
  ...
  if (n == 1) {
    if (_geoJson != avocadodb::basics::VelocyPackHelper::getBooleanValue(
                        info, "geoJson", false)) {
      return false;
    }
  }
