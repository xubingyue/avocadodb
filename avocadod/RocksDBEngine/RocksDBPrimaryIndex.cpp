...
RocksDBPrimaryIndex::RocksDBPrimaryIndex(
    avocadodb::LogicalCollection* collection, VPackSlice const& info)
    : RocksDBIndex(0, collection,
                   std::vector<std::vector<avocadodb::basics::AttributeName>>(
                       {{avocadodb::basics::AttributeName(
...
RocksDBToken RocksDBPrimaryIndex::lookupKey(transaction::Methods* trx,
                                            avocadodb::StringRef keyRef) const {
  ...
  avocadodb::Result r = mthds->Get(_cf, key, value.buffer());
  if (!r.ok()) {
    return RocksDBToken();
  }
...
avocadodb::aql::AstNode* RocksDBPrimaryIndex::specializeCondition(
