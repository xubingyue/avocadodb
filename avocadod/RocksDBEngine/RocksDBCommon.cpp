...
avocadodb::Result globalRocksDBPut(rocksdb::ColumnFamilyHandle* cf,
...
avocadodb::Result globalRocksDBRemove(rocksdb::ColumnFamilyHandle* cf,
                                     rocksdb::Slice const& key,
                                     rocksdb::WriteOptions const& options) {
  TRI_ASSERT(cf != nullptr);
  ...
  try {
    ...
    rocksdb::Slice upper(bounds.end());
    {
      ...
      if (!status.ok()) {
        ...
        avocadodb::Result r = rocksutils::convertStatus(status);
        LOG_TOPIC(WARN, avocadodb::Logger::ROCKSDB)
...
std::vector<std::pair<RocksDBKey, RocksDBValue>> collectionKVPairs(
  ...
  iterateBounds(bounds,
                ...
                avocadodb::RocksDBColumnFamily::definitions());
  return rv;
}
