...
#ifndef AVOCADOD_MMFILES_GEO_INDEX_H
#define AVOCADOD_MMFILES_GEO_INDEX_H 1
...
static_assert(sizeof(avocadodb::rocksdbengine::GeoCoordinate::data) >=
...
class RocksDBGeoIndexIterator final : public IndexIterator {
 ...
 private:
  size_t findLastIndex(avocadodb::rocksdbengine::GeoCoordinates* coords) const;
  void replaceCursor(avocadodb::rocksdbengine::GeoCursor* c);
  void createCursor(double lat, double lon);
  void evaluateCondition();  // called in constructor

  RocksDBGeoIndex const* _index;
  avocadodb::rocksdbengine::GeoCursor* _cursor;
  avocadodb::rocksdbengine::GeoCoordinate _coor;
  avocadodb::aql::AstNode const* _condition;
  double _lat;
  double _lon;
  bool _near;
  bool _inclusive;
...
class RocksDBGeoIndex final : public RocksDBIndex {
 ...
 public:
  ...
  avocadodb::rocksdbengine::GeoCoordinates* withinQuery(transaction::Methods*,
                                                       double, double,
                                                       double) const;

  /// @brief looks up the nearest points
  avocadodb::rocksdbengine::GeoCoordinates* nearQuery(transaction::Methods*,
                                                     double, double,
                                                     size_t) const;
 ...
 private:
  ...
  avocadodb::rocksdbengine::GeoIdx* _geoIndex;
};
}  // namespace avocadodb

namespace std {
template <>
class default_delete<avocadodb::rocksdbengine::GeoCoordinates> {
 public:
  void operator()(avocadodb::rocksdbengine::GeoCoordinates* result) {
    if (result != nullptr) {
