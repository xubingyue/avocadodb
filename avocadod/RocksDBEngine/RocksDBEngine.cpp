...
void RocksDBEngine::start() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::ENGINES)
  ...
  if (!basics::FileUtils::isDirectory(_path)) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(TRACE, avocadodb::Logger::ENGINES) << "created RocksDB data directory '" << _path << "'";
    } else {
      LOG_TOPIC(FATAL, avocadodb::Logger::ENGINES) << "unable to create RocksDB data directory '" << _path << "': " << systemErrorStr;
      FATAL_ERROR_EXIT();
    }
  }
  ...
  auto const* opts =
  ApplicationServer::getFeature<avocadodb::RocksDBOptionFeature>(
                                                                "RocksDBOption");
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::ROCKSDB) << "initializing RocksDB, path: '"
  ...
  bool dbExisted = false;
  {
    ...
    if (status.ok()) {
      ...
      LOG_TOPIC(DEBUG, avocadodb::Logger::STARTUP)
      ...
      for (auto const& it : cfFamilies) {
        ...
        if (it2 == existingColumnFamilies.end()) {
          LOG_TOPIC(FATAL, avocadodb::Logger::STARTUP)
              ...
              << ". if you are upgrading from an earlier alpha or beta version of AvocadoDB 3.2, "
  ...
  if (dbExisted) {
    if (s.IsNotFound() || oldVersion.data()[0] < version) {
      ...
      LOG_TOPIC(ERR, Logger::ENGINES)
      << "You are using an old version of AvocadoDB, please update "
...
void RocksDBEngine::getDatabases(avocadodb::velocypack::Builder& result) {
  ...
  for (iter->Seek(rSlice); iter->Valid() && iter->key().starts_with(rSlice);
    ...
    // deleted
    if (avocadodb::basics::VelocyPackHelper::getBooleanValue(slice, "deleted",
                                                            false)) {
      TRI_voc_tick_t id = static_cast<TRI_voc_tick_t>(
          basics::StringUtils::uint64(idSlice.copyString()));

      // database is deleted, skip it!
      LOG_TOPIC(DEBUG, avocadodb::Logger::STARTUP) << "found dropped database "
                                                  << id;
...
int RocksDBEngine::getCollectionsAndIndexes(
  ...
  for (iter->Seek(rSlice); iter->Valid() && iter->key().starts_with(rSlice);
    ...
    if (avocadodb::basics::VelocyPackHelper::readBooleanValue(slice, "deleted",
                                                             false)) {
      continue;
    }
    result.add(slice);
...
std::shared_ptr<avocadodb::velocypack::Builder>
RocksDBEngine::getReplicationApplierConfiguration(TRI_vocbase_t* vocbase,
  ...
  if (!s.ok()) {
    ...
    return std::shared_ptr<avocadodb::velocypack::Builder>();
  }
...
int RocksDBEngine::saveReplicationApplierConfiguration(
    TRI_vocbase_t* vocbase, avocadodb::velocypack::Slice slice, bool doSync) {
  auto key = RocksDBKey::ReplicationApplierConfig(vocbase->id());
...
TRI_vocbase_t* RocksDBEngine::createDatabase(
    TRI_voc_tick_t id, avocadodb::velocypack::Slice const& args, int& status) {
  status = TRI_ERROR_NO_ERROR;
...
avocadodb::Result RocksDBEngine::dropCollection(
...
avocadodb::Result RocksDBEngine::renameCollection(
    TRI_vocbase_t* vocbase, avocadodb::LogicalCollection const* collection,
...
avocadodb::Result RocksDBEngine::persistView(
    TRI_vocbase_t* vocbase, avocadodb::LogicalView const* logical) {
  auto physical = static_cast<RocksDBView*>(logical->getPhysical());
  return physical->persistProperties();
}

avocadodb::Result RocksDBEngine::dropView(TRI_vocbase_t* vocbase,
                                         avocadodb::LogicalView*) {
  // nothing to do here
  return {TRI_ERROR_NO_ERROR};
}
...
void RocksDBEngine::changeView(TRI_vocbase_t* vocbase, TRI_voc_cid_t id,
                               avocadodb::LogicalView const*, bool doSync) {
  // nothing to do here
}
...
avocadodb::Result RocksDBEngine::syncWal(bool waitForSync,
                                        bool waitForCollector,
                                        bool writeShutdownFile) {
#ifdef _WIN32
  // SyncWAL always reports "not implemented" on Windows
  return avocadodb::Result();
#else
...
TRI_vocbase_t* RocksDBEngine::openExistingDatabase(TRI_voc_tick_t id,
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error while opening database: "
                                            << ex.what();
    throw;
  } catch (...) {
...
int RocksDBEngine::handleSyncKeys(avocadodb::InitialSyncer& syncer,
