...
class RocksDBReplicationContext {
 ...
 public:
  ...
  avocadodb::Result dumpKeyChunks(velocypack::Builder& outBuilder,
                                 uint64_t chunkSize);

  /// dump all keys from collection
  avocadodb::Result dumpKeys(velocypack::Builder& outBuilder, size_t chunk,
                            size_t chunkSize, std::string const& lowKey);
  /// dump keys and document
  avocadodb::Result dumpDocuments(velocypack::Builder& b, size_t chunk,
                                 size_t chunkSize, std::string const& lowKey,
 ...
 private:
  ...
  static bool filterCollection(avocadodb::LogicalCollection* collection,
                               void* data);

  static bool sortCollections(avocadodb::LogicalCollection const* l,
 ...
 private:
  ...
  std::shared_ptr<avocadodb::velocypack::CustomTypeHandler> _customTypeHandler;
  avocadodb::velocypack::Options _vpackOptions;
