...
/// @caller js/client/modules/@avocadodb/replication.js
...
void RocksDBRestReplicationHandler::handleCommandLoggerFollow() {
  ...
  if (found) {
    avocadodb::LogicalCollection* c = _vocbase->lookupCollection(value6);
  ...
  if (length > 0) {
    if (useVst) {
      for (auto message : avocadodb::velocypack::ArrayIterator(data)) {
      ...
      for (auto marker : avocadodb::velocypack::ArrayIterator(data)) {
        dumper.dump(marker);
...
/// a given tick value
/// this is an internal method use by AvocadoDB's replication that should not
...
void RocksDBRestReplicationHandler::handleCommandRestoreCollection() {
  ...
  } catch (avocadodb::velocypack::Exception const& e) {
...
void RocksDBRestReplicationHandler::handleCommandCreateKeys() {
  ...
  // avocadodb::LogicalCollection* c = _vocbase->lookupCollection(collection);
  // avocadodb::CollectionGuard guard(_vocbase, c->cid(), false);
  // avocadodb::LogicalCollection* col = guard.collection();
...
void RocksDBRestReplicationHandler::handleCommandGetKeys() {
  ...
  uint64_t batchId = avocadodb::basics::StringUtils::uint64(id);
...
void RocksDBRestReplicationHandler::handleCommandRemoveKeys() {
  ...
  static_cast<CollectionKeysId>(avocadodb::basics::StringUtils::uint64(id));
...
void RocksDBRestReplicationHandler::handleCommandDump() {
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "enter handleCommandDump";
...
int RocksDBRestReplicationHandler::processRestoreCollection(
  ...
  std::string const name = avocadodb::basics::VelocyPackHelper::getStringValue(
      parameters, "name", "");
  ...
  if (avocadodb::basics::VelocyPackHelper::getBooleanValue(parameters, "deleted",
                                                          false)) {
  ...
  avocadodb::LogicalCollection* col = nullptr;

  if (reuseId) {
    TRI_voc_cid_t const cid =
        avocadodb::basics::VelocyPackHelper::extractIdValue(parameters);
...
int RocksDBRestReplicationHandler::createCollection(
    VPackSlice slice, avocadodb::LogicalCollection** dst, bool reuseId) {
  if (dst != nullptr) {
    *dst = nullptr;
  }
  ...
  std::string const name =
      avocadodb::basics::VelocyPackHelper::getStringValue(slice, "name", "");
  ...
  if (reuseId) {
    cid = avocadodb::basics::VelocyPackHelper::extractIdValue(slice);
...
int RocksDBRestReplicationHandler::processRestoreIndexesCoordinator(
  ...
  for (VPackSlice const& idxDef : VPackArrayIterator(indexes)) {
    ...
    res = ci->ensureIndexCoordinator(dbName, col->cid_as_string(), idxDef, true,
                                     avocadodb::Index::Compare, tmp, errorMsg,
                                     3600.0);
    if (res != TRI_ERROR_NO_ERROR) {
      errorMsg =
...
avocadodb::basics::ConditionVariable RocksDBRestReplicationHandler::_condVar;
