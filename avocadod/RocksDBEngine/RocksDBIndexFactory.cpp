...
std::shared_ptr<Index> RocksDBIndexFactory::prepareIndexFromSlice(
  ...
  avocadodb::Index::IndexType const type = avocadodb::Index::type(typeString);
  ...
  if (iid == 0 && !isClusterConstructor) {
    if (!generateKey) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << info.toJson();
    }
    // Restore is not allowed to generate an id
    TRI_ASSERT(generateKey);
    iid = avocadodb::Index::generateId();
  }
  ...
  switch (type) {
    ...
    case avocadodb::Index::TRI_IDX_TYPE_EDGE_INDEX: {
      ...
      newIdx.reset(new avocadodb::RocksDBEdgeIndex(iid, col, info, direction));
      break;
    }
    // case avocadodb::Index::TRI_IDX_TYPE_GEO1_INDEX:
    // case avocadodb::Index::TRI_IDX_TYPE_GEO2_INDEX:
    case avocadodb::Index::TRI_IDX_TYPE_HASH_INDEX: {
      newIdx.reset(new avocadodb::RocksDBHashIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_SKIPLIST_INDEX: {
      newIdx.reset(new avocadodb::RocksDBSkiplistIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_PERSISTENT_INDEX: {
      newIdx.reset(new avocadodb::RocksDBPersistentIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_GEO1_INDEX:
    case avocadodb::Index::TRI_IDX_TYPE_GEO2_INDEX: {
      newIdx.reset(new avocadodb::RocksDBGeoIndex(iid, col, info));
      break;
    }
    case avocadodb::Index::TRI_IDX_TYPE_FULLTEXT_INDEX: {
      newIdx.reset(new avocadodb::RocksDBFulltextIndex(iid, col, info));
      break;
    }
...
void RocksDBIndexFactory::fillSystemIndexes(
    ...
    std::vector<std::shared_ptr<avocadodb::Index>>& systemIndexes) const {
  ...
  systemIndexes.emplace_back(
      std::make_shared<avocadodb::RocksDBPrimaryIndex>(col, builder.slice()));
  // create edges indexes
  if (col->type() == TRI_COL_TYPE_EDGE) {
    systemIndexes.emplace_back(std::make_shared<avocadodb::RocksDBEdgeIndex>(
