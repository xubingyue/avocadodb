...
RocksDBEdgeIndexIterator::RocksDBEdgeIndexIterator(
    ...
    ManagedDocumentResult* mmdr, avocadodb::RocksDBEdgeIndex const* index,
    ...
    : IndexIterator(collection, trx, mmdr, index),
      ...
      _cache(cache),
      _builderIterator(avocadodb::basics::VelocyPackHelper::EmptyArrayValue()) {
...
void RocksDBEdgeIndexIterator::reset() {
  ...
  _keysIterator.reset();
  _builderIterator =
      VPackArrayIterator(avocadodb::basics::VelocyPackHelper::EmptyArrayValue());
}
        ...
        if (finding.found()) {
          ...
          if (cachedData.length() / 2 < limit) {
            ...
            _builderIterator = VPackArrayIterator(
                avocadodb::basics::VelocyPackHelper::EmptyArrayValue());
          } else {
            // We need to copy it.
...
void RocksDBEdgeIndexIterator::lookupInRocksDB(StringRef fromTo) {
  ...
  if (cc != nullptr) {
    ...
    if (!inserted) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::CACHE) << "Failed to cache: "
                                                << fromTo.toString();
      delete entry;
    }
  }
...
avocadodb::aql::AstNode* RocksDBEdgeIndex::specializeCondition(
