...
/// it unique
static std::vector<avocadodb::basics::AttributeName> const KeyAttribute{
    avocadodb::basics::AttributeName("_key", false)};
...
RocksDBVPackIndexIterator::RocksDBVPackIndexIterator(
    ...
    ManagedDocumentResult* mmdr, avocadodb::RocksDBVPackIndex const* index,
...
RocksDBVPackIndex::RocksDBVPackIndex(TRI_idx_iid_t iid,
    ...
    : RocksDBIndex(iid, collection, info, RocksDBColumnFamily::vpack(), false),
      _deduplicate(avocadodb::basics::VelocyPackHelper::getBooleanValue(
...
double RocksDBVPackIndex::selectivityEstimateLocal(
    avocadodb::StringRef const*) const {
...
void RocksDBVPackIndex::buildIndexValues(VPackBuilder& leased,
  ...
  if (_expanding[level] == -1) {  // the trivial, non-expanding case
    ...
    if (slice.isNone() || slice.isNull()) {
      ...
      sliceStack.emplace_back(avocadodb::basics::VelocyPackHelper::NullValue());
    } else {
  ...
  VPackSlice illegalSlice = avocadodb::basics::VelocyPackHelper::IllegalValue();
  ...
  std::unordered_set<VPackSlice, avocadodb::basics::VelocyPackHelper::VPackHash,
      ...
      seen(2, avocadodb::basics::VelocyPackHelper::VPackHash(),
           avocadodb::basics::VelocyPackHelper::VPackEqual());
  ...
  for (auto const& member : VPackArrayIterator(current)) {
    ...
    for (size_t i = _expanding[level] + 1; i < n; i++) {
      if (!current2.isObject()) {
        if (!_sparse) {
          moveOn(avocadodb::basics::VelocyPackHelper::NullValue());
        }
        doneNull = true;
        break;
      }
...
void RocksDBVPackIndex::fillPaths(std::vector<std::vector<std::string>>& paths,
  ...
  paths.clear();
  expanding.clear();
  for (std::vector<avocadodb::basics::AttributeName> const& list : _fields) {
    paths.emplace_back();
...
Result RocksDBVPackIndex::removeInternal(transaction::Methods* trx,
  ...
  size_t const count = elements.size();
  for (size_t i = 0; i < count; ++i) {
    avocadodb::Result r = mthds->Delete(_cf, elements[i]);
    if (!r.ok()) {
      res = r.errorNumber();
    }
  }
...
IndexIterator* RocksDBVPackIndex::iteratorForCondition(
  ...
  if (node == nullptr) {
    ...
    auto getValueAccess = [&](avocadodb::aql::AstNode const* comp,
                              avocadodb::aql::AstNode const*& access,
                              avocadodb::aql::AstNode const*& value) -> bool {
    ...
    for (; usedFields < _fields.size(); ++usedFields) {
      ...
      avocadodb::aql::AstNode const* access = nullptr;
      avocadodb::aql::AstNode const* value = nullptr;
      getValueAccess(comp, access, value);
      // We found an access for this field

      if (comp->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ) {
...
avocadodb::aql::AstNode* RocksDBVPackIndex::specializeCondition(
  ...
  for (size_t i = 0; i < _fields.size(); ++i) {
    ...
    std::sort(nodes.begin(), nodes.end(),
              [](avocadodb::aql::AstNode const* lhs,
