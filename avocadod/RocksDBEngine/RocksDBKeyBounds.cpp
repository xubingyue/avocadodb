...
RocksDBKeyBounds RocksDBKeyBounds::EdgeIndexVertex(
    uint64_t indexId, avocadodb::StringRef const& vertexId) {
...
RocksDBKeyBounds RocksDBKeyBounds::FulltextIndexPrefix(
    uint64_t objectId, avocadodb::StringRef const& word) {
...
RocksDBKeyBounds RocksDBKeyBounds::FulltextIndexComplete(
    uint64_t indexId, avocadodb::StringRef const& word) {
...
RocksDBKeyBounds::RocksDBKeyBounds(RocksDBEntryType type, uint64_t first)
  ...
  switch (_type) {
    ...
    case RocksDBEntryType::View: {
      ...
      // Key: 1 + 8-byte AvocadoDB database ID + 8-byte AvocadoDB collection ID
...
std::ostream& operator<<(std::ostream& stream, RocksDBKeyBounds const& bounds) {
  stream << "[bounds cf: " << RocksDBColumnFamily::columnFamilyName(bounds.columnFamily()) << " type: " << avocadodb::rocksDBEntryTypeName(bounds.type()) << " ";
