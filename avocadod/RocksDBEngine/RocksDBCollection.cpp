...
avocadodb::Result RocksDBCollection::updateProperties(VPackSlice const& slice,
                                                     bool doSync) {
...
avocadodb::Result RocksDBCollection::persistProperties() {
...
void RocksDBCollection::prepareIndexes(
    avocadodb::velocypack::Slice indexesSlice) {
  ...
  for (auto const& v : VPackArrayIterator(indexesSlice)) {
    ...
    if (value.isString()) {
      ...
      avocadodb::Index::IndexType const type =
          avocadodb::Index::type(tmp.c_str());
      if (type == Index::IndexType::TRI_IDX_TYPE_EDGE_INDEX) {
        ...
        if (fields.isArray() && fields.length() == 2) {
          ...
          for (auto const& f : VPackObjectIterator(v)) {
            if (avocadodb::StringRef(f.key) == "fields") {
          ...
          for (auto const& f : VPackObjectIterator(v)) {
            if (avocadodb::StringRef(f.key) == "fields") {
              ...
              to.close();
            } else if (avocadodb::StringRef(f.key) == "id") {
        ...
        b.openObject();
        for (auto const& f : VPackObjectIterator(v)) {
          if (avocadodb::StringRef(f.key) == "id") {
            last++;
...
Result RocksDBCollection::insert(avocadodb::transaction::Methods* trx,
                                 avocadodb::velocypack::Slice const slice,
                                 avocadodb::ManagedDocumentResult& mdr,
                                 OperationOptions& options,
...
Result RocksDBCollection::update(avocadodb::transaction::Methods* trx,
...
Result RocksDBCollection::replace(
    transaction::Methods* trx, avocadodb::velocypack::Slice const newSlice,
    ...
    avocadodb::velocypack::Slice const toSlice) {
  resultMarkerTick = 0;
...
Result RocksDBCollection::remove(avocadodb::transaction::Methods* trx,
                                 avocadodb::velocypack::Slice const slice,
...
void RocksDBCollection::addIndex(std::shared_ptr<avocadodb::Index> idx) {
  // LOCKED from the outside
...
avocadodb::Result RocksDBCollection::fillIndexes(
    transaction::Methods* trx, std::shared_ptr<avocadodb::Index> added) {
  ...
  if (!res.ok()) {
    ...
    avocadodb::Result res2;// do not overwrite original error
...
avocadodb::RocksDBPrimaryIndex* RocksDBCollection::primaryIndex() const {
  ...
  return static_cast<avocadodb::RocksDBPrimaryIndex*>(primary.get());
}
...
avocadodb::Result RocksDBCollection::lookupRevisionVPack(
    ...
    avocadodb::ManagedDocumentResult& mdr, bool withCache) const {
  TRI_ASSERT(trx->state()->isRunning());
  TRI_ASSERT(_objectId != 0);
...
uint64_t RocksDBCollection::recalculateCounts() {
  ...
  avocadodb::SingleCollectionTransaction trx(
      avocadodb::transaction::StandaloneContext::Create(
          _logicalCollection->vocbase()),
...
avocadodb::Result RocksDBCollection::serializeIndexEstimates(
...
avocadodb::Result RocksDBCollection::serializeKeyGenerator(
    rocksdb::Transaction* rtrx) const {
