...
RocksDBKey RocksDBKey::PrimaryIndexValue(
    uint64_t indexId, avocadodb::StringRef const& primaryKey) {
...
RocksDBKey RocksDBKey::EdgeIndexValue(uint64_t indexId,
                                      avocadodb::StringRef const& vertexId,
                                      TRI_voc_rid_t revisionId) {
...
RocksDBKey RocksDBKey::FulltextIndexValue(uint64_t indexId,
                                          avocadodb::StringRef const& word,
                                          TRI_voc_rid_t revisionId) {
...
avocadodb::StringRef RocksDBKey::primaryKey(RocksDBKey const& key) {
  return primaryKey(key._buffer.data(), key._buffer.size());
}

avocadodb::StringRef RocksDBKey::primaryKey(rocksdb::Slice const& slice) {
...
RocksDBKey::RocksDBKey(RocksDBEntryType type, uint64_t first,
                       avocadodb::StringRef const& second)
    : _type(type), _buffer() {
  switch (_type) {
...
RocksDBKey::RocksDBKey(RocksDBEntryType type, uint64_t first,
                       avocadodb::StringRef const& second, uint64_t third)
    : _type(type), _buffer() {
  switch (_type) {
...
avocadodb::StringRef RocksDBKey::primaryKey(char const* data, size_t size) {
  TRI_ASSERT(data != nullptr);
  TRI_ASSERT(size > sizeof(uint64_t));
