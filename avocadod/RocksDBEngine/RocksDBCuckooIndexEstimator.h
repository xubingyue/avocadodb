...
class RocksDBCuckooIndexEstimator {
 ...
 public:
  ...
  RocksDBCuckooIndexEstimator(avocadodb::StringRef const serialized)
      : _randState(0x2636283625154737ULL),
        _slotSize(sizeof(uint16_t)),     // Sort out offsets and alignments
        _counterSize(sizeof(uint32_t)),  // Sort out offsets and alignments
        _logSize(0),
 ...
 private:  // methods
  ...
  void deserializeUncompressed(avocadodb::StringRef const& serialized) {
 ...
 private:               // member variables
  ...
  avocadodb::basics::ReadWriteLock mutable _bucketLock;
};
