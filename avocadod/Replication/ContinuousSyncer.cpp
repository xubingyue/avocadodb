...
retry:
  ...
  if (res == TRI_ERROR_NO_ERROR) {
    ...
    if (_requireFromPresent && !_masterIs27OrHigher) {
      LOG_TOPIC(WARN, Logger::REPLICATION) << "requireFromPresent feature is not supported on master server < AvocadoDB 2.7";
    }
...
int ContinuousSyncer::processDocument(TRI_replication_operation_e type,
  ...
  if (cname.isString()) {
    ...
    avocadodb::LogicalCollection* col = getCollectionByIdOrName(cid, cnameString);
...
int ContinuousSyncer::changeCollection(VPackSlice const& slice) {
  ...
  avocadodb::CollectionGuard guard(_vocbase, cid);
