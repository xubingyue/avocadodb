...
int InitialSyncer::handleCollectionDump(avocadodb::LogicalCollection* col,
    ...
    if (batch == 1) {
      ...
      if (!found) {
        errorMsg = "got invalid response from master at " +
                   ...
                   ": could not find 'X-Avocado-Async' header";
        return TRI_ERROR_REPLICATION_INVALID_RESPONSE;
      }
      ...
      while (true) {
        ...
        if (response != nullptr && response->isComplete()) {
          if (response->hasHeaderField("x-avocado-async-id")) {
...
int InitialSyncer::handleCollectionSync(avocadodb::LogicalCollection* col,
                                        std::string const& cid,
                                        std::string const& collectionName,
                                        TRI_voc_tick_t maxTick,
                                        std::string& errorMsg) {
  ...
  // complete,
  // so we're sending the x-avocado-async header here
  auto headers = createHeaders();
  headers["X-Avocado-Async"] = "store";
  std::unique_ptr<SimpleHttpResult> response(
...
int InitialSyncer::changeCollection(avocadodb::LogicalCollection* col,
                                    VPackSlice const& slice) {
  avocadodb::CollectionGuard guard(_vocbase, col->cid());
  bool doSync =
...
int64_t InitialSyncer::getSize(avocadodb::LogicalCollection* col) {
  SingleCollectionTransaction trx(
      transaction::StandaloneContext::Create(_vocbase), col->cid(),
...
int InitialSyncer::handleCollection(VPackSlice const& parameters,
  ...
  // -------------------------------------------------------------------------------------
    ...
    avocadodb::LogicalCollection* col = getCollectionByIdOrName(cid, masterName);
