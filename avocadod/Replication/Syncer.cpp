...
std::string Syncer::getCName(VPackSlice const& slice) const {
  return avocadodb::basics::VelocyPackHelper::getStringValue(slice, "cname", "");
}
...
avocadodb::LogicalCollection* Syncer::getCollectionByIdOrName(TRI_voc_cid_t cid, std::string const& name) { 
  avocadodb::LogicalCollection* idCol = nullptr;
  avocadodb::LogicalCollection* nameCol = nullptr;
...
int Syncer::createCollection(VPackSlice const& slice, avocadodb::LogicalCollection** dst) {
  if (dst != nullptr) {
    *dst = nullptr;
  }
  ...
  avocadodb::LogicalCollection* col = getCollectionByIdOrName(cid, name);
...
int Syncer::dropCollection(VPackSlice const& slice, bool reportError) {
  avocadodb::LogicalCollection* col = getCollectionByIdOrName(getCid(slice), getCName(slice));
...
int Syncer::dropIndex(avocadodb::velocypack::Slice const& slice) {
  std::string id;
