...
void HttpCommTask::addResponse(HttpResponse* response,
  ...
  if (!_origin.empty()) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "handling CORS response";
  ...
  if (stat != nullptr && avocadodb::Logger::isEnabled(avocadodb::LogLevel::TRACE,
                                                     Logger::REQUESTS)) {
    LOG_TOPIC(TRACE, Logger::REQUESTS)
...
bool HttpCommTask::processRead(double startTime) {
  ...
  if (!_readRequestBody) {
    ...
    if (ptr < end) {
      ...
      if (slen == 11 && std::memcmp(sptr, "VST/1.1\r\n\r\n", 11) == 0) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "got VST request on HTTP port";
        _closeRequested = true;
        return false;
      }

      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "HTTP READ FOR " << (void*)this << ": "
      ...
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "server port " << _connectionInfo.serverPort
      ...
      switch (_requestType) {
        ...
        default: {
          ...
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "got corrupted HTTP request '" << std::string(sptr, l)
                    << "'";
      ...
      if (_readRequestBody) {
        ...
        if (found && StringUtils::trim(expect) == "100-continue") {
          LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "received a 100-continue request";
  ...
  if (_readRequestBody) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << std::string(_readBuffer.c_str() + _bodyPosition, _bodyLength);
  ...
  if (connectionType == "close") {
    ...
    // the connection
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "connection close requested by client";
    _closeRequested = true;
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "no keep-alive, connection close requested by client";
    _closeRequested = true;

  } else if (!_useKeepAliveTimer) {
    // if keepAliveTimeout was set to 0.0, we'll close even keep-alive
    // connections immediately
    LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "keep-alive disabled by admin";
    _closeRequested = true;
  }
  ...
  // forbidden
    ...
    std::string realm = "Bearer token_type=\"JWT\", realm=\"AvocadoDB\"";
...
void HttpCommTask::processRequest(std::unique_ptr<HttpRequest> request) {
  ...
  if (found) {
    uint64_t timeStampInt =
        avocadodb::basics::HybridLogicalClock::decodeTimeStamp(timeStamp);
...
bool HttpCommTask::checkContentLength(HttpRequest* request,
  ...
  if (!expectContentLength && bodyLength > 0) {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "received HTTP GET/HEAD request with content-length, this "
                 "should not happen";
  }

  if ((size_t)bodyLength > MaximalBodySize) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "maximal body size is " << MaximalBodySize
...
void HttpCommTask::processCorsOptions(std::unique_ptr<HttpRequest> request) {
  ...
  if (!_origin.empty()) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "got CORS preflight request";
    ...
    if (!allowHeaders.empty()) {
      ...
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "client requested validation of the following headers: "
                 << allowHeaders;
    }
...
ResponseCode HttpCommTask::handleAuthHeader(HttpRequest* request) const {
  ...
  if (methodPos != std::string::npos) {
    ...
    LOG_TOPIC(DEBUG, avocadodb::Logger::REQUESTS) << "Authorization header: " << authStr;
    try {
