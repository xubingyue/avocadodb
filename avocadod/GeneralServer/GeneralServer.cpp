...
void GeneralServer::startListening() {
  for (auto& it : _endpointList->allEndpoints()) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "trying to bind to endpoint '" << it.first
               << "' for requests";

    bool ok = openEndpoint(it.second);

    if (ok) {
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "bound to endpoint '" << it.first << "'";
    } else {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "failed to bind to endpoint '" << it.first
