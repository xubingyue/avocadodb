...
RestHandler* RestHandlerFactory::createHandler(
  ...
  if (_maintenanceMode.load()) {
    if ((!ServerState::instance()->isCoordinator() &&
      ...
      LOG_TOPIC(DEBUG, avocadodb::Logger::FIXME) << "Maintenance mode: refused path: " << path;
      return new MaintenanceHandler(request.release(), response.release());
    }
  }
  ...
  if (i == ii.end()) {
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "no direct handler found, trying prefixes";
    ...
    if (prefix.empty()) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "no prefix handler found, trying catch all";

      i = ii.find(ROOT_PATH);
      if (i != ii.end()) {
        LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "found catch all handler '/'";
      ...
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "found prefix match '" << prefix << "'";
  ...
  if (i == ii.end()) {
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "no not-found handler, giving up";
    return nullptr;
  }

  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "found handler for path '" << *modifiedPath << "'";
