        ...
        if (name == "bfs") {
          ...
          if (value->stringEquals("path", true)) {
            options->uniqueVertices =
                avocadodb::traverser::TraverserOptions::UniquenessLevel::PATH;
...
static std::unique_ptr<graph::BaseOptions> CreateShortestPathOptions(
    avocadodb::transaction::Methods* trx, AstNode const* node) {
...
void ExecutionPlan::getCollectionsFromVelocyPack(Ast* ast,
  ...
  for (auto const& collection : VPackArrayIterator(collectionsSlice)) {
    ast->query()->collections()->add(
        avocadodb::basics::VelocyPackHelper::checkAndGetStringValue(collection,
                                                                   "name"),
        AccessMode::fromString(
            avocadodb::basics::VelocyPackHelper::checkAndGetStringValue(
                collection, "type")
                .c_str()));
  }
}
...
ExecutionNode* ExecutionPlan::fromSlice(VPackSlice const& slice) {
  ...
  for (auto const& it : VPackArrayIterator(nodes)) {
    ...
    if (ret->getType() == avocadodb::aql::ExecutionNode::SUBQUERY) {
