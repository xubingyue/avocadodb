...
std::shared_ptr<std::vector<std::string>> Collection::shardIds() const {
  ...
  if (coll->isSmart() && coll->type() == TRI_COL_TYPE_EDGE) {
    ...
    for (auto const& n : names) {
      ...
      auto list = clusterInfo->getShardList(
          avocadodb::basics::StringUtils::itoa(collectionInfo->cid()));
  ...
  return clusterInfo->getShardList(
      avocadodb::basics::StringUtils::itoa(getPlanId()));
}
...
void Collection::setCollection(avocadodb::LogicalCollection* coll) {
  collection = coll;
}
