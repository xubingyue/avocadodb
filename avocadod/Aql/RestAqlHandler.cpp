...
void RestAqlHandler::createQueryFromVelocyPack() {
  ...
  if (queryBuilder == nullptr) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "invalid VelocyPack plan in query";
    return;
  }
  ...
  if (plan.isNone()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Invalid VelocyPack: \"plan\" attribute missing.";
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to instantiate the query: " << ex.what();
    generateError(rest::ResponseCode::BAD, TRI_ERROR_QUERY_BAD_JSON_PLAN, ex.what());
    return;
  } catch (...) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to instantiate the query";
  ...
  if (found) {
    ttl = avocadodb::basics::StringUtils::doubleDecimal(ttlstring);
  }
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "could not keep query in registry";
    generateError(rest::ResponseCode::BAD, TRI_ERROR_INTERNAL,
  ...
  try {
    ...
    answerBody.add("queryId",
                   VPackValue(avocadodb::basics::StringUtils::itoa(_qId)));
...
void RestAqlHandler::parseQuery() {
  ...
  if (queryString.empty()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "body must be an object with attribute \"query\"";
    generateError(rest::ResponseCode::BAD, TRI_ERROR_INTERNAL,
  ...
  if (res.code != TRI_ERROR_NO_ERROR) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to instantiate the Query: " << res.details;
...
void RestAqlHandler::createQueryFromString() {
  ...
  std::string const part =
      VelocyPackHelper::getStringValue(querySlice, "part", "");
  if (part.empty()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "body must be an object with attribute \"part\"";
    generateError(rest::ResponseCode::BAD, TRI_ERROR_INTERNAL,
...
void RestAqlHandler::useQuery(std::string const& operation,
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed during use of Query: " << ex.what();

    generateError(rest::ResponseCode::SERVER_ERROR,
                  TRI_ERROR_HTTP_SERVER_ERROR, ex.what());
  } catch (...) {
    _queryRegistry->close(_vocbase, _qId);
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed during use of Query: Unknown exception occurred";
...
void RestAqlHandler::getInfoQuery(std::string const& operation,
  ...
  try {
    ...
    if (operation == "count") {
      ...
      if (shardId.empty()) {
        ...
        if (block->getPlanNode()->getType() != ExecutionNode::SCATTER &&
          ...
          THROW_AVOCADO_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "unexpected node type");
        }
      ...
      _queryRegistry->close(_vocbase, _qId);
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "referenced query not found";
    ...
    _queryRegistry->close(_vocbase, _qId);
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed during use of query: " << ex.message();
    generateError(rest::ResponseCode::SERVER_ERROR, ex.code(),
                  ex.message());
  } catch (std::exception const& ex) {
    _queryRegistry->close(_vocbase, _qId);
...
RestStatus RestAqlHandler::execute() {
  ...
  // " << avocadodb::ServerState::instance()->getId() << ": " <<
  ...
  switch (type) {
    case rest::RequestType::POST: {
      if (suffixes.size() != 1) {
        ...
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Unknown API";
        generateError(rest::ResponseCode::NOT_FOUND,
                      TRI_ERROR_HTTP_NOT_FOUND);
      }
      break;
    ...
    case rest::RequestType::PUT: {
      if (suffixes.size() != 2) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "unknown PUT API";
        generateError(rest::ResponseCode::NOT_FOUND,
                      TRI_ERROR_HTTP_NOT_FOUND);
      } else {
    ...
    case rest::RequestType::GET: {
      if (suffixes.size() != 2) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Unknown GET API";
        generateError(rest::ResponseCode::NOT_FOUND,
                      TRI_ERROR_HTTP_NOT_FOUND);
      } else {
...
bool RestAqlHandler::findQuery(std::string const& idString, Query*& query) {
  _qId = avocadodb::basics::StringUtils::uint64(idString);
  query = nullptr;
...
std::shared_ptr<VPackBuilder> RestAqlHandler::parseVelocyPackBody() {
  try {
    ...
    if (body == nullptr) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "cannot parse json object";
    ...
    if (!tmp.isObject()) {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "body of request must be a VelocyPack object";
      generateError(rest::ResponseCode::BAD,
