...
///
/// @file avocadod/Aql/GraphNode.cpp
///
/// DISCLAIMER
///
...
GraphNode::GraphNode(ExecutionPlan* plan,
  ...
  for (auto const& it : VPackArrayIterator(dirList)) {
    uint64_t dir = avocadodb::basics::VelocyPackHelper::stringUInt64(it);
  ...
  for (auto const& it : VPackArrayIterator(list)) {
    std::string e = avocadodb::basics::VelocyPackHelper::getStringValue(it, "");
  ...
  for (auto const& it : VPackArrayIterator(list)) {
    std::string v = avocadodb::basics::VelocyPackHelper::getStringValue(it, "");
