...
int avocadodb::aql::CompareAstNodes(AstNode const* lhs, AstNode const* rhs,
                                   bool compareUtf8) {
  ...
  switch (lType) {
    ...
    case VPackValueType::Array: {
      ...
      for (size_t i = 0; i < n; ++i) {
        int res = avocadodb::aql::CompareAstNodes(
...
AstNode::AstNode(Ast* ast, avocadodb::velocypack::Slice const& slice)
...
void AstNode::sort() {
  ...
  std::sort(members.begin(), members.end(),
            [](AstNode const* lhs, AstNode const* rhs) {
              return (avocadodb::aql::CompareAstNodes(lhs, rhs, true) < 0);
            });
...
bool AstNode::isAttributeAccessForVariable(
  ...
  while (node->type == NODE_TYPE_ATTRIBUTE_ACCESS ||
    ...
    if (node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
      avocadodb::basics::AttributeName attr(node->getString(), expandNext);
...
/// catching the error
void AstNode::stringify(avocadodb::basics::StringBuffer* buffer, bool verbose,
...
void AstNode::appendValue(avocadodb::basics::StringBuffer* buffer) const {
...
std::ostream& operator<<(std::ostream& stream,
  ...
  if (node != nullptr) {
    stream << avocadodb::aql::AstNode::toString(node);
  }
  return stream;
}
