...
#ifndef AVOCADOD_AQL_SORT_CONDITION_H
#define AVOCADOD_AQL_SORT_CONDITION_H 1
...
class SortCondition {
 ...
 public:
  ...
  size_t coveredAttributes(
      Variable const*,
      std::vector<std::vector<avocadodb::basics::AttributeName>> const&) const;

 private:

  /// @brief fields used in the sort conditions
  std::vector<std::pair<Variable const*,
                        std::vector<avocadodb::basics::AttributeName>>> _fields;
  
  /// @brief const attributes
  std::vector<std::vector<avocadodb::basics::AttributeName>> const _constAttributes;
