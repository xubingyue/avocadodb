...
struct CoordinatorInstanciator : public WalkerWorker<ExecutionNode> {
  ...
  struct EngineInfo {
    EngineInfo(EngineLocation location, size_t id, avocadodb::aql::QueryPart p,
    ...
    avocadodb::aql::QueryPart part;  // only relevant for DBserver parts
    ...
    for (ExecutionNode const* current : info->nodes) {
      ...
      if (current->getType() == ExecutionNode::REMOTE) {
        ...
        static_cast<RemoteNode*>(clone)
            ->server("server:" + avocadodb::ServerState::instance()->getId());
  ...
  void distributePlanToShard(avocadodb::CoordTransactionID& coordTransactionID,
                             EngineInfo* info,
    ...
    if (info->part == avocadodb::aql::PART_MAIN) {
    ...
    //LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "GENERATED A PLAN FOR THE REMOTE SERVERS: " << *(body.get());
    // << "\n";
    ...
    if (cc != nullptr) {
      ...
      std::string const url("/_db/"
                            + avocadodb::basics::StringUtils::urlEncode(collection->vocbase->name()) +
                            "/_api/aql/instantiate");

      auto headers = std::make_unique<std::unordered_map<std::string, std::string>>();
      (*headers)["X-Avocado-Nolock"] = shardId;  // Prevent locking
      cc->asyncRequest("", coordTransactionID, "shard:" + shardId,
                       avocadodb::rest::RequestType::POST,
  ...
  void aggregateQueryIds(EngineInfo* info,
                         std::shared_ptr<avocadodb::ClusterComm>& cc,
                         avocadodb::CoordTransactionID& coordTransactionID,
                         Collection* collection) {
    ...
    for (count = (int)shardIds->size(); count > 0; count--) {
      ...
      if (res.status == avocadodb::CL_COMM_RECEIVED) {
        if (res.answer_code == avocadodb::rest::ResponseCode::OK ||
            res.answer_code == avocadodb::rest::ResponseCode::CREATED ||
            res.answer_code == avocadodb::rest::ResponseCode::ACCEPTED) {
          ...
          std::string theID =
              avocadodb::basics::StringUtils::itoa(info->idOfRemoteNode) + ":" +
              res.shardID;
    ...
    //LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "GOT ALL RESPONSES FROM DB SERVERS: " << nrok << "\n";
  ...
  void distributePlansToShards(EngineInfo* info, QueryId connectedId) {
    //LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "distributePlansToShards: " << info.id;
    Collection* collection = info->getCollection();
    ...
    avocadodb::CoordTransactionID coordTransactionID = TRI_NewTickServer();
    auto cc = avocadodb::ClusterComm::instance();
    if (cc != nullptr) {
      // nullptr only happens on controlled shutdown
    ...
    try {
      ...
      for (auto en = info->nodes.begin(); en != info->nodes.end(); ++en) {
        ...
        if (nodeType == ExecutionNode::GATHER) {
          ...
          for (auto const& shardId : *shardIds) {
            std::string theId =
                avocadodb::basics::StringUtils::itoa(remoteNode->id()) + ":" +
                shardId;
  ...
  void buildTraverserEnginesForNode(GraphNode* en) {
    ...
    std::vector<std::unique_ptr<avocadodb::aql::Collection>> const& edges = en->edgeColls();
    ...
    std::vector<std::unique_ptr<avocadodb::aql::Collection>> const& vertices =
        en->vertexColls();
    if (vertices.empty()) {
    ...
    std::string const url("/_db/" + avocadodb::basics::StringUtils::urlEncode(
                                        query->vocbase()->name()) +
                          "/_internal/traverser");
    auto cc = avocadodb::ClusterComm::instance();
    if (cc == nullptr) {
    ...
    for (auto const& list : mappingServerToCollections) {
      ...
      if (!shardSet.empty()) {
        ...
        headers["X-Avocado-Nolock"] = shardList;  // Prevent locking
        ...
        if (res->status != CL_COMM_SENT) {
          ...
          avocadodb::basics::StringBuffer& body = res->result->getBody();
  ...
  ExecutionEngine* buildEngines() {
    ...
    for (auto it = engines.rbegin(); it != engines.rend(); ++it) {
      EngineInfo* info = &(*it);
      //LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Doing engine: " << it->id << " location:"
      //          << it->location;
      if (info->location == COORDINATOR) {
        ...
        if ((*it).id > 0) {
          ...
          try {
            std::string queryId = avocadodb::basics::StringUtils::itoa(id);
            std::string theID =
                avocadodb::basics::StringUtils::itoa(it->idOfRemoteNode) + "/" +
...
ExecutionEngine* ExecutionEngine::instantiateFromPlan(
    ...
    bool planRegisters) {
  auto role = avocadodb::ServerState::instance()->getRole();
  bool const isCoordinator =
      avocadodb::ServerState::instance()->isCoordinator(role);
  bool const isDBServer = avocadodb::ServerState::instance()->isDBServer(role);
    ...
    if (isCoordinator) {
      ...
      try {
        ...
        for (auto& te : inst.get()->traverserEngines) {
          std::string traverserId = avocadodb::basics::StringUtils::itoa(te.first);
        ...
        for (auto& q : inst.get()->queryIds) {
          ...
          if (pos != std::string::npos) {
            ...
            //          << queryId << std::endl;
            QueryId qId = avocadodb::basics::StringUtils::uint64(queryId);
        ...
        for (auto& p : forLocking) {
          ...
          if (isTraverserEngine) {
            std::string const url(
                "/_db/" +
                avocadodb::basics::StringUtils::urlEncode(vocbase->name()) +
          ...
          for (auto& q : inst.get()->queryIds) {
            ...
            if (pos != std::string::npos) {
              ...
              if (!res->errorMessage.empty()) {
                ...
                LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << msg;
              }
            } else {
              // Remove query from registry:
              try {
                queryRegistry->destroy(
                    vocbase, avocadodb::basics::StringUtils::uint64(queryId),
                    TRI_ERROR_INTERNAL);
              } catch (...) {
                // Ignore problems
              }
