...
using StringBuffer = avocadodb::basics::StringBuffer;
...
AqlItemBlock* GatherBlock::getSome(size_t atLeast, size_t atMost) {
  ...
  std::unique_ptr<AqlItemBlock> res(requestBlock(toSend, static_cast<avocadodb::aql::RegisterId>(nrRegs)));
...
static bool throwExceptionAfterBadSyncRequest(ClusterCommResult* res,
  ...
  if (res->status == CL_COMM_ERROR) {
    ...
    if (res->result != nullptr) {
      ...
      avocadodb::basics::StringBuffer const& responseBodyBuf(res->result->getBody());
...
RemoteBlock::RemoteBlock(ExecutionEngine* engine, RemoteNode const* en,
  ...
  TRI_ASSERT(
      (avocadodb::ServerState::instance()->isCoordinator() && ownName.empty()) ||
...
std::unique_ptr<ClusterCommResult> RemoteBlock::sendRequest(
    avocadodb::rest::RequestType type, std::string const& urlPart,
    ...
    ++_engine->_stats.httpRequests;
    {
      ...
      auto result =
          cc->syncRequest(clientTransactionId, coordTransactionId, _server, type,
                          std::string("/_db/") +
                              avocadodb::basics::StringUtils::urlEncode(
...
int RemoteBlock::shutdown(int errorCode) {
  ...
  } catch (avocadodb::basics::Exception &ex) {
