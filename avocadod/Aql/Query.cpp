...
ExecutionPlan* Query::prepare() {
  ...
  if (!_queryString.empty()) {
    ...
    avocadodb::aql::Optimizer opt(_queryOptions.maxNumberOfPlans);
...
QueryResult Query::execute(QueryRegistry* registry) {
  ...
  try {
    ...
    if (useQueryCache) {
      ...
      auto cacheEntry = avocadodb::aql::QueryCache::instance()->lookup(
          _vocbase, queryHash, _queryString);
      avocadodb::aql::QueryCacheResultEntryGuard guard(cacheEntry);
...
void Query::enterContext() {
  if (!_contextOwnedByExterior) {
    if (_context == nullptr) {
      ...
      ISOLATE;
      TRI_GET_GLOBALS();
      auto ctx = static_cast<avocadodb::transaction::V8Context*>(
...
void Query::addWarningsToVelocyPackObject(
    avocadodb::velocypack::Builder& builder) const {
...
std::shared_ptr<transaction::Context> Query::createTransactionContext() {
  if (_contextOwnedByExterior) {
    // we can use v8
    return avocadodb::transaction::V8Context::Create(_vocbase, true);
  }

  return avocadodb::transaction::StandaloneContext::Create(_vocbase);
}
...
Graph const* Query::lookupGraphByName(std::string const& name) {
  ...
  std::unique_ptr<avocadodb::aql::Graph> g(
      avocadodb::lookupGraphByName(createTransactionContext(), name));
