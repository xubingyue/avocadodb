...
avocadodb::aql::AstNode* IndexBlock::makeUnique(
    avocadodb::aql::AstNode* node) const {
  if (node->type != avocadodb::aql::NODE_TYPE_ARRAY || node->numMembers() >= 2) {
...
bool IndexBlock::initIndexes() {
  ...
  if (!_nonConstExpressions.empty()) {
    ...
    if (_hasV8Expression) {
      ...
      auto engine = _engine;
      avocadodb::basics::ScopeGuard guard{
...
AqlItemBlock* IndexBlock::getSome(size_t atLeast, size_t atMost) {
  ...
  do {
    ...
    if (_returned == saveReturned) {
      ...
      for (avocadodb::aql::RegisterId i = 0; i < curRegs; ++i) {
...
avocadodb::OperationCursor* IndexBlock::orderCursor(size_t currentIndex) {
