...
ShortestPathBlock::ShortestPathBlock(ExecutionEngine* engine,
  ...
  _path = std::make_unique<avocadodb::graph::ShortestPathResult>();

  if (_opts->useWeight()) {
    _finder.reset(
        new avocadodb::graph::AttributeWeightShortestPathFinder(_opts));
  } else {
    _finder.reset(
        new avocadodb::graph::ConstantWeightShortestPathFinder(_opts));
  }
