...
TraversalBlock::TraversalBlock(ExecutionEngine* engine, TraversalNode const* ep)
  ...
  if (avocadodb::ServerState::instance()->isCoordinator()) {
#ifdef USE_ENTERPRISE
    if (ep->isSmart()) {
      _traverser.reset(new avocadodb::traverser::SmartGraphTraverser(
          _opts,
          _mmdr.get(),
          ep->engines(),
          _trx->vocbase()->name(),
...
#endif
      _traverser.reset(new avocadodb::traverser::ClusterTraverser(
          _opts,
          _mmdr.get(),
          ep->engines(),
          _trx->vocbase()->name(),
    ...
    _traverser.reset(
        new avocadodb::traverser::SingleServerTraverser(_opts, _trx, _mmdr.get()));
  }
  ...
  if (avocadodb::ServerState::instance()->isCoordinator()) {
    ...
    if (cc != nullptr) {
      ...
      std::string const url(
          "/_db/" + avocadodb::basics::StringUtils::urlEncode(_trx->vocbase()->name()) +
          "/_internal/traverser/");
      for (auto const& it : *_engines) {
        ...
        auto res = cc->syncRequest(
            ...
            url + avocadodb::basics::StringUtils::itoa(it.second), "", headers,
            30.0);
        if (res->status != CL_COMM_SENT) {
          ...
          LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << message;
        }
      }
    }
  }
