...
void V8Executor::HandleV8Error(v8::TryCatch& tryCatch,
                             v8::Handle<v8::Value>& result,
                             avocadodb::basics::StringBuffer* const buffer,
                             bool duringCompile) {
  ISOLATE;
  ...
  if (tryCatch.HasCaught()) {
    ...
    if (tryCatch.Exception()->IsObject()) {
      ...
      if (objValue->HasOwnProperty(errorNum) &&
        ...
        // found something that looks like an AvocadoError
    ...
    if (buffer) {
      std::string script(buffer->c_str(), buffer->length());
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << msg << " " << script;
...
avocadodb::basics::StringBuffer* V8Executor::initializeBuffer() {
  if (_buffer == nullptr) {
