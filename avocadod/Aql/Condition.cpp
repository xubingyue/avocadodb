...
struct PermutationState {
  PermutationState(avocadodb::aql::AstNode const* value, size_t n)
      : value(value), current(0), n(n) {}

  avocadodb::aql::AstNode const* getValue() const {
    if (value->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_OR ||
        value->type == avocadodb::aql::NODE_TYPE_OPERATOR_NARY_OR) {
  ...
  avocadodb::aql::AstNode const* value;
  size_t current;
  size_t const n;
};
...
ConditionPart::ConditionPart(
    ...
    std::vector<avocadodb::basics::AttributeName> const& attributeNames,
...
void Condition::toVelocyPack(avocadodb::velocypack::Builder& builder,
                             bool verbose) const {
  if (_root == nullptr) {
...
Condition::getConstAttributes(Variable const* reference, bool includeNull) {
  std::vector<std::vector<avocadodb::basics::AttributeName>> result;
...
bool Condition::CanRemove(ExecutionPlan const* plan, ConditionPart const& me,
                          avocadodb::aql::AstNode const* andNode,
                          bool isFromTraverser) {
