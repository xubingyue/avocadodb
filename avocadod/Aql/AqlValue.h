...
#ifndef AVOCADOD_AQL_AQL_VALUE_H
#define AVOCADOD_AQL_AQL_VALUE_H 1
...
struct AqlValue final {
 friend struct std::hash<avocadodb::aql::AqlValue>;
 friend struct std::equal_to<avocadodb::aql::AqlValue>;
 ...
 private:
  union {
    ...
    uint8_t* slice;
    avocadodb::velocypack::Buffer<uint8_t>* buffer;
 ...
 public:
  ...
  explicit AqlValue(bool value) {
    VPackSlice slice(value ? avocadodb::basics::VelocyPackHelper::TrueValue() : avocadodb::basics::VelocyPackHelper::FalseValue());
  ...
  AqlValue(avocadodb::velocypack::Buffer<uint8_t>* buffer, bool& shouldDelete) {
  ...
  explicit AqlValue(avocadodb::velocypack::Builder const& builder) {
  ...
  explicit AqlValue(avocadodb::velocypack::Slice const& slice) {
    initFromSlice(slice);
  }
  ...
  avocadodb::velocypack::Slice slice() const;
 ...
 private:
  ...
  void initFromSlice(avocadodb::velocypack::Slice const& slice) {
    // intentionally do not resolve externals here
    // if (slice.isExternal()) {
    //   // recursively resolve externals
    //   slice = slice.resolveExternals();
    // }
    avocadodb::velocypack::ValueLength length = slice.byteSize();
...
struct AqlValueMaterializer {
  ...
  avocadodb::velocypack::Slice slice(AqlValue const& value,
...
}  // closes namespace avocadodb::aql
}  // closes namespace avocadodb

/// @brief hash function for AqlValue objects
namespace std {

template <>
struct hash<avocadodb::aql::AqlValue> {
  size_t operator()(avocadodb::aql::AqlValue const& x) const {
    std::hash<uint8_t> intHash;
    std::hash<void const*> ptrHash;
    avocadodb::aql::AqlValue::AqlValueType type = x.type();
    size_t res = intHash(type);
    switch (type) {
      case avocadodb::aql::AqlValue::VPACK_SLICE_POINTER: { 
        return res ^ ptrHash(x._data.pointer);
      }
      case avocadodb::aql::AqlValue::VPACK_INLINE: {
        return res ^ static_cast<size_t>(avocadodb::velocypack::Slice(&x._data.internal[0]).hash());
      }
      case avocadodb::aql::AqlValue::VPACK_MANAGED_SLICE: {
        return res ^ ptrHash(x._data.slice);
      }
      case avocadodb::aql::AqlValue::VPACK_MANAGED_BUFFER: {
        return res ^ ptrHash(x._data.buffer);
      }
      case avocadodb::aql::AqlValue::DOCVEC: {
        return res ^ ptrHash(x._data.docvec);
      }
      case avocadodb::aql::AqlValue::RANGE: {
...
template <>
struct equal_to<avocadodb::aql::AqlValue> {
  bool operator()(avocadodb::aql::AqlValue const& a,
                  avocadodb::aql::AqlValue const& b) const {
    avocadodb::aql::AqlValue::AqlValueType type = a.type();
    if (type != b.type()) {
      return false;
    }
    switch (type) {
      ...
      case avocadodb::aql::AqlValue::VPACK_INLINE: {
        return avocadodb::velocypack::Slice(&a._data.internal[0]).equals(avocadodb::velocypack::Slice(&b._data.internal[0]));
      }
      case avocadodb::aql::AqlValue::VPACK_MANAGED_SLICE: {
