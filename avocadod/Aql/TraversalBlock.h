...
#ifndef AVOCADOD_AQL_TRAVERSAL_BLOCK_H
#define AVOCADOD_AQL_TRAVERSAL_BLOCK_H 1
...
class TraversalBlock final : public ExecutionBlock {
 ...
 private:
  ...
  std::vector<avocadodb::aql::AqlValue> _vertices;

  /// @brief edges buffer
  std::vector<avocadodb::aql::AqlValue> _edges;

  /// @brief path buffer
  std::vector<avocadodb::aql::AqlValue> _paths;
  ...
  avocadodb::traverser::TraverserOptions* _opts;

  /// @brief Traverser object
  std::unique_ptr<avocadodb::traverser::Traverser> _traverser;

  /// @brief The information to get the starting point, when a register id is
  /// used
  avocadodb::aql::RegisterId _reg;
