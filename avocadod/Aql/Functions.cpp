...
static AqlValue NumberValue(transaction::Methods* trx, double value, bool nullify) {
  if (std::isnan(value) || !std::isfinite(value) || value == HUGE_VAL || value == -HUGE_VAL) {
    ...
    // convert to 0
    return AqlValue(avocadodb::basics::VelocyPackHelper::ZeroValue());
  }
...
static void RegisterWarning(avocadodb::aql::Query* query,
                            char const* functionName, int code) {
  std::string msg;

  if (code == TRI_ERROR_QUERY_FUNCTION_ARGUMENT_TYPE_MISMATCH) {
    msg = avocadodb::basics::Exception::FillExceptionString(code, functionName);
  } else {
...
static void RegisterInvalidArgumentWarning(avocadodb::aql::Query* query,
...
static bool ListContainsElement(transaction::Methods* trx,
  ...
  while (it.valid()) {
    if (avocadodb::basics::VelocyPackHelper::compare(testeeSlice, it.value(), false, options) == 0) {
...
static bool ListContainsElement(VPackOptions const* options,
  ...
  for (size_t i = 0; i < static_cast<size_t>(list.length()); ++i) {
    if (avocadodb::basics::VelocyPackHelper::compare(testee, list.at(i),
                                                    false, options) == 0) {
      index = i;
      return true;
    }
...
///        object parameters
AqlValue Functions::MergeParameters(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters,
                                 char const* funcName,
                                 bool recursive) {
  ...
  if (n == 0) {
    return AqlValue(avocadodb::basics::VelocyPackHelper::EmptyObjectValue());
  }
  ...
  if (initial.isArray() && n == 1) {
    ...
    for (auto const& it : VPackArrayIterator(initialSlice)) {
      ...
      builder = avocadodb::basics::VelocyPackHelper::merge(builder.slice(), it, false,
                                                          recursive);
    }
  ...
  for (size_t i = 1; i < n; ++i) {
    ...
    builder = avocadodb::basics::VelocyPackHelper::merge(initialSlice, slice, false,
                                                        recursive);
...
AqlValue Functions::IsNull(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
...
AqlValue Functions::IsBool(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::IsNumber(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::IsString(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::IsArray(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::IsObject(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::Typename(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
...
AqlValue Functions::ToNumber(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
  bool failed;
...
AqlValue Functions::ToString(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);

  transaction::StringBufferLeaser buffer(trx);
  avocadodb::basics::VPackStringBufferAdapter adapter(buffer->stringBuffer());
...
AqlValue Functions::ToBool(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  AqlValue a = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::ToArray(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
  ...
  if (value.isNull(true)) {
    return AqlValue(avocadodb::basics::VelocyPackHelper::EmptyArrayValue());
  }
...
AqlValue Functions::Length(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
...
AqlValue Functions::First(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
/// @brief function LAST
AqlValue Functions::Last(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
/// @brief function NTH
AqlValue Functions::Nth(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Contains(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Concat(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  transaction::StringBufferLeaser buffer(trx);
  avocadodb::basics::VPackStringBufferAdapter adapter(buffer->stringBuffer());
...
AqlValue Functions::ConcatSeparator(avocadodb::aql::Query* query,
                                    transaction::Methods* trx,
                                    VPackFunctionParameters const& parameters) {
  transaction::StringBufferLeaser buffer(trx);
  avocadodb::basics::VPackStringBufferAdapter adapter(buffer->stringBuffer());
...
AqlValue Functions::CharLength(avocadodb::aql::Query* query,
                                    transaction::Methods* trx,
                                    VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Lower(avocadodb::aql::Query* query,
                                    transaction::Methods* trx,
                                    VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Upper(avocadodb::aql::Query* query,
                                    transaction::Methods* trx,
                                    VPackFunctionParameters const& parameters) {
...
/// @brief function LIKE
AqlValue Functions::Like(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
  ...
  if (matcher == nullptr) {
    matcher =
        avocadodb::basics::Utf8Helper::DefaultUtf8Helper.buildMatcher(pattern);
  ...
  bool error = false;
  bool const result = avocadodb::basics::Utf8Helper::DefaultUtf8Helper.matches(
...
AqlValue Functions::RegexTest(avocadodb::aql::Query* query,
                              transaction::Methods* trx,
                              VPackFunctionParameters const& parameters) {
...
AqlValue Functions::RegexReplace(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
  ...
  bool error = false;
  std::string result = avocadodb::basics::Utf8Helper::DefaultUtf8Helper.replace(
...
AqlValue Functions::Passthru(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  if (parameters.empty()) {
    return AqlValue(avocadodb::basics::VelocyPackHelper::NullValue());
...
AqlValue Functions::Unset(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::UnsetRecursive(avocadodb::aql::Query* query,
                                   transaction::Methods* trx,
                                   VPackFunctionParameters const& parameters) {
...
/// @brief function KEEP
AqlValue Functions::Keep(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Merge(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::MergeRecursive(avocadodb::aql::Query* query,
                                   transaction::Methods* trx,
                                   VPackFunctionParameters const& parameters) {
...
/// @brief function HAS
AqlValue Functions::Has(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
  if (n < 2) {
...
AqlValue Functions::Attributes(avocadodb::aql::Query* query,
                               transaction::Methods* trx,
                               VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
  ...
  if (doSort) {
    std::set<std::string, avocadodb::basics::VelocyPackHelper::AttributeSorterUTF8>
        keys;
...
AqlValue Functions::Values(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
...
/// @brief function MIN
AqlValue Functions::Min(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
  ...
  for (auto const& it : VPackArrayIterator(slice)) {
    ...
    if (minValue.isNone() || avocadodb::basics::VelocyPackHelper::compare(it, minValue, true, options) < 0) {
      minValue = it;
    }
  }
  if (minValue.isNone()) {
...
/// @brief function MAX
AqlValue Functions::Max(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
  ...
  auto options = trx->transactionContextPtr()->getVPackOptions();
  for (auto const& it : VPackArrayIterator(slice)) {
    if (maxValue.isNone() || avocadodb::basics::VelocyPackHelper::compare(it, maxValue, true, options) > 0) {
      maxValue = it;
    }
  }
  if (maxValue.isNone()) {
...
/// @brief function SUM
AqlValue Functions::Sum(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::Average(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::Sleep(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::RandomToken(avocadodb::aql::Query* query,
                                transaction::Methods* trx,
                                VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
...
/// @brief function MD5
AqlValue Functions::Md5(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
  transaction::StringBufferLeaser buffer(trx);
  ...
  avocadodb::rest::SslInterface::sslMD5(buffer->c_str(), buffer->length(), p,
                                       length);
  ...
  avocadodb::rest::SslInterface::sslHEX(hash, 16, p, length);

  return AqlValue(&hex[0], 32);
}

/// @brief function SHA1
AqlValue Functions::Sha1(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
  transaction::StringBufferLeaser buffer(trx);
  ...
  avocadodb::rest::SslInterface::sslSHA1(buffer->c_str(), buffer->length(), p,
                                        length);
  ...
  avocadodb::rest::SslInterface::sslHEX(hash, 20, p, length);

  return AqlValue(&hex[0], 40);
}

/// @brief function HASH
AqlValue Functions::Hash(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
  AqlValue value = ExtractFunctionParameterValue(trx, parameters, 0);
...
AqlValue Functions::Unique(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  ...
  auto options = trx->transactionContextPtr()->getVPackOptions();
  std::unordered_set<VPackSlice, avocadodb::basics::VelocyPackHelper::VPackHash,
                     avocadodb::basics::VelocyPackHelper::VPackEqual>
      values(512, avocadodb::basics::VelocyPackHelper::VPackHash(),
             avocadodb::basics::VelocyPackHelper::VPackEqual(options));
...
AqlValue Functions::SortedUnique(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
  ...
  avocadodb::basics::VelocyPackHelper::VPackLess<true> less(trx->transactionContext()->getVPackOptions(), &slice, &slice);
  std::set<VPackSlice, avocadodb::basics::VelocyPackHelper::VPackLess<true>> values(less);
  for (auto const& it : VPackArrayIterator(slice)) {
    if (!it.isNone()) {
      values.insert(it);
    }
...
AqlValue Functions::Union(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::UnionDistinct(avocadodb::aql::Query* query,
                                  transaction::Methods* trx,
                                  VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Intersection(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Outersection(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Distance(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Flatten(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
...
/// @brief function ZIP
AqlValue Functions::Zip(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
AqlValue Functions::JsonStringify(avocadodb::aql::Query* query,
                                  transaction::Methods* trx,
                                  VPackFunctionParameters const& parameters) {
...
AqlValue Functions::JsonParse(avocadodb::aql::Query* query,
                              transaction::Methods* trx,
                              VPackFunctionParameters const& parameters) {
...
AqlValue Functions::ParseIdentifier(
    avocadodb::aql::Query* query, transaction::Methods* trx,
    VPackFunctionParameters const& parameters) {
  ...
  std::vector<std::string> parts =
      avocadodb::basics::StringUtils::split(identifier, "/");
...
AqlValue Functions::Slice(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Minus(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
  ...
  std::unordered_map<VPackSlice, size_t,
                     avocadodb::basics::VelocyPackHelper::VPackHash,
                     avocadodb::basics::VelocyPackHelper::VPackEqual>
      contains(512, avocadodb::basics::VelocyPackHelper::VPackHash(),
               avocadodb::basics::VelocyPackHelper::VPackEqual(options));
...
AqlValue Functions::Document(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Round(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
/// @brief function ABS
AqlValue Functions::Abs(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function CEIL
AqlValue Functions::Ceil(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Floor(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
/// @brief function SQRT
AqlValue Functions::Sqrt(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
/// @brief function POW
AqlValue Functions::Pow(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function LOG
AqlValue Functions::Log(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function LOG2
AqlValue Functions::Log2(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Log10(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
/// @brief function EXP
AqlValue Functions::Exp(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function EXP2
AqlValue Functions::Exp2(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
/// @brief function SIN
AqlValue Functions::Sin(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function COS
AqlValue Functions::Cos(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function TAN
AqlValue Functions::Tan(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
/// @brief function ASIN
AqlValue Functions::Asin(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
/// @brief function ACOS
AqlValue Functions::Acos(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
/// @brief function ATAN
AqlValue Functions::Atan(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Atan2(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Radians(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Degrees(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
...
/// @brief function PI
AqlValue Functions::Pi(avocadodb::aql::Query* query,
                       transaction::Methods* trx,
                       VPackFunctionParameters const& parameters) {
...
/// @brief function RAND
AqlValue Functions::Rand(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
AqlValue Functions::FirstDocument(avocadodb::aql::Query* query,
                                  transaction::Methods* trx,
                                  VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
  for (size_t i = 0; i < n; ++i) {
...
AqlValue Functions::FirstList(avocadodb::aql::Query* query,
                              transaction::Methods* trx,
                              VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
  for (size_t i = 0; i < n; ++i) {
...
/// @brief function PUSH
AqlValue Functions::Push(avocadodb::aql::Query* query,
                         transaction::Methods* trx,
                         VPackFunctionParameters const& parameters) {
...
/// @brief function POP
AqlValue Functions::Pop(avocadodb::aql::Query* query,
                        transaction::Methods* trx,
                        VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Append(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Unshift(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Shift(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::RemoveValue(avocadodb::aql::Query* query,
                                transaction::Methods* trx,
                                VPackFunctionParameters const& parameters) {
  ...
  for (auto const& it : VPackArrayIterator(v)) {
    ...
    if (avocadodb::basics::VelocyPackHelper::compare(r, it, false, options) == 0) {
      --limit;
      continue;
    }
    builder->add(it);
...
AqlValue Functions::RemoveValues(avocadodb::aql::Query* query,
                                 transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
...
AqlValue Functions::RemoveNth(avocadodb::aql::Query* query,
                              transaction::Methods* trx,
                              VPackFunctionParameters const& parameters) {
...
AqlValue Functions::NotNull(avocadodb::aql::Query* query,
                            transaction::Methods* trx,
                            VPackFunctionParameters const& parameters) {
  size_t const n = parameters.size();
  for (size_t i = 0; i < n; ++i) {
...
AqlValue Functions::Median(avocadodb::aql::Query* query,
                           transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Percentile(avocadodb::aql::Query* query,
                               transaction::Methods* trx,
                               VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Range(avocadodb::aql::Query* query,
                          transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
...
AqlValue Functions::Position(avocadodb::aql::Query* query,
                             transaction::Methods* trx,
                             VPackFunctionParameters const& parameters) {
  ...
  if (list.length() > 0) {
    ...
    if (ListContainsElement(trx, options, list, searchValue, index)) {
      if (!returnIndex) {
        // return true
        return AqlValue(avocadodb::basics::VelocyPackHelper::TrueValue());
      }
      // return position
      transaction::BuilderLeaser builder(trx);
  ...
  if (!returnIndex) {
    // return false
    return AqlValue(avocadodb::basics::VelocyPackHelper::FalseValue());
  }
...
AqlValue Functions::PregelResult(avocadodb::aql::Query* query, transaction::Methods* trx,
                                        VPackFunctionParameters const& parameters) {
