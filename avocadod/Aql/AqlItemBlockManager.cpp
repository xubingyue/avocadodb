...
AqlItemBlock* AqlItemBlockManager::requestBlock(size_t nrItems,
                                                RegisterId nrRegs) {
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "requesting AqlItemBlock of " << nrItems << " x " << nrRegs;
  ...
  while (tries++ < 2) {
    ...
    if (!_buckets[i].empty()) {
      ...
      // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "returned cached AqlItemBlock with dimensions " << block->size() << " x " << block->getNrRegs();
      break;
    }
  ...
  if (block == nullptr) {
    ...
    // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "created AqlItemBlock with dimensions " << block->size() << " x " << block->getNrRegs();
  }
...
void AqlItemBlockManager::returnBlock(AqlItemBlock*& block) {
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "returning AqlItemBlock of dimensions " << block->size() << " x " << block->getNrRegs();
