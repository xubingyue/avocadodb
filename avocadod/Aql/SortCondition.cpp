...
static bool isContained(std::vector<std::vector<avocadodb::basics::AttributeName>> const& attributes,
                        std::vector<avocadodb::basics::AttributeName> const& attribute) {
  for (auto const& it : attributes) {
    if (avocadodb::basics::AttributeName::isIdentical(it, attribute, false)) {
      return true;
    }
  }
...
SortCondition::SortCondition(
    ...
    std::vector<std::vector<avocadodb::basics::AttributeName>> const& constAttributes,
  ...
  std::vector<avocadodb::basics::AttributeName> fieldNames;
      ...
      if (node != nullptr && node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
        ...
        while (node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
          fieldNames.emplace_back(
              avocadodb::basics::AttributeName(node->getString(), false));
...
size_t SortCondition::coveredAttributes(
  ...
  for (size_t i = 0; i < n; /* no hoisting */) {
    ...
    if (reference == field.first &&
        avocadodb::basics::AttributeName::isIdentical(field.second, indexAttributes[i], false)) {
      // field match
      ++fieldsPosition;
      ++numCovered;
      ++i; // next index field
