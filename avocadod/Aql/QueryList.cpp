...
QueryEntryCopy::QueryEntryCopy(TRI_voc_tick_t id,
                               ...
                               std::shared_ptr<avocadodb::velocypack::Builder> bindParameters,
                               double started,
...
QueryList::QueryList(TRI_vocbase_t*)
    : _lock(),
      ...
      _slow(),
      _slowCount(0),
      _enabled(application_features::ApplicationServer::getFeature<avocadodb::QueryRegistryFeature>("QueryRegistry")->trackSlowQueries()),
      _trackSlowQueries(application_features::ApplicationServer::getFeature<avocadodb::QueryRegistryFeature>("QueryRegistry")->trackSlowQueries()),
      _trackBindVars(application_features::ApplicationServer::getFeature<avocadodb::QueryRegistryFeature>("QueryRegistry")->trackBindVars()),
      _slowQueryThreshold(application_features::ApplicationServer::getFeature<avocadodb::QueryRegistryFeature>("QueryRegistry")->slowQueryThreshold()),
...
int QueryList::kill(TRI_voc_tick_t id) {
  ...
  LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "killing AQL query " << id << " '" << query->queryString() << "'";
...
uint64_t QueryList::killAll(bool silent) {
  ...
  for (auto& it : _current) {
    ...
    if (silent) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "killing AQL query " << query->id() << " '" << query->queryString() << "'";
    } else {
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "killing AQL query " << query->id() << " '" << query->queryString() << "'";
    }
