...
RestStatus RestBatchHandler::executeHttp() {
  ...
  LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "boundary of multipart-message is '" << boundary << "'";
  ...
  while (true) {
    ...
    if (!extractPart(&helper)) {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "received a corrupted multipart message";
    ...
    LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "part header is: " << std::string(headerStart, headerLength);
    ...
    if (bodyLength > 0) {
      LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "part body is '" << std::string(bodyStart, bodyLength)
                 << "'";
    ...
    if (key[0] == 'c' || key[0] == 'C') {
      ...
      if (key == StaticStrings::ContentTypeHeader) {
        ...
        if (value == StaticStrings::BatchContentType) {
          ...
          LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "unexpected content-type '" << value
