...
void RestCursorHandler::processQuery(VPackSlice const& slice) {
  ...
  avocadodb::aql::Query query(false, _vocbase, avocadodb::aql::QueryString(queryString, static_cast<size_t>(l)),
...

  {
    ...
    if (n <= batchSize) {
      ...
      try {
        ...
        if (avocadodb::basics::VelocyPackHelper::getBooleanValue(opts, "count",
                                                                false)) {
    ...
    double ttl = avocadodb::basics::VelocyPackHelper::getNumericValue<double>(
        opts, "ttl", 30);
    bool count = avocadodb::basics::VelocyPackHelper::getBooleanValue(
        opts, "count", false);
...
void RestCursorHandler::registerQuery(avocadodb::aql::Query* query) {
...
std::shared_ptr<VPackBuilder> RestCursorHandler::buildExtra(
    avocadodb::aql::QueryResult& queryResult) const {
...
void RestCursorHandler::modifyCursor() {
  ...
  auto cursorId = static_cast<avocadodb::CursorId>(
      avocadodb::basics::StringUtils::uint64(id));
  bool busy;
