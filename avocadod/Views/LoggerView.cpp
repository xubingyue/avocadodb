...
#define VIEW_LOG_TOPIC(a)                                               \
  !avocadodb::Logger::isEnabled(static_cast<avocadodb::LogLevel>(a),      \
                               Logger::VIEWS)                           \
      ? (void)0                                                         \
      : avocadodb::LogVoidify() & (avocadodb::LoggerStream()              \
                                  << static_cast<avocadodb::LogLevel>(a) \
                                  << Logger::VIEWS                      \
...
std::unique_ptr<ViewImplementation> LoggerView::creator(
    LogicalView* view, avocadodb::velocypack::Slice const& info, bool isNew) {
...
LoggerView::LoggerView(ConstructionGuard const&, LogicalView* logical,
                       avocadodb::velocypack::Slice const& info, bool isNew)
...
avocadodb::Result LoggerView::updateProperties(
    avocadodb::velocypack::Slice const& slice, bool partialUpdate, bool doSync) {
