...
Index::Index(TRI_idx_iid_t iid, avocadodb::LogicalCollection* collection,
             VPackSlice const& slice)
    : _iid(iid),
      _collection(collection),
      _fields(),
      _unique(avocadodb::basics::VelocyPackHelper::getBooleanValue(
          slice, "unique", false)),
      _sparse(avocadodb::basics::VelocyPackHelper::getBooleanValue(
          slice, "sparse", false)),
      _clusterSelectivity(0.1) {
...
Index::Index(VPackSlice const& slice)
    : _iid(avocadodb::basics::StringUtils::uint64(
          avocadodb::basics::VelocyPackHelper::checkAndGetStringValue(slice,
                                                                     "id"))),
      _collection(nullptr),
      _fields(),
      _unique(avocadodb::basics::VelocyPackHelper::getBooleanValue(
...
size_t Index::sortWeight(avocadodb::aql::AstNode const* node) {
  switch (node->type) {
...
void Index::setFields(VPackSlice const& fields, bool allowExpansion) {
  ...
  for (auto const& name : VPackArrayIterator(fields)) {
    ...
    std::vector<avocadodb::basics::AttributeName> parsedAttributes;
...
bool Index::Compare(VPackSlice const& lhs, VPackSlice const& rhs) {
  ...
  if (avocadodb::basics::VelocyPackHelper::compare(lhsType, rhs.get("type"),
                                                  false) != 0) {
    return false;
  }
  ...
  if (value.isBoolean()) {
    if (avocadodb::basics::VelocyPackHelper::compare(value, rhs.get("unique"),
                                                    false) != 0) {
      return false;
    }
  }
  ...
  if (value.isBoolean()) {
    if (avocadodb::basics::VelocyPackHelper::compare(value, rhs.get("sparse"),
                                                    false) != 0) {
      return false;
    }
  }
  ...
  if (type == IndexType::TRI_IDX_TYPE_GEO1_INDEX) {
    ...
    if (value.isBoolean()) {
      if (avocadodb::basics::VelocyPackHelper::compare(value, rhs.get("geoJson"),
                                                      false) != 0) {
        return false;
      }
    }
    ...
    if (value.isNumber()) {
      if (avocadodb::basics::VelocyPackHelper::compare(
    ...
    if (type == IndexType::TRI_IDX_TYPE_HASH_INDEX) {
      ...
      for (size_t i = 0; i < nv; ++i) {
        ...
        for (auto const& vr : VPackArrayIterator(r)) {
          if (avocadodb::basics::VelocyPackHelper::compare(v, vr, false) == 0) {
            found = true;
            break;
          }
        }
      ...
      if (avocadodb::basics::VelocyPackHelper::compare(value, rhs.get("fields"),
                                                      false) != 0) {
        return false;
      }
    }
...
bool Index::supportsFilterCondition(avocadodb::aql::AstNode const*,
...
bool Index::supportsSortCondition(avocadodb::aql::SortCondition const*,
                                  avocadodb::aql::Variable const*,
...
avocadodb::aql::AstNode* Index::specializeCondition(
    avocadodb::aql::AstNode* node, avocadodb::aql::Variable const*) const {
  return node;
}
...
bool Index::canUseConditionPart(
  ...
  if (_sparse) {
    if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_NIN) {
      return false;
    }

    if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN &&
        (other->type == avocadodb::aql::NODE_TYPE_EXPANSION ||
         other->type == avocadodb::aql::NODE_TYPE_ATTRIBUTE_ACCESS)) {
      ...
      */
    } else if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN &&
               access->type == avocadodb::aql::NODE_TYPE_EXPANSION) {
      // value[*] IN a.b
      if (!other->isConstant()) {
        return false;
      }
      ...
      */
    } else if (access->type == avocadodb::aql::NODE_TYPE_ATTRIBUTE_ACCESS) {
      // a.b == value  OR  a.b IN values
      if (!other->isConstant()) {
        return false;
      }

      if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_NE &&
          other->isNullValue()) {
      ...
      } else if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GT) {
        // > null. now note that a certain attribute cannot become null
        try {
          nonNullAttributes.emplace(access->toString());
        } catch (...) {
        }
      } else if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE &&
                 !other->isNullValue()) {
      ...
      if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LT ||
          op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_LE) {
      ...
      if (other->isNullValue() &&
          (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_EQ ||
           op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_GE)) {
      ...
      if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN &&
          other->type == avocadodb::aql::NODE_TYPE_ARRAY) {
  ...
  if (op->type == avocadodb::aql::NODE_TYPE_OPERATOR_BINARY_IN &&
      (other->type == avocadodb::aql::NODE_TYPE_EXPANSION ||
       other->type == avocadodb::aql::NODE_TYPE_ATTRIBUTE_ACCESS)) {
    // value IN a.b  OR  value IN a.b[*]
    avocadodb::aql::Ast::getReferencedVariables(access, variables);
      if (other->type == avocadodb::aql::NODE_TYPE_ATTRIBUTE_ACCESS &&
          variables.find(reference) != variables.end()) {
        variables.clear();
        avocadodb::aql::Ast::getReferencedVariables(other, variables);
      }
  } else {
    // a.b == value  OR  a.b IN values
    avocadodb::aql::Ast::getReferencedVariables(other, variables);
...
void Index::warmup(avocadodb::transaction::Methods*) {
...
std::ostream& operator<<(std::ostream& stream, avocadodb::Index const* index) {
