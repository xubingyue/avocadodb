...
void EngineSelectorFeature::prepare() {
  ...
  if (basics::FileUtils::isRegularFile(_engineFilePath)) {
    try {
      ...
      LOG_TOPIC(FATAL, Logger::STARTUP) << "unable to read content of 'ENGINE' file '" << _engineFilePath << "': " << ex.what() << ". please make sure the file/directory is readable for the avocadod process and user";
      FATAL_ERROR_EXIT();
    }
  } else {
    if (_engine == "auto") {
...
void EngineSelectorFeature::start() {
  ...
  if (!basics::FileUtils::isRegularFile(_engineFilePath)) {
    try {
      ...
      LOG_TOPIC(FATAL, Logger::STARTUP) << "unable to write 'ENGINE' file '" << _engineFilePath << "': " << ex.what() << ". please make sure the file/directory is writable for the avocadod process and user";
      FATAL_ERROR_EXIT();
    }
  }
}
