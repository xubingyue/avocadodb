...
namespace std {
template <>
struct hash<avocadodb::DocumentIdentifierToken> {
  inline size_t operator()(avocadodb::DocumentIdentifierToken const& token) const noexcept {
    return token._data;
  }
};

template <>
struct equal_to<avocadodb::DocumentIdentifierToken> {
  bool operator()(avocadodb::DocumentIdentifierToken const& lhs,
                  avocadodb::DocumentIdentifierToken const& rhs) const {
    return lhs == rhs;
  }
};
