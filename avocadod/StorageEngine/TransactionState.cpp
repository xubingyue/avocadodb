...
int TransactionState::addCollection(TRI_voc_cid_t cid,
  ...
  // LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "cid: " << cid
...
void TransactionState::clearQueryCache() {
  ...
  try {
    ...
    if (!collections.empty()) {
      avocadodb::aql::QueryCache::instance()->invalidate(_vocbase, collections);
    }
  } catch (...) {
    // in case something goes wrong, we have to remove all queries from the
    // cache
    avocadodb::aql::QueryCache::instance()->invalidate(_vocbase);
  }
}
