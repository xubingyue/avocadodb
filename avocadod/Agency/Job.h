...
#ifndef AVOCADOD_CONSENSUS_JOB_H
#define AVOCADOD_CONSENSUS_JOB_H 1
...
struct Job {
  ...
  static std::vector<std::string> availableServers(
    const avocadodb::consensus::Node&);
...
inline avocadodb::consensus::write_ret_t singleWriteTransaction(
  AgentInterface* _agent,
...
inline avocadodb::consensus::trans_ret_t generalTransaction(
...
inline avocadodb::consensus::trans_ret_t transient(AgentInterface* _agent,
                                                  Builder const& transaction,
                                                  bool waitForCommit = true) {
  query_t envelope = std::make_shared<Builder>();
