...
using buffer_t = std::shared_ptr<avocadodb::velocypack::Buffer<uint8_t>>;
using query_t = std::shared_ptr<avocadodb::velocypack::Builder>;
...
inline std::ostream& operator<<(std::ostream& o, avocadodb::consensus::log_t const& l) {
  o << l.index << " " << l.term << " " << VPackSlice(l.entry->data()).toJson()
    << " " << " " << l.clientId << " "<< l.timestamp.count();
  return o;
}
