...
bool State::persist(index_t index, term_t term,
                    avocadodb::velocypack::Slice const& entry,
...
bool State::loadCollections(TRI_vocbase_t* vocbase,
  ...
  if (loadPersisted()) {
    ...
    if (_log.empty()) {
      ...
      VPackSlice value = avocadodb::basics::VelocyPackHelper::EmptyObjectValue();
...
bool State::loadLastCompactedSnapshot(Store& store, index_t& index,
  ...
  avocadodb::aql::Query query(false, _vocbase, aql::QueryString(aql), bindVars,
                             nullptr, avocadodb::aql::PART_MAIN);
...
bool State::loadCompacted() {
  ...
  if (result.isArray() && result.length()) {
    ...
    for (auto const& i : VPackArrayIterator(result)) {
      ...
      buffer_t tmp = std::make_shared<avocadodb::velocypack::Buffer<uint8_t>>();
      (*_agent) = ii;
      try {
...
bool State::removeObsolete(index_t cind) {
  if (cind > 3 * _agent->config().compactionStepSize()) {
    ...
    avocadodb::aql::Query query(false, _vocbase, aql::QueryString(aql),
                               bindVars, nullptr, avocadodb::aql::PART_MAIN);
...
query_t State::allLogs() const {
  ...
  avocadodb::aql::Query compq(false, _vocbase, aql::QueryString(comp),
                             bindVars, nullptr, avocadodb::aql::PART_MAIN);
  avocadodb::aql::Query logsq(false, _vocbase, aql::QueryString(logs),
                             bindVars, nullptr, avocadodb::aql::PART_MAIN);
