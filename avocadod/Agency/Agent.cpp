  ...
  for (auto const& followerId : _config.active()) {
    ...
    if (followerId != myid && leading()) {
      ...
      cc->asyncRequest(
        ...
        avocadodb::rest::RequestType::POST, path.str(),
...
trans_ret_t Agent::transact(query_t const& queries) {
  ...
  avocadodb::consensus::index_t maxind = 0; // maximum write index
  ...
  auto ret = std::make_shared<avocadodb::velocypack::Builder>();
  size_t failed = 0;
  ret->openArray();
  {
...
read_ret_t Agent::read(query_t const& query) {
  ...
  auto result = std::make_shared<avocadodb::velocypack::Builder>();
...
void Agent::notifyInactive() const {
  ...
  for (auto const& p : pool) {
    if (p.first != id()) {
      auto headerFields =
          std::make_unique<std::unordered_map<std::string, std::string>>();
      cc->asyncRequest("1", 1, p.second, avocadodb::rest::RequestType::POST,
...
avocadodb::consensus::index_t Agent::rebuildDBs() {
...
/// Last commit index
std::pair<avocadodb::consensus::index_t, avocadodb::consensus::index_t>
Agent::lastCommitted() const {
  MUTEX_LOCKER(ioLocker, _ioLock);
  MUTEX_LOCKER(liLocker, _liLock);
  return std::pair<avocadodb::consensus::index_t, avocadodb::consensus::index_t>(
    _lastApplied, _commitIndex);
}

/// Last commit index
void Agent::lastCommitted(avocadodb::consensus::index_t lastCommitIndex) {
  MUTEX_LOCKER(ioLocker, _ioLock);
...
/// Get readdb
avocadodb::consensus::index_t Agent::readDB(Node& node) const {
  MUTEX_LOCKER(ioLocker, _ioLock);
  node = _readDB.get();
  return _commitIndex;
}
...
Agent& Agent::operator=(VPackSlice const& compaction) {
  ...
  try {
    _commitIndex = avocadodb::basics::StringUtils::uint64(
...
query_t Agent::buildDB(avocadodb::consensus::index_t index) {
