  ...
  if (inform && _agent->leading()) {
    ...
    for (auto const& url : urls) {
      ...
      if (endpointPathFromUrl(url, endpoint, path)) {
        ...
        avocadodb::ClusterComm::instance()->asyncRequest(
  ...
  for (auto const path : query_strs) {
    ...
    if (e == pv.size()) {  // existing
      ...
      if (copy(pv).type() == LEAF && copy(pv).slice().isNone()) {
        copy(pv) = avocadodb::basics::VelocyPackHelper::EmptyObjectValue();
      }
    }
  }
...
bool Store::applies(avocadodb::velocypack::Slice const& transaction) {
