...
void AgencyFeature::start() {
  ...
  if (!feature->agencyPrefix().empty()) {
    avocadodb::consensus::Supervision::setAgencyPrefix(
      std::string("/") + feature->agencyPrefix());
    avocadodb::consensus::Job::agencyPrefix
      = std::string("/") + feature->agencyPrefix();
  }
