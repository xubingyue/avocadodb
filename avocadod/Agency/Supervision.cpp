...
std::string Supervision::_agencyPrefix = "/avocado";

Supervision::Supervision()
  : avocadodb::Thread("Supervision"),
  _agent(nullptr),
...
void Supervision::upgradeOne(Builder& builder) {
  // "/avocado/Agency/Definition" not exists or is 0
...
void Supervision::upgradeZero(Builder& builder) {
  // "/avocado/Target/FailedServers" is still an array
...
void Supervision::run() {
  // First wait until somebody has initialized the AvocadoDB data, before
  ...
  while (!this->isStopping()) {
    ...
    LOG_TOPIC(DEBUG, Logger::SUPERVISION) << "Waiting for AvocadoDB to "
      "initialize its data.";
  }
