...
static void JS_WriteAgent(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (ret.accepted) {  // Leading
    ...
    avocadodb::consensus::index_t max_index = 0;
    try {
...
void TRI_InitV8Agency(v8::Isolate* isolate, v8::Handle<v8::Context> context) {
  ...
  ft->SetClassName(TRI_V8_ASCII_STRING("AvocadoAgent"));
  ...
  ft->SetClassName(TRI_V8_ASCII_STRING("AvocadoAgentCtor"));

  TRI_AddGlobalFunctionVocbase(isolate, TRI_V8_ASCII_STRING("AvocadoAgentCtor"),
                               ft->GetFunction(), true);
  ...
  if (!aa.IsEmpty()) {
    TRI_AddGlobalVariableVocbase(isolate,
                                 TRI_V8_ASCII_STRING("AvocadoAgent"), aa);
  }
}
