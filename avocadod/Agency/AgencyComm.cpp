...
std::string AgencyCommResult::errorMessage() const {
  ...
  try {
    ...
    return avocadodb::basics::VelocyPackHelper::getStringValue(body, "message",
                                                              "");
...
std::string AgencyComm::version() {
  AgencyCommResult result =
      sendWithFailover(avocadodb::rest::RequestType::GET,
...
bool AgencyComm::exists(std::string const& key) {
  ...
  auto parts = avocadodb::basics::StringUtils::split(key, "/");
...
AgencyCommResult AgencyComm::getValues(std::string const& key) {
  ...
  AgencyCommResult result =
      sendWithFailover(avocadodb::rest::RequestType::POST,
                       AgencyCommManager::CONNECTION_OPTIONS._requestTimeout,
                       url, builder.slice());
...
AgencyCommResult AgencyComm::casValue(std::string const& key,
                                      avocadodb::velocypack::Slice const& json,
...
void AgencyComm::updateEndpoints(avocadodb::velocypack::Slice const& current) {
...
AgencyCommResult AgencyComm::sendWithFailover(
    avocadodb::rest::RequestType method, double const timeout,
  ...
  while (true) {  // will be left by timeout eventually
    ...
    if (result._statusCode ==
        (int)avocadodb::rest::ResponseCode::TEMPORARY_REDIRECT) {
...
AgencyCommResult AgencyComm::send(
    avocadodb::httpclient::GeneralClientConnection* connection,
    avocadodb::rest::RequestType method, double timeout, std::string const& url,
    std::string const& body, std::string const& clientId) {
  TRI_ASSERT(connection != nullptr);

  if (method == avocadodb::rest::RequestType::GET ||
      method == avocadodb::rest::RequestType::HEAD ||
      method == avocadodb::rest::RequestType::DELETE_REQ) {
  ...
  LOG_TOPIC(TRACE, Logger::AGENCYCOMM)
      << "sending " << avocadodb::HttpRequest::translateMethod(method)
      << " request to agency at endpoint '"
      << connection->getEndpoint()->specification() << "', url '" << url
      << "': " << body;

  avocadodb::httpclient::SimpleHttpClientParams params(timeout, false);
  params.setJwt(ClusterComm::instance()->jwt());
  params.keepConnectionOnDestruction(true);
  avocadodb::httpclient::SimpleHttpClient client(connection, params);

  // set up headers
  std::unordered_map<std::string, std::string> headers;

  if (method == avocadodb::rest::RequestType::POST) {
    // the agency needs this content-type for the body
    headers["content-type"] = "application/json";
  }

  // send the actual request
  std::unique_ptr<avocadodb::httpclient::SimpleHttpResult> response(
