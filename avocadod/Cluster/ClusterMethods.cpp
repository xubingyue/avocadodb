...
static T ExtractFigure(VPackSlice const& slice, char const* group,
  ...
  return avocadodb::basics::VelocyPackHelper::getNumericValue<T>(g, name, 0);
}
...
static int distributeBabyOnShards(
  ...
  if (!node.isObject()) {
    ...
    if (keySlice.isNone()) {
      ...
      _key = avocadodb::basics::StringUtils::itoa(uid);
    } else {
      userSpecifiedKey = true;
    }
...
std::unordered_map<std::string, std::string> getForwardableRequestHeaders(
    avocadodb::GeneralRequest* request) {
  ...
  while (it != headers.end()) {
    ...
    if (key != "x-avocado-async" && key != "authorization" &&
...
bool shardKeysChanged(std::string const& dbname, std::string const& collname,
  ...
  for (size_t i = 0; i < shardKeys.size(); ++i) {
    ...
    VPackSlice nullValue =
        avocadodb::velocypack::buildNullValue(&buffer[0], sizeof(buffer));
    ...
    if (avocadodb::basics::VelocyPackHelper::compare(n, o, false) != 0) {
      return true;
    }
  }
...
int revisionOnCoordinator(std::string const& dbname,
  ...
  for (auto const& p : *shards) {
    ...
    cc->asyncRequest(
        ...
        avocadodb::rest::RequestType::GET,
...
int figuresOnCoordinator(std::string const& dbname, std::string const& collname,
                         std::shared_ptr<avocadodb::velocypack::Builder>& result) {
  // Set a few variables needed for our work:
  ClusterInfo* ci = ClusterInfo::instance();
  auto cc = ClusterComm::instance();
  if (cc == nullptr) {
  ...
  for (auto& req : requests) {
    ...
    if (res.status == CL_COMM_RECEIVED) {
      if (res.answer_code == avocadodb::rest::ResponseCode::OK) {
        ...
        if (answer.isObject()) {
          // add to the total
          result.emplace_back(res.shardID, avocadodb::basics::VelocyPackHelper::getNumericValue<uint64_t>(
                  answer, "count", 0));
        } else {
          return TRI_ERROR_INTERNAL;
        }
        ...
        if (answer.isObject()) {
          ...
          for(auto const& identifier : VPackObjectIterator(answer.get("identifiers"))){
            if(identifier.value.hasKey("selectivityEstimate")) {
              ...
              double estimate = avocadodb::basics::VelocyPackHelper::getNumericValue(
...
int createDocumentOnCoordinator(
    ...
    avocadodb::OperationOptions const& options, VPackSlice const& slice,
    avocadodb::rest::ResponseCode& responseCode,
    std::unordered_map<int, size_t>& errorCounter,
    std::shared_ptr<VPackBuilder>& resultBody) {
  // Set a few variables needed for our work:
  ClusterInfo* ci = ClusterInfo::instance();
  ...
  for (auto const& it : shardMap) {
    ...
    requests.emplace_back(
        "shard:" + it.first, avocadodb::rest::RequestType::POST,
...
int deleteDocumentOnCoordinator(
    std::string const& dbname, std::string const& collname,
    VPackSlice const slice, avocadodb::OperationOptions const& options,
    avocadodb::rest::ResponseCode& responseCode,
    std::unordered_map<int, size_t>& errorCounter,
    std::shared_ptr<avocadodb::velocypack::Builder>& resultBody) {
  // Set a few variables needed for our work:
  ClusterInfo* ci = ClusterInfo::instance();
  auto cc = ClusterComm::instance();
  if (cc == nullptr) {
  ...
  if (useDefaultSharding) {
    ...
    for (auto const& it : shardMap) {
      ...
      requests.emplace_back(
          "shard:" + it.first,
          avocadodb::rest::RequestType::DELETE_REQ,
          baseUrl + StringUtils::urlEncode(it.first) + optsUrlPart, body);
    }
  ...
  for (auto const& shard : *shardList) {
    requests.emplace_back(
        "shard:" + shard, avocadodb::rest::RequestType::DELETE_REQ,
  ...
  if (!useMultiple) {
    ...
    for (count = requests.size(); count > 0; count--) {
      ...
      if (res.status == CL_COMM_RECEIVED) {
        if (res.answer_code !=
                avocadodb::rest::ResponseCode::NOT_FOUND ||
            (nrok == 0 && count == 1)) {
          nrok++;
...
int getDocumentOnCoordinator(
  ...
  avocadodb::rest::RequestType reqType;
  if (!useMultiple) {
    if (options.silent) {
      reqType = avocadodb::rest::RequestType::HEAD;
    } else {
      reqType = avocadodb::rest::RequestType::GET;
    }
  } else {
    reqType = avocadodb::rest::RequestType::PUT;
    if (options.silent) {
...
int fetchEdgesFromEngines(
  ...
  for (auto const& req : requests) {
    ...
    filtered += avocadodb::basics::VelocyPackHelper::getNumericValue<size_t>(
        resSlice, "filtered", 0);
    read += avocadodb::basics::VelocyPackHelper::getNumericValue<size_t>(
...
void fetchVerticesFromEngines(
  ...
  for (auto const& v : vertexIds) {
    result.emplace(
        v, VPackBuilder::clone(avocadodb::basics::VelocyPackHelper::NullValue())
               .steal());
  }
  vertexIds.clear();
}
...
void fetchVerticesFromEngines(
    ...
    std::unordered_map<ServerID, traverser::TraverserEngineID> const* engines,
    std::unordered_set<StringRef>& vertexIds,
    std::unordered_map<StringRef, avocadodb::velocypack::Slice>& result,
    std::vector<std::shared_ptr<avocadodb::velocypack::Builder>>& datalake,
    VPackBuilder& builder) {
  auto cc = ClusterComm::instance();
  if (cc == nullptr) {
    // nullptr happens only during controlled shutdown
...
int getFilteredEdgesOnCoordinator(
  ...
  for (auto const& p : *shards) {
    requests.emplace_back(
        "shard:" + p.first, avocadodb::rest::RequestType::GET,
  ...
  responseCode = avocadodb::rest::ResponseCode::OK;
  ...
  for (auto const& req : requests) {
    ...
    bool const isError = avocadodb::basics::VelocyPackHelper::getBooleanValue(
        shardSlice, "error", false);

    if (isError) {
      // shard returned an error
      return avocadodb::basics::VelocyPackHelper::getNumericValue<int>(
    ...
    if (stats.isObject()) {
      filtered += avocadodb::basics::VelocyPackHelper::getNumericValue<size_t>(
          stats, "filtered", 0);
      scannedIndex +=
          avocadodb::basics::VelocyPackHelper::getNumericValue<size_t>(
...
int modifyDocumentOnCoordinator(
  ...
  avocadodb::rest::RequestType reqType;
  if (isPatch) {
    reqType = avocadodb::rest::RequestType::PATCH;
    if (!options.keepNull) {
...
int flushWalOnAllDBServers(bool waitForSync, bool waitForCollector) {
  ...
  for (auto it = DBservers.begin(); it != DBservers.end(); ++it) {
    ...
    cc->asyncRequest("", coordTransactionID, "server:" + *it,
                     avocadodb::rest::RequestType::PUT, url, body,
  ...
  if (nrok != (int)DBservers.size()) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "could not flush WAL on all servers. confirmed: " << nrok << ", expected: " << DBservers.size();
    return TRI_ERROR_INTERNAL;
  }
  ...
  if (!distributeShardsLike.empty()) {
    ...
    if (otherCid != 0) {
      ...
      std::string otherCidString 
        = avocadodb::basics::StringUtils::itoa(otherCid);
