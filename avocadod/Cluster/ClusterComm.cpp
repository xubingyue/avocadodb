...
std::shared_ptr<ClusterComm> avocadodb::ClusterComm::_theInstance;
...
std::atomic<int> avocadodb::ClusterComm::_theInstanceInit(0);
...
/// use this functionality for the agency mode of AvocadoDB.
...
OperationID ClusterComm::asyncRequest(
    ...
    avocadodb::rest::RequestType reqtype, std::string const& path,
...
std::unique_ptr<ClusterCommResult> ClusterComm::syncRequest(
  ...
  avocadodb::basics::ConditionVariable cv;
  bool doLogConnectionErrors = logConnectionErrors();
...
size_t ClusterComm::performRequests(std::vector<ClusterCommRequest>& requests,
                                    ...
                                    avocadodb::LogTopic const& logTopic,
...
size_t ClusterComm::performSingleRequest(
    ...
    size_t& nrDone, avocadodb::LogTopic const& logTopic) {
  CoordTransactionID coordinatorTransactionID = TRI_NewTickServer();
...
std::pair<ClusterCommResult*, HttpRequest*> ClusterComm::prepareRequest(std::string const& destination,
      avocadodb::rest::RequestType reqtype, std::string const* body,
      std::unordered_map<std::string, std::string> const& headerFields) {
  ...
  if (destination.substr(0, 6) == "shard:") {
    if (CollectionLockState::_noLockHeaders != nullptr) {
      ...
      if (it != CollectionLockState::_noLockHeaders->end()) {
        ...
        headersCopy["X-Avocado-Nolock"] = result->shardID;
      }
    }
  }
  ...
  headersCopy[StaticStrings::HLCHeader] =
    avocadodb::basics::HybridLogicalClock::encodeTimeStamp(timeStamp);
  ...
  headersCopy["X-Avocado-BT-SYNC"] = bt;
#endif
#endif
#endif
...
void ClusterCommThread::run() {
  ...
  while (!isStopping()) {
    try {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception in ClusterCommThread: " << ex.what();
    } catch (...) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught unknown exception in ClusterCommThread";
    }
  }
