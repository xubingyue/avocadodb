      ...
      if (shortIdMap.isObject()) {
        for (auto const& s : VPackObjectIterator(shortIdMap)) {
          if (s.key.copyString() == "ShortName") {
            if (avocadodb::basics::VelocyPackHelper::getStringValue(s.value, "") == shortName) {
...
void mkdir (std::string const& path) {
  if (!TRI_IsDirectory(path.c_str())) {
    if (!avocadodb::basics::FileUtils::createDirectory(path)) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Couldn't create file directory " << path << " (UUID)";
      FATAL_ERROR_EXIT();
    }
  }
}
  ...
  while (++count <= 600) {
    ...
    if (!result.successful()) {
      ...
      if (!slice.isObject()) {
        ...
        if (slice.isObject()) {
          id = avocadodb::basics::VelocyPackHelper::getStringValue(slice, "ID",
                                                                  "");
          if (id.empty()) {
...
ServerState::RoleEnum ServerState::checkServersList(std::string const& id) {
  ...
  if (!me.isNone()) {
    ...
    for (auto const& s : VPackObjectIterator(dbservers)) {
      ...
      std::string name =
          avocadodb::basics::VelocyPackHelper::getStringValue(slice, "");
  ...
  if (isClusterRole(role)) {
    ...
    if (role == ServerState::ROLE_COORDINATOR) {
      try {
        ...
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "out of memory";
        FATAL_ERROR_EXIT();
      }
    ...
    if (trx) {
      if (AgencyCommManager::MANAGER) {
        ...
        if (!result.successful()) {
          if (fatalError) {
            LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "unable to register server in agency";
            FATAL_ERROR_EXIT();
          } else {
            return false;
          }
