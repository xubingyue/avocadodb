...
class CollectionInfoCurrent {
 ...
 public:
  ...
  int errorNum(ShardID const& shardID) const {
    ...
    if (it != _vpacks.end()) {
      VPackSlice slice = it->second->slice();
      return avocadodb::basics::VelocyPackHelper::getNumericValue<int>(slice,
                                                                "errorNum", 0);
    }
    return 0;
  }
  ...
  std::unordered_map<ShardID, int> errorNum() const {
    ...
    for (auto const& it: _vpacks) {
      s = avocadodb::basics::VelocyPackHelper::getNumericValue<int>(it.second->slice(), "errorNum", 0);
 ...
 private:
  bool getFlag(char const* name, ShardID const& shardID) const {
    auto it = _vpacks.find(shardID);
    if (it != _vpacks.end()) {
      return avocadodb::basics::VelocyPackHelper::getBooleanValue(it->second->slice(), name, false);
    }
    return false;
  }
  ...
  std::unordered_map<ShardID, bool> getFlag(char const* name) const {
    ...
    for (auto const& it: _vpacks) {
      ...
      bool b = avocadodb::basics::VelocyPackHelper::getBooleanValue(vpack->slice(), name, false);
...
class ClusterInfo {
 ...
 public:
  ...
  int ensureIndexCoordinator(
      ...
      bool (*compare)(avocadodb::velocypack::Slice const&,
                      avocadodb::velocypack::Slice const&),
      avocadodb::velocypack::Builder& resultBuilder, std::string& errorMsg, double timeout);
  ...
  int getResponsibleShard(LogicalCollection*, avocadodb::velocypack::Slice,
 ...
 private:
  ...
  int ensureIndexCoordinatorWithoutRollback(
      std::string const& databaseName, std::string const& collectionID,
      std::string const& idSlice, avocadodb::velocypack::Slice const& slice, bool create,
      bool (*compare)(avocadodb::velocypack::Slice const&,
                      avocadodb::velocypack::Slice const&),
      avocadodb::velocypack::Builder& resultBuilder, std::string& errorMsg, double timeout);
  ...
  struct ProtectionData {
    ...
    avocadodb::basics::ReadWriteLock lock;
  ...
  avocadodb::Mutex _failedServersMutex;  
