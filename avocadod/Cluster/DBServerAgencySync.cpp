...
DBServerAgencySyncResult DBServerAgencySync::execute() {
  ...
  if (context == nullptr) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "DBServerAgencySync::execute no V8 context";
    return result;
  }

  double now = TRI_microtime();
  if (now - startTime > 5.0) {
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "DBServerAgencySync::execute took more than 5s to get free V8 context, starting handle-plan-change now";
  }
  ...
  try {
    ...
    auto content =
        TRI_V8_ASCII_STRING("require('@avocadodb/cluster').handlePlanChange");
