...
using ClusterTraverser = avocadodb::traverser::ClusterTraverser;
...
void ClusterTraverser::setStartVertex(std::string const& vid) {
  ...
  if (_opts->useBreadthFirst) {
    _enumerator.reset(
        new avocadodb::graph::BreadthFirstEnumerator(this, idSlice, _opts));
  } else {
    _enumerator.reset(
        new avocadodb::traverser::DepthFirstEnumerator(this, vid, _opts));
  }
  _done = false;
}
...
bool ClusterTraverser::getSingleVertex(avocadodb::velocypack::Slice edge, StringRef const sourceVertexId,
