...
void ClusterFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options->addObsoleteOption("--cluster.disable-dispatcher-kickstarter",
                             "The dispatcher feature isn't available anymore; Use AvocadoDBStarter for this now!",
                             true);
  options->addObsoleteOption("--cluster.disable-dispatcher-frontend",
                             "The dispatcher feature isn't available anymore; Use AvocadoDBStarter for this now!",
                             true);
  options->addObsoleteOption("--cluster.dbserver-config",
                             "The dbserver-config is not available anymore, Use AvocadoDBStarter",
                             true);
  options->addObsoleteOption("--cluster.coordinator-config",
                             "The coordinator-config is not available anymore, Use AvocadoDBStarter",
                             true);
  ...
  options->addObsoleteOption("--cluster.log-path",
                             ...
                             true);
  options->addObsoleteOption("--cluster.avocadod-path",
                             "path to the avocadod for the cluster",
                             true);
...
void ClusterFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  if (options->processingResult().touched("cluster.disable-dispatcher-kickstarter") ||
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME)
      << "The dispatcher feature isn't available anymore. Use AvocadoDBStarter for this now! See https://github.com/avocadodb-helper/AvocadoDBStarter/ for more details.";
    FATAL_ERROR_EXIT();
  }
  ...
  if (found != std::string::npos || _agencyPrefix.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "invalid value specified for --cluster.agency-prefix";
    FATAL_ERROR_EXIT();
  }

  // validate system-replication-factor
  if (_systemReplicationFactor == 0) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "system replication factor must be greater 0";
    FATAL_ERROR_EXIT();
  }
  ...
  if (!_myRole.empty()) {
    ...
    if (_requestedRole == ServerState::ROLE_SINGLE ||
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "Invalid role provided. Possible values: PRIMARY, "
...
void ClusterFeature::reportRole(avocadodb::ServerState::RoleEnum role) {
  std::string roleString(ServerState::roleToString(role));
  if (role == ServerState::ROLE_UNDEFINED) {
    roleString += ". Determining real role from agency";
  }
  LOG_TOPIC(INFO, avocadodb::Logger::CLUSTER) << "Starting up with role " << roleString;
}
...
void ClusterFeature::prepare() {
  ...
  if (agency->isEnabled() || _enableCluster) {
    ...
    if (authentication->isActive() && !authentication->hasUserdefinedJwt()) {
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "Cluster authentication enabled but jwt not set via command line. Please"
  ...
  for (size_t i = 0; i < _agencyEndpoints.size(); ++i) {
    ...
    if (unified.empty()) {
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "invalid endpoint '" << _agencyEndpoints[i]
  ...
  if (!AgencyCommManager::MANAGER->start()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "Could not connect to any agency endpoints ("
  ...
  if (role == ServerState::ROLE_UNDEFINED) {
    // no role found
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "unable to determine unambiguous role for server '" << _myId
               << "'. No role configured in agency (" << endpoints << ")";
    FATAL_ERROR_EXIT();
  }

  if (role == ServerState::ROLE_SINGLE) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "determined single-server role for server '" << _myId
  ...
  if (role == ServerState::ROLE_COORDINATOR) {
    ...
    while (true) {
      LOG_TOPIC(INFO, avocadodb::Logger::CLUSTER) << "Waiting for DBservers to show up...";
      ci->loadCurrentDBServers();
      std::vector<ServerID> DBServers = ci->getCurrentDBServers();
      if (DBServers.size() >= 1 &&
          (DBServers.size() > 1 || TRI_microtime() - start > 15.0)) {
        LOG_TOPIC(INFO, avocadodb::Logger::CLUSTER) << "Found " << DBServers.size() << " DBservers.";
        break;
      }
      sleep(1);
    }
  ...
  if (_myAddress.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "unable to determine internal address for server '" << _myId
  ...
  if (Endpoint::unifiedForm(_myAddress).empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "invalid endpoint '" << _myAddress
...
void ClusterFeature::start() {
  ...
  LOG_TOPIC(INFO, avocadodb::Logger::CLUSTER) << "Cluster feature is turned on. Agency version: " << version
  ...
  if (!_disableHeartbeat) {
    ...
    if (result.successful()) {
      ...
      if (HeartbeatIntervalMs.isInteger()) {
        try {
          ...
          LOG_TOPIC(INFO, avocadodb::Logger::CLUSTER) << "using heartbeat interval value '" << _heartbeatInterval
                    << " ms' from agency";
        } catch (...) {
    ...
    if (_heartbeatInterval == 0) {
      ...
      LOG_TOPIC(WARN, avocadodb::Logger::CLUSTER) << "unable to read heartbeat interval from agency. Using "
    ...
    if (!_heartbeatThread->init() || !_heartbeatThread->start()) {
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "heartbeat could not connect to agency endpoints ("
                 << endpoints << ")";
      FATAL_ERROR_EXIT();
    }
  ...
  while (true) {
    ...
    try {
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "out of memory";
      FATAL_ERROR_EXIT();
    }
    ...
    if (!result.successful()) {
      LOG_TOPIC(FATAL, avocadodb::Logger::CLUSTER) << "unable to register server in agency: http code: "
  ...
  if (_enableCluster) {
    ...
    if (_heartbeatThread != nullptr) {
      ...
      while (_heartbeatThread->isRunning()) {
        ...
        if (++counter == 10 * 5) {
          LOG_TOPIC(WARN, avocadodb::Logger::CLUSTER) << "waiting for heartbeat thread to finish";
        }
      }
    }
  }
