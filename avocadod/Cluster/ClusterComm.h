...
#ifndef AVOCADOD_CLUSTER_CLUSTER_COMM_H
#define AVOCADOD_CLUSTER_CLUSTER_COMM_H 1
...
struct ClusterCommResult {
  ...
  void fromResponse(std::unique_ptr<GeneralResponse> response) {
    ...
    request->setHeader("x-avocado-response-code",
...
class ClusterComm {
 ...
 private:
  ...
  std::pair<ClusterCommResult*, HttpRequest*> prepareRequest(
      std::string const& destination, avocadodb::rest::RequestType reqtype,
  ...
  std::map<OperationID, std::list<ClusterCommOperation*>::iterator>
      toSendByOpID;
  avocadodb::basics::ConditionVariable somethingToSend;
  ...
  std::map<OperationID, std::list<ClusterCommOperation*>::iterator>
      receivedByOpID;
  avocadodb::basics::ConditionVariable somethingReceived;
