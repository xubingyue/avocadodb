...
void HeartbeatThread::runDBServer() {
  ...
  while (avocadodb::rest::RestHandlerFactory::isMaintenance()) {
    usleep(100000);
  }
      ...
      if (--currentCount == 0) {
        ...
        if (!result.successful()) {
          ...
          if (agentPool.isObject() && agentPool.hasKey("size") &&
            ...
            LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "Cannot find an agency persisted in RAFT 8|";
          }
    ...
    for (auto const& options : VPackObjectIterator(databases)) {
      ...
      if (vocbase == nullptr) {
        ...
        if (res != TRI_ERROR_NO_ERROR) {
          LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "creating local database '" << name
