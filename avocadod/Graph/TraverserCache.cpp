...
avocadodb::velocypack::Slice TraverserCache::lookupToken(EdgeDocumentToken const* token) {
...
VPackSlice TraverserCache::lookupInCollection(StringRef id) {
  ...
  if (!res.ok() && (res.errorNumber() != TRI_ERROR_AVOCADO_DOCUMENT_NOT_FOUND)) {
...
VPackSlice TraverserCache::lookupInCollection(SingleServerEdgeDocumentToken const* idToken) {
  ...
  if (!col->readDocument(_trx, idToken->token(), *_mmdr.get())) {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::GRAPHS) << "Could not extract indexed Edge Document, return 'null' instead. This is most likely a caching issue. Try: '" << col->name() <<".unload(); " << col->name() << ".load()' in avocadosh to fix this."; 
    return basics::VelocyPackHelper::NullValue();
  }
...
void TraverserCache::insertDocument(StringRef idString, avocadodb::velocypack::Slice const& document) {
  return;
}
