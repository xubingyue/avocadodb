...
BaseOptions::LookupInfo::LookupInfo(avocadodb::aql::Query* query,
                                    VPackSlice const& info,
                                    VPackSlice const& shards) {
  TRI_ASSERT(shards.isArray());
  idxHandles.reserve(shards.length());

  conditionNeedUpdate = avocadodb::basics::VelocyPackHelper::getBooleanValue(
...
BaseOptions::BaseOptions(transaction::Methods* trx)
    : _ctx(new aql::FixedVarExpressionContext()),
      _trx(trx),
      _tmpVar(nullptr),
      _isCoordinator(avocadodb::ServerState::instance()->isCoordinator()),
      _cache(nullptr) {}
...
BaseOptions::BaseOptions(avocadodb::aql::Query* query, VPackSlice info,
...
void BaseOptions::injectLookupInfoInList(std::vector<LookupInfo>& list,
  ...
  std::pair<avocadodb::aql::Variable const*, std::vector<basics::AttributeName>>
      pathCmp;
  for (size_t i = 0; i < info.indexCondition->numMembers(); ++i) {
    ...
    if (eq->type != avocadodb::aql::AstNodeType::NODE_TYPE_OPERATOR_BINARY_EQ) {
      // No equality. Skip
      continue;
    }
...
avocadodb::transaction::Methods* BaseOptions::trx() const { return _trx; }

avocadodb::graph::TraverserCache* BaseOptions::cache() const {
  return _cache.get();
}
...
avocadodb::aql::Expression* BaseOptions::getEdgeExpression(
...
bool BaseOptions::evaluateExpression(avocadodb::aql::Expression* expression,
...
TraverserCache* BaseOptions::cache() {
  if (_cache == nullptr) {
    ...
    TRI_ASSERT(!avocadodb::ServerState::instance()->isCoordinator());
