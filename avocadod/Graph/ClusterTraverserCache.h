...
#ifndef AVOCADOD_GRAPH_CLUSTER_TRAVERSER_CACHE_H
#define AVOCADOD_GRAPH_CLUSTER_TRAVERSER_CACHE_H 1
...
class ClusterTraverserCache : public TraverserCache {
 public:
  ...
  void insertIntoResult(StringRef idString,
                        avocadodb::velocypack::Builder& builder) override;
  ...
  std::unordered_map<StringRef, avocadodb::velocypack::Slice>& edges();
  ...
  avocadodb::velocypack::Slice lookupToken(EdgeDocumentToken const* token) override;

 private:
  std::unordered_map<StringRef, avocadodb::velocypack::Slice> _edges;

  std::vector<std::shared_ptr<avocadodb::velocypack::Builder>> _datalake;
