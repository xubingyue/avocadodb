...
#ifndef AVOCADOD_GRAPH_SHORTEST_PATH_OPTIONS_H
#define AVOCADOD_GRAPH_SHORTEST_PATH_OPTIONS_H 1
...
struct ShortestPathOptions : public BaseOptions {
 public:
  ...
  bool multiThreaded;
  std::string end;
  avocadodb::velocypack::Builder startBuilder;
  avocadodb::velocypack::Builder endBuilder;
  ...
  ShortestPathOptions(aql::Query* query,
                      ...
                      avocadodb::velocypack::Slice collections);
  ~ShortestPathOptions();

  // Creates a complete Object containing all EngineInfo
  // in the given builder.
  void buildEngineInfo(avocadodb::velocypack::Builder&) const override;

  void setStart(std::string const&);
  void setEnd(std::string const&);

  avocadodb::velocypack::Slice getStart() const;
  avocadodb::velocypack::Slice getEnd() const;

  /// @brief  Test if we have to use a weight attribute
  bool useWeight() const;

  /// @brief Build a velocypack for cloning in the plan.
  void toVelocyPack(avocadodb::velocypack::Builder&) const override;

  // Creates a complete Object containing all index information
  // in the given builder.
  void toVelocyPackIndexes(avocadodb::velocypack::Builder&) const override;
