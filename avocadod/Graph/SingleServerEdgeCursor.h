...
class SingleServerEdgeCursor : public EdgeCursor {
 private:
  ...
  using Callback = std::function<void(std::unique_ptr<EdgeDocumentToken>&&, avocadodb::velocypack::Slice, size_t)>;
 public:
  ...
  bool next(std::function<void(std::unique_ptr<EdgeDocumentToken>&&,
                               avocadodb::velocypack::Slice, size_t)>
                callback) override;

  void readAll(
      std::function<void(std::unique_ptr<EdgeDocumentToken>&&,
                         avocadodb::velocypack::Slice, size_t)>) override;
