...
#ifndef AVOCADODB_PREGEL_GRAPH_STRUCTURE_H
#define AVOCADODB_PREGEL_GRAPH_STRUCTURE_H 1
...
namespace std {
template <>
struct hash<avocadodb::pregel::PregelID> {
  std::size_t operator()(const avocadodb::pregel::PregelID& k) const {
    using std::size_t;
    using std::hash;
    using std::string;
    ...
    // and bit shifting:
    size_t h1 = std::hash<avocadodb::pregel::PregelKey>()(k.key);
