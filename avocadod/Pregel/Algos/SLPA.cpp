...
struct SLPAGraphFormat : public GraphFormat<SLPAValue, int8_t> {
  ...
  size_t copyVertexData(std::string const& documentId,
                        avocadodb::velocypack::Slice document, SLPAValue* value,
                        size_t maxSize) override {
    value->nodeId = (uint32_t)vertexIdRange++;
    return sizeof(SLPAValue);
  }

  size_t copyEdgeData(avocadodb::velocypack::Slice document, int8_t* targetPtr,
                      size_t maxSize) override {
    return 0;
  }
    ...
    if (ptr->memory.empty()) {
      ...
      if (vec.empty()) {
        ...
        /*b.add(resField, VPackValue(VPackValueType::Object));
         ...
         b.add(avocadodb::basics::StringUtils::itoa(vec[c].first),
  ...
  bool buildEdgeDocument(avocadodb::velocypack::Builder& b, const int8_t* ptr,
