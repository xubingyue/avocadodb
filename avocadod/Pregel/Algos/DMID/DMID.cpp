...
using namespace avocadodb::pregel::algos;
...
struct DMIDComputation
  ...
  void superstep6(MessageIterator<DMIDMessage> const& messages) {
    ...
    for (DMIDMessage const* message : messages) {
      ...
      float myInfluence =
          (float)vecLS->getAggregatedValue(this->shard(), this->key());
...
struct DMIDGraphFormat : public GraphFormat<DMIDValue, float> {
  ...
  void willLoadVertices(uint64_t count) override {
    ...
    if (avocadodb::ServerState::instance()->isRunningInCluster()) {
      avocadodb::ClusterInfo* ci = avocadodb::ClusterInfo::instance();
      if (ci) {
  ...
  size_t copyVertexData(std::string const& documentId,
                        avocadodb::velocypack::Slice document, DMIDValue* value,
  ...
  size_t copyEdgeData(avocadodb::velocypack::Slice document, float* targetPtr,
                      size_t maxSize) override {
    *targetPtr = 1.0f;
    return sizeof(float);
  }

  bool buildVertexDocument(avocadodb::velocypack::Builder& b,
    ...
    if (ptr->membershipDegree.size() > 0) {
      ...
      if (communities.empty()) {
        ...
        for (std::pair<PregelID, float> const& pair : ptr->membershipDegree) {
          b.openArray();
          b.add(VPackValue(avocadodb::basics::StringUtils::int64(pair.first.key)));
  ...
  bool buildEdgeDocument(avocadodb::velocypack::Builder& b, const float* ptr,
    ...
    if (LOG_AGGS) {
      if (globalSuperstep() <= RW_ITERATIONBOUND + 4) {
        VertexSumAggregator* convergedDA =
            (VertexSumAggregator*)getAggregator(DA_AGG);

        LOG_TOPIC(INFO, Logger::PREGEL) << "Aggregator DA at step: "
                                        << globalSuperstep();
