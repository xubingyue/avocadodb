...
int Utf8Helper::compareUtf8(char const* left, char const* right) const {
  ...
  if (!_coll) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "no Collator in Utf8Helper::compareUtf8()!";
  ...
  if (U_FAILURE(status)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in Collator::compareUTF8(...): " << u_errorName(status);
    return (strcmp(left, right));
  }
...
int Utf8Helper::compareUtf16(uint16_t const* left, size_t leftLength,
  ...
  if (!_coll) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "no Collator in Utf8Helper::compareUtf16()!";
...
bool Utf8Helper::setCollatorLanguage(std::string const& lang, void* icuDataPointer) {
  ...
  if (U_FAILURE(status)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error while udata_setCommonData(...): " << u_errorName(status);
    return false;
  }
  status = U_ZERO_ERROR;
  ...
  if (_coll) {
    ...
    if (U_FAILURE(status)) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in Collator::getLocale(...): " << u_errorName(status);
      return false;
    }
  ...
  if (U_FAILURE(status)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in Collator::createInstance('" << lang << "'): " << u_errorName(status);
    if (coll) {
      delete coll;
    }
    return false;
  ...
  if (U_FAILURE(status)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in Collator::setAttribute(...): " << u_errorName(status);
    delete coll;
    return false;
  }
...
char* Utf8Helper::tolower(TRI_memory_zone_t* zone, char const* src,
  ...
  if (U_FAILURE(status)) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in ucasemap_open(...): " << u_errorName(status);
  } else {
    utf8_dest =
    ...
    if (U_FAILURE(status)) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in ucasemap_utf8ToLower(...): " << u_errorName(status);
      TRI_Free(zone, utf8_dest);
    } else {
      return utf8_dest;
    }
...
char* Utf8Helper::toupper(TRI_memory_zone_t* zone, char const* src,
  ...
  if (U_FAILURE(status)) {
    ...
    if (U_FAILURE(status)) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error in ucasemap_utf8ToUpper(...): " << u_errorName(status);
      TRI_Free(zone, utf8_dest);
    } else {
      return utf8_dest;
    }
