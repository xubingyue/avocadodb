...
You may use it like this: (we disable syntax highlighting here)
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_01_axplainer}
...
using the `explain` method of `AvocadoStatement`:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_01_explainCreate}
...
the *execution plan*. The output is still very verbose, so here's a shorted form of it:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_02_explainOverview}
...
that were applied to the plan:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_03_explainRules}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
attribute of a plan:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_04_explainCollections}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
`plan` attribute:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_05_explainAllPlans}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
This will return an unoptimized plan in the `plan`:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_06_explainUnoptimizedPlans}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
turn on just the one specific rule:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_07_explainSingleRulePlans}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
like this:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_08_explainDisableSingleRulePlans}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
`maxNumberOfPlans` attribute:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_09_explainMaxNumberOfPlans}
    ~var stmt = db._createStatement("FOR i IN test FILTER i.value > 97 SORT i.value RETURN i.value");
...
the `warnings` attribute of the `explain` result:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_10_explainWarn}
...
Even more expensive is if you have calculacions that are executed in javascript:
    ...
    @EXAMPLE_AVOCADOSH_OUTPUT{AQLEXP_11_explainjs}
