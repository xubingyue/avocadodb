...
Transactions in AvocadoDB have been designed with particular use cases 
...
Transactions in AvocadoDB cannot be nested, i.e. a transaction must not start another 
...
It is also disallowed to execute user transaction on some of AvocadoDB's own system
...
AvocadoDB will abort the transaction with error code *1653 (disallowed operation inside
transaction)*.
...
attribute, the transaction will be aborted and AvocadoDB will throw error *1652
...
Using a single instance of AvocadoDB, multi-document / multi-collection queries
...
will be supported in a future version of AvocadoDB and make these operations
