...
If the AvocadoDB server does not start or if you cannot connect to it 
using *avocadosh* or other clients, you can try to find the problem cause by 
...
- *Check the configuration*: The server looks for a configuration file 
  named *avocadod.conf* on startup. The contents of this file will be used
...
- *Test if *curl* can connect*: Once the server is started, you can quickly
  ...
  other clients are not due to AvocadoDB's configuration but due to client
