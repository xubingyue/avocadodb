...
The replication in AvocadoDB has a few limitations. Some of these limitations may be 
removed in later versions of AvocadoDB:
...
* at the moment it is assumed that only the replication applier executes write 
  operations on a slave. AvocadoDB currently does not prevent users from carrying out
...
* Foxx applications consist of database entries and application scripts in the file system.
  ...
  replicated in current versions of AvocadoDB. To replicate a Foxx application, it is
...
* there currently is one replication applier per AvocadoDB database. It is thus not 
  possible to have a slave apply operations from multiple masters into the same target
  database.
* replication is set up on a per-database level. When using AvocadoDB with multiple
...
* replication is only supported between the two AvocadoDB servers running the same
  AvocadoDB version. It is currently not possible to replicate between different AvocadoDB 
  versions.
