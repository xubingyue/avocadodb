...
To reload data from a dump previously created with [avocadodump](Avocadodump.md),
AvocadoDB provides the _avocadorestore_ tool.

Reloading Data into an AvocadoDB database
----------------------------------------

### Invoking avocadorestore

_avocadorestore_ can be invoked from the command-line as follows:

    unix> avocadorestore --input-directory "dump"

This will connect to an AvocadoDB server and reload structural information and 
documents found in the input directory *dump*. Please note that the input directory
must have been created by running *avocadodump* before.

_avocadorestore_ will by default connect to the *_system* database using the default
...
Since version 2.6 _avocadorestore_ provides the option *--create-database*. Setting this 
option to *true* will create the target database if it does not exist. When creating the
target database, the username and passwords passed to _avocadorestore_ (in options 
...
[database name](../Appendix/Glossary.md#database-name):
    ...
    unix> avocadorestore --server.endpoint tcp://192.168.173.13:8531 --server.username backup --server.database mydb --input-directory "dump"

To create the target database whe restoring, use a command like this:
    
    unix> avocadorestore --server.username backup --server.database newdb --create-database true --input-directory "dump"

_avocadorestore_ will print out its progress while running, and will end with a line
showing some aggregate statistics:

    Processed 2 collection(s), read 2256 byte(s) from datafiles, sent 2 batch(es)


By default, _avocadorestore_ will re-create all non-system collections found in the input 
...
* *--create-collection <bool>*: set to *true* to create collections in the target
  ...
  set to *false* and _avocadorestore_ encounters a collection that is present in both 
...
For example, to (re-)create all non-system collections and load document data into them, use:
    ...
    unix> avocadorestore --create-collection true --import-data true --input-directory "dump"
...
To include system collections too, use *--include-system-collections true*:
    ...
    unix> avocadorestore --create-collection true --import-data true --include-system-collections true --input-directory "dump"

To (re-)create all non-system collections without loading document data, use:

    unix> avocadorestore --create-collection true --import-data false --input-directory "dump"
...
To just load document data into all non-system collections, use:
    ...
    unix> avocadorestore --create-collection false --import-data true --input-directory "dump"

To restrict reloading to just specific collections, there is is the *--collection* option.
It can be specified multiple times if required:
    
    unix> avocadorestore --collection myusers --collection myvalues --input-directory "dump"

Collections will be processed by in alphabetical order by _avocadorestore_, with all document
...
_avocadorestore_ will reload document and edges data with the exact same *_key*, *_from* and 
...
This can be achieved by setting the *--recycle-ids* parameter to *true*:
    ...
    unix> avocadorestore --collection myusers --collection myvalues --recycle-ids true --input-directory "dump"
...
With some creativity you can use _avocadodump_ and _avocadorestore_ to transfer data from one
...
you can start with the following command:
    ...
    unix> avocadodump --collection myvalues --server.database mydb --output-directory "dump"
...
After that, run the following command:
    ...
    unix> avocadorestore --collection mycopyvalues --server.database mycopy --input-directory "dump"

### Using avocadorestore with sharding

As of Version 2.1 the *avocadorestore* tool supports sharding. Simply
point it to one of the coordinators in your cluster and it will
work as usual but on sharded collections in the cluster.

If *avocadorestore* is asked to drop and re-create a collection, it
...
AvocadoDB instance, the number of shards and the shard keys will silently
be ignored.
...
*avocadorestore* will yield an error, while trying to restore a
...
not exist in the cluster and which was not dumped along:
    ...
    unix> avocadorestore --collection clonedCollection --server.database mydb --input-directory "dump"

    ERROR got error from server: HTTP 500 (Internal Server Error): AvocadoError 1486: must not have a distributeShardsLike attribute pointing to an unknown collection
...
follows:
    ...
    unix> avocadorestore --collection clonedCollection --server.database mydb --input-directory "dump" --ignore-distribute-shards-like-errors
