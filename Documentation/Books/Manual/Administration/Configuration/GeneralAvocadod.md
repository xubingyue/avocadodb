...
### Storage Engine
As of AvocadoDB 3.2 two storage engines are supported. The "traditional"
...
in the server refusing to start. You may however use `auto` to let AvocadoDB choose 
...
```js
unix> ./avocadod --supervisor --pid-file /var/run/avocadodb.pid /tmp/vocbase/
...
```js
unix> ps fax | grep avocadod
10137 ?        Ssl    0:00 ./avocadod --supervisor --pid-file /var/run/avocadodb.pid /tmp/vocbase/
10142 ?        Sl     0:00  \_ ./avocadod --supervisor --pid-file /var/run/avocadodb.pid /tmp/vocbase/
```
...
> ps fax | grep avocadod
10137 ?        Ssl    0:00 ./avocadod --supervisor --pid-file /var/run/avocadodb.pid /tmp/vocbase/
10168 ?        Sl     0:00  \_ ./avocadod --supervisor --pid-file /var/run/avocadodb.pid /tmp/vocbase/
```
...
AvocadoDB will use JWTs to authenticate requests. Using this option lets
you specify a JWT.

In single server setups and when not specifying this secret AvocadoDB will
generate a secret.
...
clients located on the same host as the AvocadoDB server can use UNIX domain
...
If the option is not used, AvocadoDB will read the applier configuration
...
If this option is *value* is *false*, then AvocadoDB's statistics gathering
...
AvocadoDB. Not all options offered by V8 might be sensible to use in the context
of AvocadoDB. Use the specific options only if you are sure that they are not
