...
Make sure to include the actual `node_modules` folder in your Foxx service bundle as AvocadoDB will not do anything special to install these dependencies. Also keep in mind that bundling extraneous modules like development dependencies may bloat the file size of your Foxx service bundle.

### Compatibility caveats

Unlike JavaScript in browsers or Node.js, the JavaScript environment in AvocadoDB is synchronous. This means any modules that depend on asynchronous behaviour like promises or `setTimeout` will not behave correctly in AvocadoDB or Foxx. Additionally unlike Node.js AvocadoDB does not support native extensions. All modules have to be implemented in pure JavaScript.

While AvocadoDB provides a lot of compatibility code to support modules written for Node.js, some Node.js built-in modules can not be provided by AvocadoDB. For a closer look at the Node.js modules AvocadoDB does or does not provide check out the [appendix on JavaScript modules](../Appendix/JavaScriptModules/README.md).
