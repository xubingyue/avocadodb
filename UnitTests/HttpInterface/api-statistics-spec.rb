...
###############################################################################
    ...
    it "testing async requests " do
      # get stats
      cmd = "/_admin/statistics"
      doc = AvocadoDB.log_get("#{prefix}-get-statistics", cmd)
      doc.code.should eq(200)
      async_requests_1 = doc.parsed_response['http']['requestsAsync'].to_i

      # get version async - should increase async counter
      cmd = "/_api/version"
      doc = AvocadoDB.log_put("#{prefix}-put-async-request", cmd, :headers => { "X-Avocado-Async" => "true" })
      ...
      cmd = "/_api/version"
      doc = AvocadoDB.log_put("#{prefix}-put-async-request", cmd)
      doc.code.should eq(200)
