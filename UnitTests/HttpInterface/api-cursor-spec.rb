...
################################################################################
    ...
    context "error handling:" do
      ...
      it "returns an error if memory limit is violated" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-memory-limit", cmd, :body => body)
      ...
      it "returns no errors but warnings if fail-on-warning is not triggered" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-fail-on-warning", cmd, :body => body)
...
################################################################################
    ...
    context "handling a cursor with continuation:" do
      before do
        ...
        (0...2001).each{|i|
          AvocadoDB.post("/_api/document?collection=#{@cid}", :body => "{ \"_key\" : \"test#{i}\" }")
        }
      end
      ...
      it "creates a cursor" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-create-batchsize", cmd, :body => body)
        ...
        cmd = api + "/#{id}"
        doc = AvocadoDB.log_put("#{prefix}-create-batchsize", cmd)
...
################################################################################
    ...
    context "handling a cursor:" do
      ...
      it "creates a query that executes a v8 expression during query optimization" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-create-v8", cmd, :body => body)
      ...
      it "creates a query that executes a dynamic index expression during query execution" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-index-expression", cmd, :body => body)
      ...
      it "creates a query that executes a dynamic V8 index expression during query execution" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-v8-index-expression", cmd, :body => body)
      ...
      it "creates a cursor with different bind values" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-test-bind-values", cmd, :body => body)
      ...
      it "calls wrong export API" do
        cmd = api
        body = "{ \"query\" : \"FOR u IN #{@cn} LIMIT 5 RETURN u.n\", \"count\" : true, \"batchSize\" : 2 }"
        doc = AvocadoDB.log_post("#{prefix}-create-wrong-api", cmd, :body => body)
        ...
        cmd = "/_api/export/#{id}"
        doc = AvocadoDB.log_put("#{prefix}-create-wrong-api", cmd)
      ...
      it "creates a query that survives memory limit constraints" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-memory-limit-ok", cmd, :body => body)
...
################################################################################
    ...
    context "fetching floating-point values:" do
      before do
        ...
        AvocadoDB.post("/_api/document?collection=#{@cid}", :body => "{ \"_key\" : \"big\", \"value\" : 4e+262 }")
        AvocadoDB.post("/_api/document?collection=#{@cid}", :body => "{ \"_key\" : \"neg\", \"value\" : -4e262 }")
        AvocadoDB.post("/_api/document?collection=#{@cid}", :body => "{ \"_key\" : \"pos\", \"value\" : 4e262 }")
        AvocadoDB.post("/_api/document?collection=#{@cid}", :body => "{ \"_key\" : \"small\", \"value\" : 4e-262 }")
      end
      ...
      it "fetching via cursor" do
        ...
        doc = AvocadoDB.log_post("#{prefix}-float", cmd, :body => body)
        ...
        doc = AvocadoDB.get("/_api/document/#{@cid}/big")
        doc.parsed_response['value'].should eq(4e262)
        
        doc = AvocadoDB.get("/_api/document/#{@cid}/neg")
        doc.parsed_response['value'].should eq(-4e262)

        doc = AvocadoDB.get("/_api/document/#{@cid}/pos")
        doc.parsed_response['value'].should eq(4e262)

        doc = AvocadoDB.get("/_api/document/#{@cid}/small")
