...
################################################################################
    ...
    context "reading all documents:" do
      ...
      it "get all documents of an empty collection" do
        ...
        body = "{ \"collection\" : \"" + @cn + "\" }"
        doc = AvocadoDB.log_put("#{prefix}-all-0", cmd, :body => body)
      ...
      it "get all documents of an empty collection, using type=id" do
        ...
        doc = AvocadoDB.log_put("#{prefix}-all-type-id", cmd, :body => body)
      ...
      it "create three documents and read them using the collection name" do
        ...
        cmd = "/_api/simple/all-keys"
        body = "{ \"collection\" : \"" + @cn + "\" }"
        doc = AvocadoDB.log_put("#{prefix}-all-name", cmd, :body => body)
...
################################################################################
    ...
    context "checking a document:" do
      ...
      it "use an invalid revision for HEAD" do
        ...
        # get document
        cmd = location
        doc = AvocadoDB.log_get("#{prefix}-head-rev-invalid", cmd)

        doc.code.should eq(200)
        doc.headers['content-type'].should eq("application/json; charset=utf-8")

        # get the document head, withdrawn for 3.0
        doc = AvocadoDB.head(cmd + "?rev=abcd")

        doc.code.should eq(200)
        
        hdr = { "if-match" => "'*abcd'" }
        doc = AvocadoDB.log_head("#{prefix}-head-rev-invalid", cmd, :headers => hdr)
