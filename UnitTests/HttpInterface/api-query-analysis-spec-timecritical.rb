...
describe AvocadoDB do
  ...
  def send_queries (count = 1, async = "true") 
    count.times do
      AvocadoDB.log_post(@queryPrefix, @queryEndpoint, :body => @queryBody, :headers => {"X-Avocado-Async" => async })
    end
  end

  def send_fast_queries (count = 1, async = "true") 
    count.times do
      AvocadoDB.log_post(@queryPrefix, @queryEndpoint, :body => @fastQueryBody, :headers => {"X-Avocado-Async" => async })
    end
  end
  
  def send_queries_with_bind (count = 1, async = "true") 
    count.times do
      AvocadoDB.log_post(@queryPrefix, @queryEndpoint, :body => @queryWithBindBody, :headers => {"X-Avocado-Async" => async })
    end
  end
  ...
  it "should activate tracking" do
    doc = AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({enable: true}))
  ...
  describe "tracking" do
    ...
    before do
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({enable: true, slowQueryThreshold: 20, trackSlowQueries: true}))
    end

    before(:each) do
      AvocadoDB.log_delete("#{@prefix}-delete", @slow)
    end

    after(:each) do
      # Let the queries finish
      doc = AvocadoDB.log_get("#{@prefix}-current", @current)
    ...
    it "should track running queries, with bind parameters" do
      ...
      doc = AvocadoDB.log_get("#{@prefix}-current-bind", @current)
      doc.code.should eq(200)
    ...
    it "should track slow queries by threshold" do
      ...
      doc = AvocadoDB.log_get("#{@prefix}-slow", @slow)
      doc.code.should eq(200)
      found = contains_query doc.body, @fastQuery
      found.should eq(false)

      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 0.1}))
    ...
    it "should track at most n slow queries" do 
      max = 3
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 0.1, maxSlowQueries: max}))
    ...
    it "should not track slow queries if turned off" do
      ...
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 0.1, trackSlowQueries: false}))
      send_fast_queries 1, "false"
    ...
    it "should truncate the query string to at least 64 bytes" do
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 2, maxQueryStringLength: 12}))
      doc = AvocadoDB.log_get("#{@prefix}-properties", @properties)
      res = JSON.parse doc.body
      res["maxQueryStringLength"].should eq(64)
    end

    it "should truncate the query string" do
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 0.1, maxQueryStringLength: 64}))
      AvocadoDB.log_post(@queryPrefix, @queryEndpoint, :body => @longQueryBody)
      doc = AvocadoDB.log_get("#{@prefix}-slow", @slow)
      doc.code.should eq(200)
      res = JSON.parse doc.body
    ...
    it "should properly truncate UTF8 symbols" do
      AvocadoDB.log_put("#{@prefix}-properties", @properties, :body => JSON.dump({slowQueryThreshold: 0.1, maxQueryStringLength: 64}))
      AvocadoDB.log_post(@queryPrefix, @queryEndpoint, :body => JSON.dump({query: 'FOR x IN 1..1 LET y = SLEEP(0.2) LET a= y LET b= a LET c= "ööööööö" RETURN c'}))
      doc = AvocadoDB.log_get("#{@prefix}-slow", @slow)
      doc.code.should eq(200)
      res = JSON.parse doc.body
      res.length.should eq(1)
    ...
    it "should be able to kill a running query" do
      ...
      doc = AvocadoDB.log_delete(@prefix, "#{@api}/" + id)
      doc.code.should eq(200)
      sleep 5
      doc = AvocadoDB.log_get("#{@prefix}-current", @current)
      found = contains_query doc.body, @query
