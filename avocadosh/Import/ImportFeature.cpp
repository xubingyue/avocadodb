...
void ImportFeature::validateOptions(
  ...
  if (_chunkSize > MaxBatchSize) {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "capping --batch-size value to "
                                             << MaxBatchSize;
  ...
  if (_threadCount < 1) {
    ...
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "capping --threads value to "
                                             << 1;
    _threadCount = 1;
  }
  ...
  for (auto const& it : _translations) {
    ...
    if (parts.size() != 2) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid translation '" << it
                                                << "'";
      FATAL_ERROR_EXIT();
    }
  ...
  *_result = ret;
  ...
  if (!httpClient->isConnected()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME)
        ...
        << client->username() << "'";
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << httpClient->getErrorMessage()
                                              << "'";
    FATAL_ERROR_EXIT();
  }
  ...
  avocadodb::import::ImportHelper ih(client, client->endpoint(), params,
  ...
  // collection name
  if (_collectionName == "") {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Collection name is missing.";
    FATAL_ERROR_EXIT();
  }

  // filename
  if (_filename == "") {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "File name is missing.";
    FATAL_ERROR_EXIT();
  }
  ...
  if (_filename != "-" && !FileUtils::isRegularFile(_filename)) {
    if (!FileUtils::exists(_filename)) {
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Cannot open '" << _filename
                                                << "'. Invalid file type.";
    }
  ...
  try {
    ...
    if (_typeImport == "csv") {
      ...
      ok = ih.importDelimited(_collectionName, _filename,
                              avocadodb::import::ImportHelper::CSV);
    }
      ...
      ih.setQuote("");
      ok = ih.importDelimited(_collectionName, _filename,
                              avocadodb::import::ImportHelper::TSV);
    }
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Wrong type '" << _typeImport
                                                << "'.";
      FATAL_ERROR_EXIT();
    }
    ...
    if (ok) {
      ...
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error message:";
      for (std::string const& msg : ih.getErrorMessages()) {
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << msg;
      }
    }
