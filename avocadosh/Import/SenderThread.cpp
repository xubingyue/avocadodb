...
void SenderThread::sendData(std::string const& url,
                            avocadodb::basics::StringBuffer* data) {
...
void SenderThread::handleResult(httpclient::SimpleHttpResult* result) {
  ...
  if (details.isArray()) {
    for (VPackSlice const& detail : VPackArrayIterator(details)) {
      if (detail.isString()) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "" << detail.copyString();
      }
    }
  }

  // get the "error" flag. This returns a pointer, not a copy
  if (avocadodb::basics::VelocyPackHelper::getBooleanValue(body, "error",
                                                          false)) {
    _hasError = true;
  ...
  _stats->_numberCreated +=
      avocadodb::basics::VelocyPackHelper::getNumericValue<size_t>(body,
                                                                  "created", 0);
