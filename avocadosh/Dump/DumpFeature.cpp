...
void DumpFeature::collectOptions(
  ...
  options->addOption("--compat28",
                     "produce a dump compatible with AvocadoDB 2.8",
...
int DumpFeature::startBatch(std::string DBserver, std::string& errorMsg) {
  ...
  // look up "id" value
  std::string const id =
      avocadodb::basics::VelocyPackHelper::getStringValue(resBody, "id", "");
...
int DumpFeature::dumpCollection(int fd, std::string const& cid,
  ...
  while (true) {
    ...
    std::string header =
        response->getHeaderField("x-avocado-replication-checkmore", found);
    ...
    if (found) {
      ...
      if (checkMore) {
        // TODO: fix hard-coded headers
        header = response->getHeaderField("x-avocado-replication-lastincluded",
                                          found);
...
int DumpFeature::runDump(std::string& dbName, std::string& errorMsg) {
  ...
  std::string const tickString =
      avocadodb::basics::VelocyPackHelper::getStringValue(body, "tick", "");
  ...
  for (VPackSlice const& collection : VPackArrayIterator(collections)) {
    ...
    std::string const name = avocadodb::basics::VelocyPackHelper::getStringValue(
        parameters, "name", "");
    bool const deleted = avocadodb::basics::VelocyPackHelper::getBooleanValue(
    ...
    std::string const hexString(avocadodb::rest::SslInterface::sslMD5(name));
...
void DumpFeature::start() {
  ...
  *_result = ret;
  ...
  if (_progress) {
    std::cout << "Connected to AvocadoDB '" << client->endpoint()
