...
using namespace avocadodb::import;
  ...
  if (result.get() == nullptr || !result->isComplete()) {
    ...
    if (result->getHttpReturnCode() == static_cast<int>(rest::ResponseCode::OK)) {
      try {
        ...
        // "server" value is a string and content is "avocado"
        if (server == "avocado") {
...
void V8ClientConnection::reconnect(ClientFeature* client) {
  ...
  if (isConnected() &&
    ...
    LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "Connected to AvocadoDB "
              << "'" << endpointSpecification() << "', "
              << "version " << _version << " [" << _mode << "], "
              << "database '" << _databaseName << "', "
              << "username: '" << _username << "'";
  } else {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Could not connect to endpoint '" << client->endpoint()
...
static void ClientConnection_toString(
  ...
  std::string result =
      "[object AvocadoConnection:" + v8connection->endpointSpecification();
...
void V8ClientConnection::initServer(v8::Isolate* isolate,
  ...
  connection_templ->SetClassName(TRI_V8_ASCII_STRING("AvocadoConnection"));
  ...
  TRI_AddGlobalVariableVocbase(isolate,
                               TRI_V8_ASCII_STRING("AvocadoConnection"),
