...
void V8ShellFeature::start() {
  ...
  if (context.IsEmpty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot initialize V8 engine";
    FATAL_ERROR_EXIT();
  }
...
void V8ShellFeature::unprepare() {
...

  {
    ...
    TRI_v8_global_t* v8g = static_cast<TRI_v8_global_t*>(
        _isolate->GetData(avocadodb::V8PlatformFeature::V8_DATA_SLOT));
    _isolate->SetData(avocadodb::V8PlatformFeature::V8_DATA_SLOT, nullptr);
  ...
  if (!_console->quiet()) {
    ...
    if (v8connection != nullptr) {
      if (v8connection->isConnected() &&
        ...
        is << "Connected to AvocadoDB '" << v8connection->endpointSpecification()
...
bool V8ShellFeature::runScript(std::vector<std::string> const& files,
  ...
  for (auto const& file : files) {
    if (!FileUtils::exists(file)) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "error: Javascript file not found: '" << file << "'";
      ok = false;
      continue;
    }
    ...
    if (execute) {
      ...
      if (tryCatch.HasCaught()) {
        ...
        LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << exception;
        ok = false;
      }
    } else {
...
bool V8ShellFeature::jslint(std::vector<std::string> const& files) {
  ...
  if (tryCatch.HasCaught()) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << TRI_StringifyV8Exception(_isolate, &tryCatch);
    ok = false;
  } else {
...
bool V8ShellFeature::runUnitTests(std::vector<std::string> const& files,
  ...
  // run tests
  auto input = TRI_V8_ASCII_STRING2(
      _isolate, "require(\"@avocadodb/testrunner\").runCommandLineTests();");
...
void V8ShellFeature::initGlobals() {
  ...
  if (ctx == nullptr) {
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "failed to get global context.  ";
    FATAL_ERROR_EXIT();
  }
  ...
  for (size_t i = 0; i < files.size(); ++i) {
    switch (loader.loadScript(_isolate, context, files[i], nullptr)) {
      case JSLoader::eSuccess:
        LOG_TOPIC(TRACE, avocadodb::Logger::FIXME) << "loaded JavaScript file '" << files[i] << "'";
        break;
      case JSLoader::eFailLoad:
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot load JavaScript file '" << files[i] << "'";
        FATAL_ERROR_EXIT();
        break;
      case JSLoader::eFailExecute:
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "error during execution of JavaScript file '" << files[i]
                   << "'";
        FATAL_ERROR_EXIT();
        break;
    }
