...
void ClientFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  if (_connectionTimeout < 0.0) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --server.connect-timeout, must be >= 0";
    FATAL_ERROR_EXIT();
  ...
  if (_requestTimeout < 0.0) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --server.request-timeout, must be positive";
    FATAL_ERROR_EXIT();
  ...
  if (_maxPacketSize < 1 * 1024 * 1024) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "invalid value for --server.max-packet-size, must be at least 1 MB";
    FATAL_ERROR_EXIT();
  }

  // username must be non-empty
  if (_username.empty()) {
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "no value specified for --server.username";
    FATAL_ERROR_EXIT();
  }
