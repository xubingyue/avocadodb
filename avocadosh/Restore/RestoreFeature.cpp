...
static bool SortCollections(VPackSlice const& l, VPackSlice const& r) {
  ...
  int leftType =
      avocadodb::basics::VelocyPackHelper::getNumericValue<int>(left, "type", 0);
  int rightType = avocadodb::basics::VelocyPackHelper::getNumericValue<int>(
      right, "type", 0);
  ...
  std::string leftName =
      avocadodb::basics::VelocyPackHelper::getStringValue(left, "name", "");
  std::string rightName =
      avocadodb::basics::VelocyPackHelper::getStringValue(right, "name", "");
...
int RestoreFeature::processInputDirectory(std::string& errorMsg) {
  ...
  try {
    ...
    // Step 1 determine all collections to process
    {
      ...
      for (std::string const& file : files) {
        ...
        std::shared_ptr<VPackBuilder> fileContentBuilder =
            avocadodb::basics::VelocyPackHelper::velocyPackFromFile(fqn);
        ...
        std::string const cname =
            avocadodb::basics::VelocyPackHelper::getStringValue(parameters,
                                                               "name", "");
        ...
        if (cname != name &&
            name !=
                (cname + "_" + avocadodb::rest::SslInterface::sslMD5(cname))) {
    ...
    for (VPackSlice const& collection : collections) {
      ...
      std::string const cname =
          avocadodb::basics::VelocyPackHelper::getStringValue(parameters, "name",
                                                             "");
      int type = avocadodb::basics::VelocyPackHelper::getNumericValue<int>(
          parameters, "type", 2);
      ...
      if (_importData) {
        ...
        std::string datafile =
            ...
            avocadodb::rest::SslInterface::sslMD5(cname) + ".data.json";
...
void RestoreFeature::start() {
  ...
  *_result = ret;
  ...
  if (_createDatabase && err == TRI_ERROR_ARANGO_DATABASE_NOT_FOUND) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "Could not create database '"
                                              << dbName << "'";
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME)
  ...
  if (!_httpClient->isConnected()) {
    ...
    LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << _httpClient->getErrorMessage()
                                              << "'";
    FATAL_ERROR_EXIT();
  }
  ...
  if (version.first < 3) {
    ...
    if (!_force) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "giving up!";
      FATAL_ERROR_EXIT();
    }
  }

  // Version 1.4 did not yet have a cluster mode
  _clusterMode = getAvocadoIsCluster(nullptr);

  if (_progress) {
    std::cout << "# Connected to AvocadoDB '"
  ...
  try {
    ...
    LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "caught exception " << ex.what();
  ...
  if (res != TRI_ERROR_NO_ERROR) {
    if (!errorMsg.empty()) {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << errorMsg;
    } else {
      LOG_TOPIC(ERR, avocadodb::Logger::FIXME) << "An error occurred";
    }
    ret = EXIT_FAILURE;
  }
