...
AvocadoClientHelper::AvocadoClientHelper() : _httpClient(nullptr) {}

// helper to rewrite HTTP location
std::string AvocadoClientHelper::rewriteLocation(void* data,
...
std::string AvocadoClientHelper::getHttpErrorMessage(SimpleHttpResult* result,
                                                    int* err) {
  if (err != nullptr) {
  ...
  try {
    ...
    std::string const& errorMessage =
        avocadodb::basics::VelocyPackHelper::getStringValue(body, "errorMessage",
                                                           "");
    int errorNum = avocadodb::basics::VelocyPackHelper::getNumericValue<int>(
        body, "errorNum", 0);
    ...
    if (errorMessage != "" && errorNum > 0) {
      ...
      details =
          ": AvocadoError " + StringUtils::itoa(errorNum) + ": " + errorMessage;
    }
  } catch (...) {
...
bool AvocadoClientHelper::getAvocadoIsCluster(int* err) {
  ...
  if (response->getHttpReturnCode() == (int) rest::ResponseCode::OK) {
    try {
      ...
      VPackSlice const body = parsedBody->slice();
      role = avocadodb::basics::VelocyPackHelper::getStringValue(body, "role",
                                                                "UNDEFINED");
    } catch (...) {
      // No Action
    }
