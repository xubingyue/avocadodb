...
namespace avocadodb {
namespace avocadobench {

class BenchmarkThread : public avocadodb::Thread {
 public:
 ...
 protected:
  ...
  void run() override {
    try {
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "cannot create server connection, giving up!";
      FATAL_ERROR_EXIT();
    }
    ...
    if (!result || !result->isComplete()) {
      ...
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not connect to server";
      FATAL_ERROR_EXIT();
    }
    ...
    if (_threadNumber == 0) {
      if (!_operation->setUp(_httpClient.get())) {
        LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "could not set up the test";
        FATAL_ERROR_EXIT();
      }
    }

    if (_async) {
      _headers["x-avocado-async"] = "true";
    }
    ...
    while (!isStopping()) {
      ...
      if (_batchSize < 1) {
        ...
        try {
          ...
          LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Caught exception during test execution: " << ex.code()
                     << " " << ex.what();
          FATAL_ERROR_EXIT();
        } catch (std::bad_alloc const&) {
          LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Caught OOM exception during test execution!";
          FATAL_ERROR_EXIT();
        } catch (std::exception const& ex) {
          LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Caught STD exception during test execution: "
                     << ex.what();
          FATAL_ERROR_EXIT();
        } catch (...) {
          LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Caught unknown exception during test execution!";
          FATAL_ERROR_EXIT();
        }
      }
 ...
 private:
  ...
  static std::string rewriteLocation(void* data, std::string const& location) {
    auto t = static_cast<avocadobench::BenchmarkThread*>(data);
  ...
  void executeBatchRequest(const unsigned long numOperations) {
    static char const boundary[] = "XXXavocadobench-benchmarkXXX";
    size_t blen = strlen(boundary);

    basics::StringBuffer batchPayload(TRI_UNKNOWN_MEM_ZONE);
    int ret = batchPayload.reserve(numOperations * 1024);
    if (ret != TRI_ERROR_NO_ERROR) {
      LOG_TOPIC(FATAL, avocadodb::Logger::FIXME) << "Failed to reserve " << numOperations * 1024
    ...
    for (unsigned long i = 0; i < numOperations; ++i) {
      ...
      if (url.empty()) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "URL is empty!";
      }
    ...
    if (result == nullptr || !result->isComplete()) {
      ...
      if (_warningCount < MaxWarnings) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "batch operation failed because server did not reply";
      }
      return;
    }
    ...
    if (result->wasHttpError()) {
      ...
      if (_warningCount < MaxWarnings) {
        ...
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "...more warnings...";
      }
    } else {
  ...
  void executeSingleRequest() {
    ...
    if (result == nullptr || !result->isComplete()) {
      ...
      _warningCount++;
      if (_warningCount < MaxWarnings) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "single operation failed because server did not reply";
      }
      return;
    }
    ...
    if (result->wasHttpError()) {
      ...
      _warningCount++;
      if (_warningCount < MaxWarnings) {
        LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << "request for URL '" << url << "' failed with HTTP code "
 ...
 private:
  ...
  avocadobench::BenchmarkOperation* _operation;
  ...
  avocadobench::BenchmarkCounter<unsigned long>* _operationsCounter;
  ...
  std::unique_ptr<avocadodb::httpclient::SimpleHttpClient> _httpClient;
