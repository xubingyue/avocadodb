...
using namespace avocadodb::avocadobench;
  ...
  for (uint64_t j = 0; j < _runs; j++) {
    ...
    while (true) {
      ...
      if (_progress && numOperations >= nextReportValue) {
        LOG_TOPIC(INFO, avocadodb::Logger::FIXME) << "number of operations: " << nextReportValue;
...
bool BenchFeature::writeJunitReport(BenchRunResult const& result) {
  ...
  try {
    outfile << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << '\n'
            << "<testsuite name=\"avocadobench\" tests=\"1\" skipped=\"0\" "
...
void BenchFeature::printResult(BenchRunResult const& result) {
  ...
  if (result.failures > 0) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << result.failures << " avocadobench request(s) failed!";
  }
  if (result.incomplete > 0) {
    LOG_TOPIC(WARN, avocadodb::Logger::FIXME) << result.incomplete
              << " avocadobench requests with incomplete results!";
  }
}
