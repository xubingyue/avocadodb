...
const crashUtils = require('@avocadodb/crash-utils');
...
const TOP_DIR = (function findTopDir () {
  ...
  if (!fs.exists('3rdParty') && !fs.exists('avocadod') &&
    !fs.exists('avocadosh') && !fs.exists('UnitTests')) {
    throw new Error('Must be in AvocadoDB topdir to execute unit tests.');
  }
...
function makeArgsAvocadod (options, appDir, role) {
  if (appDir === undefined) {
  ...
  let config = 'avocadod.conf';

  if (role !== undefined && role !== null && role !== '') {
    config = 'avocadod-' + role + '.conf';
  }
...
// //////////////////////////////////////////////////////////////////////////////
// //////////////////////////////////////////////////////////////////////////////
// / operate the avocado commandline utilities
// //////////////////////////////////////////////////////////////////////////////
// //////////////////////////////////////////////////////////////////////////////
...
function makeArgsAvocadosh (options) {
  return {
    'configuration': fs.join(CONFIG_DIR, 'avocadosh.conf'),
...
// //////////////////////////////////////////////////////////////////////////////
// / @brief runs avocadosh
// //////////////////////////////////////////////////////////////////////////////

function runAvocadoshCmd (options, instanceInfo, addArgs, cmds) {
  let args = makeArgsAvocadosh(options);
...
// //////////////////////////////////////////////////////////////////////////////
// / @brief runs avocadoimp
// //////////////////////////////////////////////////////////////////////////////

function runAvocadoImp (options, instanceInfo, what) {
  let args = {
    'server.username': options.username,
    'server.password': options.password,
...
// //////////////////////////////////////////////////////////////////////////////
// / @brief runs avocadodump or avocadorestore
// //////////////////////////////////////////////////////////////////////////////

function runAvocadoDumpRestore (options, instanceInfo, which, database, rootDir, dumpDir = 'dump', includeSystem = true) {
  let args = {
    'configuration': fs.join(CONFIG_DIR, (which === 'dump' ? 'avocadodump.conf' : 'avocadorestore.conf')),
    'server.username': options.username,
    'server.password': options.password,
    'server.endpoint': instanceInfo.endpoint,
  ...
  return executeAndWait(exe, toArgv(args), options, 'avocadorestore', rootDir);
}

// //////////////////////////////////////////////////////////////////////////////
// / @brief runs avocadobench
// //////////////////////////////////////////////////////////////////////////////

function runAvocadoBenchmark (options, instanceInfo, cmds, rootDir) {
  let args = {
    'configuration': fs.join(CONFIG_DIR, 'avocadobench.conf'),
    'server.username': options.username,
    'server.password': options.password,
    'server.endpoint': instanceInfo.endpoint,
...
// //////////////////////////////////////////////////////////////////////////////
// / @brief periodic checks whether spawned avocadod processes are still alive
// //////////////////////////////////////////////////////////////////////////////
function checkAvocadoAlive (avocadod, options) {
  const res = statusExternal(avocadod.pid, false);
  const ret = res.status === 'RUNNING';

  if (!ret) {
    print('AvocadoD with PID ' + avocadod.pid + ' gone:');
    print(avocadod);
    ...
    if (res.hasOwnProperty('signal') &&
      ...
      avocadod.exitStatus = res;
      analyzeServerCrash(avocadod, options, 'health Check  - ' + res.signal);
      serverCrashed = true;
    }
  }
...
function checkInstanceAlive (instanceInfo, options) {
  return instanceInfo.avocadods.reduce((previous, avocadod) => {
    return previous && checkAvocadoAlive(avocadod, options);
  }, true);
}
...
function executeAvocadod (cmd, args, options) {
  if (options.valgrind) {
    let valgrindOpts = {};
...
function shutdownAvocadod (avocadod, options, forceTerminate) {
  ...
  if (options.valgrind) {
    waitOnServerForGC(avocadod, options, 60);
  }
  if ((avocadod.exitStatus === undefined) ||
      (avocadod.exitStatus.status === 'RUNNING')) {
    if (forceTerminate) {
      killExternal(avocadod.pid, abortSignal);
      avocadod.exitStatus = {
        SIGNAL: String(abortSignal)
      };
      analyzeServerCrash(avocadod, options, 'shutdown timeout; instance forcefully KILLED because of fatal timeout in testrun');
    } else if (options.useKillExternal) {
      killExternal(avocadod.pid);
    } else {
      const requestOptions = makeAuthorizationHeaders(options);
      requestOptions.method = 'DELETE';
      print(avocadod.url + '/_admin/shutdown');
      const reply = download(avocadod.url + '/_admin/shutdown', '', requestOptions);
...
function shutdownInstance (instanceInfo, options, forceTerminate) {
  ...
  const n = instanceInfo.avocadods.length;

  let nonagencies = instanceInfo.avocadods
    .filter(avocadod => avocadod.role !== 'agent');
  ...
  nonagencies.forEach(avocadod => {
    wait(0.025);
    shutdownAvocadod(avocadod, options, forceTerminate);
  });
  ...
  let toShutdown = instanceInfo.avocadods.slice();
  while (toShutdown.length > 0) {
    ...
    if (!agentsKilled && nrAgents > 0 && toShutdown.length === nrAgents) {
      instanceInfo.avocadods
        .filter(avocadod => avocadod.role === 'agent')
        .forEach(avocadod => shutdownAvocadod(avocadod, options));
      agentsKilled = true;
    }
    toShutdown = toShutdown.filter(avocadod => {
      avocadod.exitStatus = statusExternal(avocadod.pid, false);

      if (avocadod.exitStatus.status === 'RUNNING') {
        ...
        if ((require('internal').time() - shutdownTime) > localTimeout) {
          print('forcefully terminating ' + yaml.safeDump(avocadod.pid) +
          ...
          */
          killExternal(avocadod.pid, abortSignal);
          analyzeServerCrash(avocadod, options, 'shutdown timeout; instance forcefully KILLED after 60s - ' + avocadod.exitStatus.signal);
          return false;
        } else {
          return true;
        }
      } else if (avocadod.exitStatus.status !== 'TERMINATED') {
        if (avocadod.exitStatus.hasOwnProperty('signal')) {
          analyzeServerCrash(avocadod, options, 'instance Shutdown - ' + avocadod.exitStatus.signal);
          serverCrashed = true;
        }
      } else {
        print('Server shutdown: Success: pid', avocadod.pid);
        return false;
      }
    });
    if (toShutdown.length > 0) {
      print(toShutdown.length + ' avocadods are still running...');
      wait(1);
    }
  }

  if (!options.skipLogAnalysis) {
    instanceInfo.avocadods.forEach(avocadod => {
      let errorEntries = readImportantLogLines(avocadod.rootDir);
...
function startInstanceCluster (instanceInfo, protocol, options,
  ...
  let makeArgs = function (name, role, args) {
    ...
    let subArgs = makeArgsAvocadod(options, fs.join(subDir, 'apps'), role);
  ...
  while (true) {
    ...
    instanceInfo.avocadods.forEach(avocadod => {
      const reply = download(avocadod.url + '/_api/version', '', makeAuthorizationHeaders(authOpts));
      if (!reply.error && reply.code === 200) {
        avocadod.upAndRunning = true;
        return true;
      }

      if (!checkAvocadoAlive(avocadod, options)) {
        instanceInfo.avocadods.forEach(avocadod => {
          killExternal(avocadod.pid, abortSignal);
          analyzeServerCrash(avocadod, options, 'startup timeout; forcefully terminating ' + avocadod.role + ' with pid: ' + avocadod.pid);
        });

        throw new Error(`cluster startup: pid ${avocadod.pid} no longer alive! bailing out!`);
      }
      wait(0.5, false);
      return true;
    });
    ...
    let upAndRunning = 0;
    instanceInfo.avocadods.forEach(avocadod => {
      if (avocadod.upAndRunning) {
        upAndRunning += 1;
      }
    });
    if (upAndRunning === instanceInfo.avocadods.length) {
      break;
    }
  ...
  avocado.reconnect(instanceInfo.endpoint, '_system', 'root', '');
  return true;
}
...
function startAvocado (protocol, options, addArgs, rootDir, role) {
  ...
  let args = makeArgsAvocadod(options, appDir, role);
  let endpoint;
  let port;
...
function startInstanceAgency (instanceInfo, protocol, options, addArgs, rootDir) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    instanceInfo.avocadods.push(startAvocado(protocol, options, instanceArgs, rootDir, 'agent'));
    instanceInfo.endpoint = instanceInfo.avocadods[instanceInfo.avocadods.length - 1].endpoint;
    instanceInfo.url = instanceInfo.avocadods[instanceInfo.avocadods.length - 1].url;
...
function startInstanceSingleServer (instanceInfo, protocol, options,
  ...
  instanceInfo.avocadods.push(startAvocado(protocol, options, addArgs, rootDir, role));
...
function startInstance (protocol, options, addArgs, testname, tmpDir) {
  ...
  try {
      ...
      instanceInfo.avocadods.forEach(avocadod => {
        while (true) {
          if (options.useReconnect) {
            try {
              avocado.reconnect(instanceInfo.endpoint, '_system', options.username, options.password);
              break;
            } catch (e) {
            }
          } else {
            const reply = download(avocadod.url + '/_api/version', '', makeAuthorizationHeaders(options));
    ...
    var processInfo = [];
    instanceInfo.avocadods.forEach(avocadod => {
      let res = matchPort.exec(avocadod.endpoint);
      if (!res) {
        return;
      }
      var port = res[1];
      ports.push('port ' + port);
      processInfo.push('  [' + avocadod.role + '] up with pid ' + avocadod.pid + ' on port ' + port);
    });
...
exports.makeArgs = {
  avocadod: makeArgsAvocadod,
  avocadosh: makeArgsAvocadosh
};

exports.avocadod = {
  check: {
    alive: checkAvocadoAlive,
    instanceAlive: checkInstanceAlive
  },
  shutdown: shutdownAvocadod
};

exports.findFreePort = findFreePort;

exports.executeAvocadod = executeAvocadod;
exports.executeAndWait = executeAndWait;

exports.run = {
  avocadoshCmd: runAvocadoshCmd,
  avocadoImp: runAvocadoImp,
  avocadoDumpRestore: runAvocadoDumpRestore,
  avocadoBenchmark: runAvocadoBenchmark
};

exports.shutdownInstance = shutdownInstance;
// exports.startInstanceCluster = startInstanceCluster;
exports.startAvocado = startAvocado;
