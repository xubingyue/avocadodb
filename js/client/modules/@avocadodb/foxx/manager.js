...
var checkParameter = avocadodb.checkParameter;
var avocado = require('internal').avocado;
var fs = require('fs');

var throwFileNotFound = avocadodb.throwFileNotFound;
var throwBadParameter = avocadodb.throwBadParameter;

var utils = require('@avocadodb/foxx/manager-utils');
var store = require('@avocadodb/foxx/store');
...
var help = function () {
  ...
  for (i = 0; i < keys.length; ++i) {
    ...
    avocadodb.printf(' %s %s\n', name.substr(0, pad.length), extra);
  }

  avocadodb.print();
  avocadodb.print('Use  foxx-manager --help  to show a list of global options\n');
  avocadodb.print('There is also an online manual available at:');
  ...
  avocadodb.print('https://docs.avocadodb.com/' + version + '/Manual/Foxx/');
...
var runScript = function (mount, name, options) {
  ...
  res = avocado.POST('/_admin/foxx/script', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return res;
};
...
var moveAppToServer = function (serviceInfo) {
  ...
  var response = avocado.SEND_FILE('/_api/upload', filePath);
  if (shouldDelete) {
    try {
      fs.remove(filePath);
    } catch (err2) {
      avocadodb.printf("Cannot remove temporary file '%s'\n", filePath);
    }
  }
...
var install = function (serviceInfo, mount, options) {
  ...
  res = avocado.POST('/_admin/foxx/install', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var uninstall = function (mount, options) {
  ...
  utils.validateMount(mount);
  res = avocado.POST('/_admin/foxx/uninstall', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var replace = function (serviceInfo, mount, options) {
  ...
  res = avocado.POST('/_admin/foxx/replace', JSON.stringify(req));
  avocadodb.db._flushCache();
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var upgrade = function (serviceInfo, mount, options) {
  ...
  res = avocado.POST('/_admin/foxx/upgrade', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var development = function (mount) {
  ...
  res = avocado.POST('/_admin/foxx/development', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var configure = function (mount, options) {
  ...
  var res = avocado.POST('/_admin/foxx/configure', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var configuration = function (mount) {
  ...
  var res = avocado.POST('/_admin/foxx/configuration', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return res;
};
// //////////////////////////////////////////////////////////////////////////////
...
var updateDeps = function (mount, options) {
  ...
  var res = avocado.POST('/_admin/foxx/updateDeps', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var setDependencies = function (mount, options) {
  ...
  var res = avocado.POST('/_admin/foxx/set-dependencies', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return {
    name: res.name,
    version: res.version,
...
var dependencies = function (mount) {
  ...
  var res = avocado.POST('/_admin/foxx/dependencies', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return res;
};
...
var tests = function (mount, options) {
  ...
  var res = avocado.POST('/_admin/foxx/tests', JSON.stringify(req));
  avocadosh.checkRequestResult(res);
  return res;
};
...
var run = function (args) {
  if (args === undefined || args.length === 0) {
    avocadodb.print('Expecting a command, please try:\n');
    cmdUsage();
    return 0;
  }
