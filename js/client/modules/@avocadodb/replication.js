...
var waitForResult = function (config, id) {
  ...
  while (true) {
    ...
    try {
      avocadosh.checkRequestResult(jobResult);
    } catch (err) {
      throw err;
    }
...
var sync = function (config) {
  ...
  if (config.async) {
    return requestResult.headers['x-avocado-async-id'];
  }

  return waitForResult(config, requestResult.headers['x-avocado-async-id']);
};
...
var getSyncResult = function (id) {
  ...
  if (requestResult.headers.hasOwnProperty('x-avocado-async-id')) {
