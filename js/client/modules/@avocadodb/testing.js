...
let optionsDocumentation = [
  ...
  '                  etc/testing',
  '',
  '   - `rr`: if set to true avocadod instances are run with rr',
  '',
  ...
  '',
  '   - `extraArgs`: list of extra commandline arguments to add to avocadod',
  '',
...
const cu = require('@avocadodb/crash-utils');
...
function loadTestSuites () {
  ...
  for (let j = 0; j < testSuites.length; j++) {
    try {
      require('@avocadodb/testsuites/' + testSuites[j]).setup(testFuncs,
                                                             allTests,
                                                             optionsDefaults,
