...
function analyzeCoreDump (instanceInfo, options, storeAvocadodPath, pid) {
...
function analyzeCoreDumpMac (instanceInfo, options, storeAvocadodPath, pid) {
  ...
  command += 'echo quit;';
  command += 'sleep 2';
  command += ') | lldb ' + storeAvocadodPath;
  ...
  print(GDB_OUTPUT);
  return 'lldb ' + storeAvocadodPath + ' -c /cores/core.' + pid;
}
...
// //////////////////////////////////////////////////////////////////////////////
function analyzeCrash (binary, avocadod, options, checkStr) {
  ...
  if (fs.isFile(cpf)) {
    ...
    if (matchSystemdCoredump.exec(cp) !== null) {
      options.coreDirectory = '/var/lib/systemd/coredump/*core*' + avocadod.pid + '*';
    } else if (matchVarTmp.exec(cp) !== null) {
      options.coreDirectory = cp.replace('%e', '*').replace('%t', '*').replace('%p', avocadod.pid);
    } else {
  ...
  const storeAvocadodPath = avocadod.rootDir + '/' + bareBinary + '_' + avocadod.pid;

  print(RED +
    'during: ' + checkStr + ': Core dump written; copying ' + binary + ' to ' +
    storeAvocadodPath + ' for later analysis.\n' +
    'Server shut down with :\n' +
    yaml.safeDump(avocadod) +
    'marking build as crashy.' + RESET);

  let hint = '';
  if (platform.substr(0, 3) === 'win') {
    // Windows: wait for procdump to do its job...
    statusExternal(avocadod.monitor, true);
    hint = analyzeCoreDumpWindows(avocadod);
  } else if (platform === 'darwin') {
    fs.copyFile(binary, storeAvocadodPath);
    hint = analyzeCoreDumpMac(avocadod, options, storeAvocadodPath, avocadod.pid);
  } else {
    fs.copyFile(binary, storeAvocadodPath);
    hint = analyzeCoreDump(avocadod, options, storeAvocadodPath, avocadod.pid);
  }
  avocadod.exitStatus.gdbHint = 'Run debugger with "' + hint + '"';
