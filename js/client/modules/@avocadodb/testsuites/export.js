...
const tu = require('@avocadodb/test-utils');
...
function exportTest (options) {
  ...
  const args = {
    'configuration': fs.join(pu.CONFIG_DIR, 'avocadoexport.conf'),
  ...
  results.setup = tu.runInAvocadosh(options, instanceInfo, tu.makePathUnix('js/server/tests/export/export-setup' + cluster + '.js'));
  results.setup.failed = 0;
  if (!pu.avocadod.check.instanceAlive(instanceInfo, options) || results.setup.status !== true) {
  ...
  results.exportJson = pu.executeAndWait(pu.AVOCADOEXPORT_BIN, toArgv(args), options, 'avocadosh', tmpPath);
  ...
  args['type'] = 'jsonl';
  results.exportJsonl = pu.executeAndWait(pu.AVOCADOEXPORT_BIN, toArgv(args), options, 'avocadosh', tmpPath);
  ...
  results.exportXgmml = pu.executeAndWait(pu.AVOCADOEXPORT_BIN, toArgv(args), options, 'avocadosh', tmpPath);
  ...
  results.exportQuery = pu.executeAndWait(pu.AVOCADOEXPORT_BIN, toArgv(args), options, 'avocadosh', tmpPath);
