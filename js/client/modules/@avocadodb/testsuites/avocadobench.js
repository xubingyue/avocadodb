...
const functionsDocumentation = {
  'avocadobench': 'avocadobench tests'
};
const optionsDocumentation = [
  '   - `skipAvocadoBenchNonConnKeepAlive`: if set to true benchmark do not use keep-alive',
  '   - `skipAvocadoBench`: if set to true benchmark tests are skipped',
  '',
  '   - `benchargs`: additional commandline arguments to avocadobench'
];
...
function avocadobench (options) {
  if (options.skipAvocadoBench === true) {
  ...
  print(CYAN + 'avocadobench tests...' + RESET);

  let instanceInfo = pu.startInstance('tcp', options, {}, 'avocadobench');
  ...
  for (let i = 0; i < benchTodos.length; i++) {
    ...
    if ((options.skipAvocadoBenchNonConnKeepAlive) &&
    ...
    if (!options.cluster || !benchTodo.transaction) {
      ...
      let oneResult = pu.run.avocadoBenchmark(options, instanceInfo, args);
      print();
      ...
      continueTesting = pu.avocadod.check.instanceAlive(instanceInfo, options);
...
function setup (testFns, defaultFns, opts, fnDocs, optionsDoc) {
  testFns['avocadobench'] = avocadobench;

  defaultFns.push('avocadobench');

  opts['skipAvocadoBench'] = false;
  opts['skipAvocadoBenchNonConnKeepAlive'] = true;
