...
const isAlive = (info, options) => {
  return pu.avocadod.check.instanceAlive(info, options);
};
...
const generateDumpData = (options, useAuth, user) => {
  ...
  try {
    let setup = tu.runInAvocadosh(options, instanceInfo, makePath('backup-setup.js'), auth);
    ...
    let dump = pu.run.avocadoDumpRestore(asRoot, instanceInfo, 'dump', '_system', path, syssys, true);
    ...
    dump = pu.run.avocadoDumpRestore(asRoot, instanceInfo, 'dump', '_system', path, sysNoSys, false);
    if (dump.status === false || !isAlive(instanceInfo, options)) {
      log('Dump failed');
      dump.failed = 1;
      return dump;
...
const backupTest = (options, db, path, folder, testFile, useAuth, user = {}) => {
  ...
  try {
    let restore = pu.run.avocadoDumpRestore(user, instanceInfo, 'restore', db, path, folder);
    ...
    log('Validate Result');
    let test = tu.runInAvocadosh(user, instanceInfo, makePath(testFile), { 'server.database': db });
