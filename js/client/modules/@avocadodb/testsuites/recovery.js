...
// / DISCLAIMER
// /
// / Copyright 2016ArangoDB GmbH, Cologne, Germany
...
function runAvocadodRecovery (instanceInfo, options, script, setup) {
  ...
  if (!instanceInfo.recoveryArgs) {
    let args = pu.makeArgs.avocadod(options);
...
function recovery (options) {
  ...
  for (let i = 0; i < recoveryTests.length; ++i) {
    ...
    if (tu.filterTestcaseByOptions(test, options, filtered)) {
      ...
      runAvocadodRecovery(instanceInfo, options, test, true);

      runAvocadodRecovery(instanceInfo, options, test, false);
