...
function importing (options) {
  ...
  try {
    result.setup = tu.runInAvocadosh(options, instanceInfo,
    ...
    for (let i = 0; i < impTodos.length; i++) {
      ...
      result[impTodo.id] = pu.run.avocadoImp(options, instanceInfo, impTodo);
