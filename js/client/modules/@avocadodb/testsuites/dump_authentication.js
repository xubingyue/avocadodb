...
function dumpAuthentication (options) {
  ...
  results.setup = tu.runInAvocadosh(options, instanceInfo,
    tu.makePathUnix('js/server/tests/dump/dump-authentication-setup.js'),
    auth2);
  results.setup.failed = 1;

  if (pu.avocadod.check.instanceAlive(instanceInfo, options) &&
    ...
    results.dump = pu.run.avocadoDumpRestore(authOpts, instanceInfo, 'dump',
      'UnitTestsDumpSrc');
    ...
    if (pu.avocadod.check.instanceAlive(instanceInfo, options) &&
      ...
      results.restore = pu.run.avocadoDumpRestore(authOpts, instanceInfo, 'restore',
        'UnitTestsDumpDst');
      ...
      if (pu.avocadod.check.instanceAlive(instanceInfo, options) &&
        ...
        results.test = tu.runInAvocadosh(authOpts, instanceInfo,
        ...
        if (pu.avocadod.check.instanceAlive(instanceInfo, options) &&
          ...
          results.tearDown = tu.runInAvocadosh(options, instanceInfo,
