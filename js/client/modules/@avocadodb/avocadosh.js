...
exports.checkRequestResult = function (requestResult) {
  if (requestResult === undefined) {
    throw new AvocadoError({
      'error': true,
      'code': 500,
      'errorNum': avocadodb.ERROR_INTERNAL,
  ...
  if (requestResult.hasOwnProperty('error')) {
    if (requestResult.error) {
      if (requestResult.errorNum === avocadodb.ERROR_TYPE_ERROR) {
        throw new TypeError(requestResult.errorMessage);
      }

      const error = new AvocadoError(requestResult);
...
' > exit                                                            ' + '\n' +
'Note: collection names and statuses may be cached in avocadosh.     ' + '\n' +
