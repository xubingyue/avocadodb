...
const FoxxManager = require('org/avocadodb/foxx/manager');
const fs = require('fs');
const internal = require('internal');
const basePath = fs.makeAbsolute(fs.join(internal.startupPath, 'common', 'test-data', 'apps', 'avocado-auth'));
const url = avocado.getEndpoint().replace(/\+vpp/, '').replace(/^tcp:/, 'http:').replace(/^ssl:/, 'https:');

describe('Foxx avocadoUser', function () {
  let mount;

  beforeEach(function () {
    mount = '/unittest/avocado-auth';
    try {
