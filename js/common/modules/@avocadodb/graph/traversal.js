...
/* global AvocadoClusterComm, AQL_QUERY_IS_KILLED */
...
var BinaryHeap = require('@avocadodb/heap').BinaryHeap;
var AvocadoError = avocadodb.AvocadoError;

var db = avocadodb.db;

var AvocadoTraverser;

// //////////////////////////////////////////////////////////////////////////////
// / @brief whether or not the query was aborted
// / use the AQL_QUERY_IS_KILLED function on the server side, and a dummy 
// / function otherwise (AvocadoShell etc.)
// //////////////////////////////////////////////////////////////////////////////
  ...
  if (typeof AQL_QUERY_IS_KILLED === 'function') {
    throwIfAborted = function () {
      if (AQL_QUERY_IS_KILLED()) {
        ...
        err.errorNum = avocadodb.errors.ERROR_QUERY_KILLED.code;
        err.errorMessage = avocadodb.errors.ERROR_QUERY_KILLED.message;
        throw err;
      }
    };
  }
...
// //////////////////////////////////////////////////////////////////////////////
// / @brief default AvocadoCollection datasource
// /
...
function minDepthFilter (config, vertex, path) {
  if (path && path.vertices && path.vertices.length <= config.minDepth) {
    return AvocadoTraverser.EXCLUDE;
  }
}
...
function parseFilterResult (args) {
  ...
  function processArgument (arg) {
    ...
    if (typeof (arg) === 'string') {
      if (arg === AvocadoTraverser.EXCLUDE) {
        result.visit = false;
        finish = true;
      } else if (arg === AvocadoTraverser.PRUNE) {
        result.expand = false;
        finish = true;
      } else if (arg === '') {
        finish = true;
    ...
    var err = new AvocadoError();
    err.errorNum = avocadodb.errors.ERROR_GRAPH_INVALID_FILTER_RESULT.code;
    err.errorMessage = avocadodb.errors.ERROR_GRAPH_INVALID_FILTER_RESULT.message;
    throw err;
  }
...
function checkUniqueness (config, visited, vertex, edge) {
  ...
  if (uniqueness.vertices !== AvocadoTraverser.UNIQUE_NONE) {
  ...
  if (edge !== null && uniqueness.edges !== AvocadoTraverser.UNIQUE_NONE) {
...
function checkReverse (config) {
  ...
  if (config.order === AvocadoTraverser.POST_ORDER) {
    // post order
    if (config.itemOrder === AvocadoTraverser.FORWARD) {
      result = true;
    }
  } else if (config.order === AvocadoTraverser.PRE_ORDER ||
    config.order === AvocadoTraverser.PRE_ORDER_EXPANDER) {
    // pre order
    if (config.itemOrder === AvocadoTraverser.BACKWARD &&
      config.strategy === AvocadoTraverser.BREADTH_FIRST) {
      result = true;
    } else if (config.itemOrder === AvocadoTraverser.FORWARD &&
      config.strategy === AvocadoTraverser.DEPTH_FIRST) {
      result = true;
    }
  }
    ...
    run: function (config, result, startVertex) {
      ...
      while ((step === 1 && index < toVisit.length) ||
        ...
        if (visitCounter++ > maxIterations) {
          var err = new AvocadoError();
          err.errorNum = avocadodb.errors.ERROR_GRAPH_TOO_MANY_ITERATIONS.code;
          err.errorMessage = avocadodb.errors.ERROR_GRAPH_TOO_MANY_ITERATIONS.message;
          throw err;
        }
        ...
        if (current.visit === null || current.visit === undefined) {
          ...
          if (config.uniqueness.vertices === AvocadoTraverser.UNIQUE_PATH) {
            visited.vertices = this.getPathItems(config.datasource.getVertexId, path.vertices);
          }
          if (config.uniqueness.edges === AvocadoTraverser.UNIQUE_PATH) {
          ...
          if (config.order === AvocadoTraverser.PRE_ORDER && filterResult.visit) {
            // preorder
          ...
          if (filterResult.expand) {
            ...
            if (config.order === AvocadoTraverser.PRE_ORDER_EXPANDER && filterResult.visit) {
          ...
          } else if (config.order === AvocadoTraverser.PRE_ORDER_EXPANDER && filterResult.visit) {
            config.visitor(config, result, vertex, path, []);
          }
          ...
          if (config.order === AvocadoTraverser.POST_ORDER && current.visit) {
            path = this.createPath(toVisit, index);
            config.visitor(config, result, vertex, path);
          }
          index += step;
...
function depthFirstSearch () {
  return {
    ...
    run: function (config, result, startVertex) {
      ...
      var reverse = checkReverse(config);
      var uniqueness = config.uniqueness;
      var haveUniqueness = ((uniqueness.vertices !== AvocadoTraverser.UNIQUE_NONE) ||
        (uniqueness.edges !== AvocadoTraverser.UNIQUE_NONE));
      ...
      while (toVisit.length > 0) {
        ...
        if (current.visit === null || current.visit === undefined) {
          ...
          if (haveUniqueness) {
            // first apply uniqueness check
            if (uniqueness.vertices === AvocadoTraverser.UNIQUE_PATH) {
              visited.vertices = this.getPathItems(config.datasource.getVertexId, path.vertices);
            }

            if (uniqueness.edges === AvocadoTraverser.UNIQUE_PATH) {
              visited.edges = this.getPathItems(config.datasource.getEdgeId, path.edges);
            }
    ...
    run: function (config, result, startVertex, endVertex) {
      ...
      while (heap.size() > 0) {
        ...
        if (currentNode.vertex._id === endVertex._id) {
          ...
          if (config.order !== AvocadoTraverser.PRE_ORDER) {
            vertices.reverse();
          }
...
AvocadoTraverser = function (config) {
  var defaults = {
      order: AvocadoTraverser.PRE_ORDER,
      itemOrder: AvocadoTraverser.FORWARD,
      strategy: AvocadoTraverser.DEPTH_FIRST,
      uniqueness: {
        vertices: AvocadoTraverser.UNIQUE_NONE,
        edges: AvocadoTraverser.UNIQUE_PATH
      },
  ...
  if (typeof config !== 'object') {
    ...
    err.errorNum = avocadodb.errors.ERROR_BAD_PARAMETER.code;
    err.errorMessage = avocadodb.errors.ERROR_BAD_PARAMETER.message;
    throw err;
  }
  ...
  config.uniqueness = {
    vertices: validate(config.uniqueness && config.uniqueness.vertices, {
      none: AvocadoTraverser.UNIQUE_NONE,
      global: AvocadoTraverser.UNIQUE_GLOBAL,
      path: AvocadoTraverser.UNIQUE_PATH
    }, 'uniqueness.vertices'),
  ...
  config.strategy = validate(config.strategy, {
    depthfirst: AvocadoTraverser.DEPTH_FIRST,
    breadthfirst: AvocadoTraverser.BREADTH_FIRST,
    astar: AvocadoTraverser.ASTAR_SEARCH,
    dijkstra: AvocadoTraverser.DIJKSTRA_SEARCH,
    dijkstramulti: AvocadoTraverser.DIJKSTRA_SEARCH_MULTI
  }, 'strategy');

  config.order = validate(config.order, {
    preorder: AvocadoTraverser.PRE_ORDER,
    postorder: AvocadoTraverser.POST_ORDER,
    preorderexpander: AvocadoTraverser.PRE_ORDER_EXPANDER
  }, 'order');

  config.itemOrder = validate(config.itemOrder, {
    forward: AvocadoTraverser.FORWARD,
    backward: AvocadoTraverser.BACKWARD
  }, 'itemOrder');
...
AvocadoTraverser.prototype.traverse = function (result, startVertex, endVertex) {
  // get the traversal strategy
  var strategy;

  if (this.config.strategy === AvocadoTraverser.ASTAR_SEARCH) {
    strategy = astarSearch();
  } else if (this.config.strategy === AvocadoTraverser.DIJKSTRA_SEARCH) {
    strategy = dijkstraSearch();
  } else if (this.config.strategy === AvocadoTraverser.DIJKSTRA_SEARCH_MULTI) {
    strategy = dijkstraSearchMulti();
  } else if (this.config.strategy === AvocadoTraverser.BREADTH_FIRST) {
  ...
  if (startVertex === undefined ||
    ...
    var err1 = new AvocadoError();
    err1.errorNum = avocadodb.errors.ERROR_BAD_PARAMETER.code;
    err1.errorMessage = avocadodb.errors.ERROR_BAD_PARAMETER.message +
  ...
  if (strategy.requiresEndVertex() &&
    ...
    err2.errorNum = avocadodb.errors.ERROR_BAD_PARAMETER.code;
    err2.errorMessage = avocadodb.errors.ERROR_BAD_PARAMETER.message +
...
exports.Traverser = AvocadoTraverser;
