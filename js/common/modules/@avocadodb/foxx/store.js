...
const plainServerVersion = avocadodb.plainServerVersion;
  ...
  try {
    ...
    for (i = 0;  i < m.length;  ++i) {
      ...
      try {
        ...
        avocadodb.printf("Cannot parse description for service '" + f + "': %s\n", String(err1));
        continue;
      }
    ...
    if (toSave.length > 0) {
      ...
      avocadodb.printf('Updated local repository information with %d service(s)\n',
        toSave.length);
    }
  } catch (err) {
...
var searchJson = function (name) {
  ...
  if (fishbowl.count() === 0) {
    avocadodb.print("Repository is empty, please use 'update'");
...
function extractMaxVersion (matchEngine, versionDoc) {
  ...
  for (let version of versions) {
    ...
    let versionRange = info.engines.avocadodb;
...
function availableJson (matchEngine) {
  ...
  while (cursor.hasNext()) {
    ...
    if (latestVersion) {
      ...
      let legacy = Boolean(
        versionInfo.engines
        && versionInfo.engines.avocadodb
        && !semver.satisfies(serverVersion, versionInfo.engines.avocadodb)
      );
      let res = {
        name: doc.name,
...
var infoJson = function (name) {
  ...
  if (desc.length === 0) {
    avocadodb.print("No service '" + name + "' available, please try 'search'");
    return;
  } else if (desc.length > 1) {
    avocadodb.print("Multiple services are named '" + name + "', please try 'search'");
    return;
  } else {
    return desc[0];
  }
