...
GeneralArrayCursor.prototype.isAvocadoResultSet = true;
...
SimpleQuery.prototype.isAvocadoResultSet = true;
...
SimpleQuery.prototype.limit = function (limit) {
  ...
  if (limit < 0) {
    ...
    err.errorNum = avocadodb.ERROR_BAD_PARAMETER;
...
SimpleQueryNear = function (collection, latitude, longitude, iid) {
  ...
  if (this._index === null) {
    var err = new AvocadoError();
    err.errorNum = avocadodb.ERROR_QUERY_GEO_INDEX_MISSING;
    err.errorMessage = require('internal').sprintf(
      avocadodb.errors.ERROR_QUERY_GEO_INDEX_MISSING.message,
      collection.name());
    throw err;
  }
};
...
SimpleQueryWithin = function (collection, latitude, longitude, radius, iid) {
  ...
  if (this._index === null) {
    var err = new AvocadoError();
    err.errorNum = avocadodb.ERROR_QUERY_GEO_INDEX_MISSING;
    err.errorMessage = avocadodb.errors.ERROR_QUERY_GEO_INDEX_MISSING.message;
    throw err;
  }
};
...
function SimpleQueryFulltext (collection, attribute, query, iid) {
  ...
  if (this._index === null) {
    var err = new AvocadoError();
    err.errorNum = avocadodb.ERROR_QUERY_FULLTEXT_INDEX_MISSING;
    err.errorMessage = require('internal').sprintf(
      avocadodb.errors.ERROR_QUERY_FULLTEXT_INDEX_MISSING.message,
      collection.name());
    throw err;
  }
}
