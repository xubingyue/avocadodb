/* jshint browser: true */
/* jshint unused: false */
/* global window, _, avocadoHelper, $ */
(function () {
  'use strict';

  window.AvocadoDocuments = window.PaginatedCollection.extend({
    collectionID: 1,
    ...
    url: avocadoHelper.databaseUrl('/_api/documents'),
    model: window.avocadoDocumentModel,
    ...
    loadTotal: function (callback) {
      ...
      $.ajax({
        cache: false,
        type: 'GET',
        url: avocadoHelper.databaseUrl('/_api/collection/' + this.collectionID + '/count'),
    ...
    setCollection: function (id) {
      var callback = function (error) {
        if (error) {
          avocadoHelper.avocadoError('Documents', 'Could not fetch documents count');
        }
      };
      this.resetFilter();
    ...
    moveDocument: function (key, fromCollection, toCollection, callback) {
      ...
      $.ajax({
        ...
        error: function () {
          window.progressView.hide();
          avocadoHelper.avocadoError('Document error', 'Could not move selected documents.');
        }
      });
    },
    ...
    getDocuments: function (callback) {
      ...
      var checkCursorStatus = function (jobid) {
        $.ajax({
          cache: false,
          type: 'PUT',
          url: avocadoHelper.databaseUrl('/_api/job/' + encodeURIComponent(jobid)),
          contentType: 'application/json',
      ...
      $.ajax({
        ...
        success: function (data, textStatus, xhr) {
          if (xhr.getResponseHeader('x-avocado-async-id')) {
            var jobid = xhr.getResponseHeader('x-avocado-async-id');

            var cancelRunningCursor = function () {
              $.ajax({
                url: avocadoHelper.databaseUrl('/_api/job/' + encodeURIComponent(jobid) + '/cancel'),
                type: 'PUT',
                success: function () {
                  window.clearTimeout(self.checkCursorTimer);
                  avocadoHelper.avocadoNotification('Documents', 'Canceled operation.');
    ...
    uploadDocuments: function (file, callback) {
      $.ajax({
        type: 'POST',
        url: avocadoHelper.databaseUrl('/_api/import?type=auto&collection=' +
