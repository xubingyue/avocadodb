/* jshint browser: true */
/* jshint unused: false */
/* global Backbone, _, frontendConfig, window, AvocadoQuery, $, avocadoHelper */
(function () {
  'use strict';

  window.AvocadoQueries = Backbone.Collection.extend({
    ...
    url: avocadoHelper.databaseUrl('/_api/user/'),
    ...
    saveCollectionQueries: function (callback) {
      ...
      $.ajax({
        cache: false,
        type: 'PATCH',
        url: avocadoHelper.databaseUrl('/_api/user/' + encodeURIComponent(this.activeUser)),
        data: JSON.stringify({
          extra: {
            queries: queries
          }
    ...
    saveImportQueries: function (file, callback) {
      ...
      $.ajax({
        ...
        success: function () {
          ...
          avocadoHelper.avocadoNotification('Queries successfully imported.');
          callback();
        },
        error: function () {
          window.progressView.hide();
          avocadoHelper.avocadoError('Query error', 'queries could not be imported');
        }
      });
    }
