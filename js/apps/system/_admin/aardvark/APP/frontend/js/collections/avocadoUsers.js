...
/* global window, atob, Backbone, $,_, window, frontendConfig, avocadoHelper */

window.AvocadoUsers = Backbone.Collection.extend({
  model: window.Users,
  ...
  login: function (username, password, callback) {
    ...
    $.ajax({
      url: avocadoHelper.databaseUrl('/_open/auth'),
      method: 'POST',
      data: JSON.stringify({
        username: username,
        password: password
      ...
      function (data) {
        ...
        if (self.activeUser === undefined) {
          avocadoHelper.setCurrentJwt(data.jwt, null);
        } else {
          avocadoHelper.setCurrentJwt(data.jwt, self.activeUser);
        }
      ...
      function () {
        avocadoHelper.setCurrentJwt(null, null);
  ...
  logout: function () {
    avocadoHelper.setCurrentJwt(null);
  ...
  loadUserSettings: function (callback) {
    ...
    if (!frontendConfig.authenticationEnabled) {
      url = avocadoHelper.databaseUrl('/_api/user/root');
    } else {
      url = avocadoHelper.databaseUrl('/_api/user/' + encodeURIComponent(self.activeUser));
    }
