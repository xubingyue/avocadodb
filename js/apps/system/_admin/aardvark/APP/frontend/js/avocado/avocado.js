/* jshint unused: false */
/* global Blob, window, sigma, $, Tippy, document, _, avocadoHelper, frontendConfig, avocadoHelper, localStorage */
...
(function () {
  ...
  window.avocadoHelper = {
    ...
    fixTooltips: function (selector, placement) {
      avocadoHelper.createTooltips(selector, placement);
      /*
      $(selector).tooltip({
        placement: placement,
        hide: false,
    ...
    hotkeysFunctions: {
      ...
      showHotkeysModal: function () {
        var buttons = [];
        var content = window.avocadoHelper.allHotkeys;
    ...
    */
    ...
    enableKeyboardHotkeys: function (enable) {
      var hotkeys = window.avocadoHelper.hotkeysFunctions;
      if (enable === true) {
    ...
    openDocEditor: function (id, type, callback) {
      ...
      var docFrameView = new window.DocumentView({
        collection: window.App.avocadoDocumentStore
      });
      ...
      docFrameView.el = '.avocadoFrame .innerDiv';
      docFrameView.render();
      ...
      */
      ...
      // remove header
      $('.avocadoFrame .headerBar').remove();
      // append close button
      $('.avocadoFrame .outerDiv').prepend('<i class="fa fa-times"></i>');
      // add close events
      $('.avocadoFrame .outerDiv').click(function () {
        self.closeDocEditor();
      });
      $('.avocadoFrame .innerDiv').click(function (e) {
        e.stopPropagation();
      });
      ...
      $('.avocadoFrame #deleteDocumentButton').click(function () {
        docFrameView.deleteDocumentModal();
      });
      $('.avocadoFrame #saveDocumentButton').click(function () {
        docFrameView.saveDocument();
      });
      $('.avocadoFrame #deleteDocumentButton').css('display', 'none');
    },

    closeDocEditor: function () {
      $('.avocadoFrame .outerDiv .fa-times').remove();
      $('.avocadoFrame').hide();
    },
                ...
                _.each(AaJobs, function (aardvark) {
                  if (aardvark.type === type || aardvark.type === undefined) {
                    ...
                    if (found) {
                      ...
                      window.avocadoHelper.deleteAardvarkJob(aardvark.id);
                    }
                  }
                });
              } else {
    ...
    collectionApiType: function (identifier, refresh, toRun) {
      ...
      if (refresh || this.CollectionTypes[identifier] === undefined) {
        var callback = function (error, data, toRun) {
          if (error) {
            avocadoHelper.avocadoError('Error', 'Could not detect collection type');
          } else {
        ...
        this.avocadoDocumentStore.getCollectionInfo(identifier, callback, toRun);
      } else {
    ...
    checkCollectionPermissions: function (collectionID, roCallback) {
      var url = avocadoHelper.databaseUrl('/_api/user/' +
      ...
      $.ajax({
        ...
        error: function (data) {
          avocadoHelper.avocadoError('User', 'Could not fetch collection permissions.');
        }
      });
    },
