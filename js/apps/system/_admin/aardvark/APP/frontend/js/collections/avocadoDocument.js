...
/* global Backbone, window, avocadoDocumentModel, $, avocadoHelper */

window.AvocadoDocument = Backbone.Collection.extend({
  url: '/_api/document/',
  model: avocadoDocumentModel,
  collectionInfo: {},
  ...
  deleteDocument: function (colid, dockey, callback) {
    ...
    $.ajax({
      ...
      url: avocadoHelper.databaseUrl('/_api/simple/remove-by-keys'),
      success: function () {
        callback(false);
      },
      error: function () {
  ...
  createTypeEdge: function (collectionID, from, to, key, callback) {
    ...
    $.ajax({
      cache: false,
      type: 'POST',
      url: avocadoHelper.databaseUrl('/_api/document?collection=' + encodeURIComponent(collectionID)),
      data: newEdge,
      contentType: 'application/json',
      processData: false,
  ...
  createTypeDocument: function (collectionID, key, callback, returnNew) {
    ...
    var url = avocadoHelper.databaseUrl('/_api/document?collection=' + encodeURIComponent(collectionID));
  ...
  getCollectionInfo: function (identifier, callback, toRun) {
    ...
    $.ajax({
      cache: false,
      type: 'GET',
      url: avocadoHelper.databaseUrl('/_api/collection/' + identifier + '?' + avocadoHelper.getRandomToken()),
      contentType: 'application/json',
      processData: false,
      success: function (data) {
  ...
  getDocument: function (colid, dockey, callback) {
    ...
    $.ajax({
      cache: false,
      type: 'PUT',
      url: avocadoHelper.databaseUrl('/_api/simple/lookup-by-keys'),
      contentType: 'application/json',
      data: JSON.stringify(data),
      processData: false,
      success: function (data) {
      ...
      error: function (data) {
        avocadoHelper.avocadoError('Error', data.responseJSON.errorMessage + ' - error number: ' + data.responseJSON.errorNum);
  ...
  saveDocument: function (colid, dockey, model, callback, from, to) {
    if (typeof model === 'string') {
      try {
        ...
        avocadoHelper.avocadoError('Could not parse document: ' + model);
      }
    }
    model._key = dockey;
    ...
    $.ajax({
      cache: false,
      type: 'PUT',
      url: avocadoHelper.databaseUrl('/_api/document/' + encodeURIComponent(colid)),
