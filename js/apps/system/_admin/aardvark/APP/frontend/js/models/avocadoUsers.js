...
window.Users = Backbone.Model.extend({
  ...
  url: function () {
    if (this.isNew()) {
      return avocadoHelper.databaseUrl('/_api/user');
    }
    if (this.get('user') !== '') {
      return avocadoHelper.databaseUrl('/_api/user/' + this.get('user'));
    }
    return avocadoHelper.databaseUrl('/_api/user');
  },
  ...
  checkPassword: function (passwd, callback) {
    $.ajax({
      cache: false,
      type: 'POST',
      url: avocadoHelper.databaseUrl('/_api/user/' + this.get('user')),
