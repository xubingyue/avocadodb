/* jshint browser: true */
/* jshint unused: false */
/* global Backbone, window, avocadoCollectionModel, $, avocadoHelper, _ */
(function () {
  'use strict';

  window.AvocadoCollections = Backbone.Collection.extend({
    url: avocadoHelper.databaseUrl('/_api/collection'),

    model: avocadoCollectionModel,
    ...
    parse: function (response) {
      ...
      _.each(response.result, function (val) {
        val.isSystem = avocadoHelper.isSystemCollection(val);
        val.type = avocadoHelper.collectionType(val);
