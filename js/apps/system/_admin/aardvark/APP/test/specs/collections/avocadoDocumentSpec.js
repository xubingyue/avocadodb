...
/* global XMLHttpRequestException, describe, beforeEach, afterEach, it, spyOn,
 avocadoHelper, expect*/
/* global $*/

(function () {
  'use strict';

  describe('avocadoDocument', function () {
    var col;

    beforeEach(function () {
      col = new window.avocadoDocument();
      window.avocadoDocumentStore = col;
    });
    ...
    it('should getCollectionInfo and succeed', function () {
      ...
      spyOn(avocadoHelper, 'getRandomToken').andReturn('2');
      spyOn($, 'ajax').andCallFake(function (opt) {
    ...
    it('should getEdge and succeed', function () {
      ...
      spyOn(window.avocadoDocumentStore, 'add');
      result = col.getEdge(colid, docid);
      expect(result).toEqual(true);
      expect(window.avocadoDocumentStore.add).toHaveBeenCalledWith('bla');
    });
    ...
    it('should updateLocalDocument', function () {
      spyOn(window.avocadoDocumentStore, 'reset');
      var result = col.updateLocalDocument({x: 'y'});
      expect(window.avocadoDocumentStore.reset).toHaveBeenCalled();
    });
