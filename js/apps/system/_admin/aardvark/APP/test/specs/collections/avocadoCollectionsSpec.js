/* jshint browser: true */
/* jshint unused: false */
/* global describe, avocadoHelper, beforeEach, afterEach, it, spyOn, expect*/
/* global $, jasmine*/

(function () {
  'use strict';

  describe('avocadoCollections', function () {
    var col;

    beforeEach(function () {
      col = new window.avocadoCollections();
      window.collectionsView = new window.CollectionsView({
        collection: col
      });
      window.avocadoCollectionsStore = col;
    });
    ...
    it('should translate various typePictures', function () {
      ...
      expect(col.translateTypePicture('default')).toEqual('img/icon_avocado.png');
    });
    ...
    it('should parse a response', function () {
      ...
      expect(response.collections[0].picture).toEqual('img/icon_avocado.png');
    });

    it('should getPosition for loaded documents sorted by type', function () {
      col.models = [
        new window.avocadoCollectionModel({isSystem: true, name: 'Heinz Herbert'}),
        new window.avocadoCollectionModel({type: 'edge', name: 'Heinz Herbert'}),
        new window.avocadoCollectionModel({type: 'document', name: 'Heinz Herbert Gustav'}),
        new window.avocadoCollectionModel({type: 'document', name: 'Heinz Herbert'}),
        new window.avocadoCollectionModel({status: 'loaded', name: 'Heinz Herbert Karl'}),
        new window.avocadoCollectionModel({status: 'unloaded', name: 'Heinz Herbert'}),
        new window.avocadoCollectionModel({name: 'Heinbert'})
      ];
      col.searchOptions = {
