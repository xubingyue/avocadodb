...
var FoxxManager = require('@avocadodb/foxx/manager');
var shallowCopy = require('@avocadodb/util').shallowCopy;
...
function requireModule (name, route, parentModule) {
  ...
  try {
    ...
    if (err instanceof internal.AvocadoError && err.errorNum === internal.errors.ERROR_MODULE_NOT_FOUND.code) {
      return notImplementedFunction(route, util.format(
...
function foxxRouting (req, res, options, next) {
  ...
  try {
    ...
    options.error = {
      ...
      num: e.errorNum || avocadodb.ERROR_HTTP_SERVICE_UNAVAILABLE,
...
function buildRouting (dbname) {
  ...
  // compute all routes
  var routes = [];
  var routing = avocadodb.db._collection('_routing');
  if (routing !== null) {
    let i = routing.all();
...
function routeRequest (req, res, routes) {
  ...
  function execute () {
    if (action.route === undefined) {
      resultNotFound(req, res, avocadodb.ERROR_HTTP_NOT_FOUND,
...
function getJsonBody (req, res, code) {
  ...
  try {
    ...
    resultBad(req, res, avocadodb.ERROR_HTTP_CORRUPTED_JSON, err1);
    return undefined;
  }

  if (!body || !(body instanceof Object)) {
    if (code === undefined) {
      code = avocadodb.ERROR_HTTP_CORRUPTED_JSON;
    }

    err = new internal.AvocadoError();
    err.errorNum = code;
...
function resultNotImplemented (req, res, msg, headers) {
  ...
  resultError(req,
    ...
    avocadodb.ERROR_NOT_IMPLEMENTED,
    msg,
    headers);
}
...
function resultUnsupported (req, res, headers) {
  ...
  resultError(req, res,
    ...
    avocadodb.ERROR_HTTP_METHOD_NOT_ALLOWED,
    'Unsupported method',
    headers);
}
...
function collectionNotFound (req, res, collection, headers) {
  ...
  if (collection === undefined) {
    resultError(req, res,
      exports.HTTP_BAD, avocadodb.ERROR_HTTP_BAD_PARAMETER,
...
function avocadoErrorToHttpCode (num) {
  if (!num) {
  ...
  switch (num) {
    ...
    case avocadodb.ERROR_QUERY_NOT_FOUND:
    case avocadodb.ERROR_SERVICE_NOT_FOUND:
    ...
    case avocadodb.ERROR_SERVICE_MOUNTPOINT_CONFLICT:
    ...
    case avocadodb.ERROR_CLUSTER_SHARD_LEADER_REFUSES_REPLICATION:
    case avocadodb.ERROR_CLUSTER_SHARD_FOLLOWER_REFUSES_OPERATION:
...
function resultException (req, res, err, headers, verbose) {
  ...
  if (err instanceof internal.AvocadoError) {
    if (err.errorMessage !== '') {
      if (verbose !== false) {
    ...
    num = err.errorNum || avocadodb.ERROR_INTERNAL;
    code = avocadoErrorToHttpCode(num);
  } else if (err instanceof TypeError) {
    num = avocadodb.ERROR_TYPE_ERROR;
    code = exports.HTTP_BAD;
  } else if (err.statusCode) {
    num = err.statusCode;
    code = err.statusCode;
  } else {
    num = avocadodb.ERROR_HTTP_SERVER_ERROR;
...
exports.avocadoErrorToHttpCode = avocadoErrorToHttpCode;
