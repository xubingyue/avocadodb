...
module.exports = function collectionStorage (cfg) {
  if (typeof cfg === 'string' || cfg.isAvocadoCollection) {
    cfg = {collection: cfg};
  }
  if (!cfg) {
    cfg = {};
  ...
  assert(collection.isAvocadoCollection, `No such collection: ${cfg.collection}`);
  return {
    prune() {
    ...
    fromClient(sid) {
      try {
        ...
        if (e.isAvocadoError && e.errorNum === NOT_FOUND) {
          return null;
        }
        throw e;
      }
    ...
    save(session) {
      ...
      if (!session._key) {
        // generate a new key
        let crypto = require('@avocadodb/crypto');
        while (true) {
          ...
          try {
            ...
            if (!e.isAvocadoError || e.errorNum !== UNIQUE_CONSTRAINT) {
              throw e;
            }
          }
        }
