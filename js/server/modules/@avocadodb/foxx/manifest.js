...
const il = require('@avocadodb/util').inline;
...
function checkManifest (filename, inputManifest, mount, complainAboutVersionMismatches) {
  const serverVersion = avocadodb.plainServerVersion();
  const errors = [];
  const manifest = {};
  let legacy = false;
  ...
  if (manifest.engines && manifest.engines.avocadodb) {
    if (semver.gtr('3.0.0', manifest.engines.avocadodb)) {
      legacy = true;
      if (complainAboutVersionMismatches) {
        console.infoLines(il`
          Service at "${mount}" expects version "${manifest.engines.avocadodb}"
          and will run in legacy compatibility mode.
        `);
      }
    } else if (!semver.satisfies(serverVersion, manifest.engines.avocadodb)) {
      if (complainAboutVersionMismatches) {
