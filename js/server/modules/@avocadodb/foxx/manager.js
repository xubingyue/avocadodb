...
const utils = require('@avocadodb/foxx/manager-utils');
const store = require('@avocadodb/foxx/store');
const FoxxService = require('@avocadodb/foxx/service');
const ensureServiceExecuted = require('@avocadodb/foxx/routing').routeService;
...
const AvocadoClusterControl = require('@avocadodb/cluster');
const request = require('@avocadodb/request');
const actions = require('@avocadodb/actions');
const isZipBuffer = require('@avocadodb/util').isZipBuffer;
...
function getAllCoordinatorIds () {
  if (!AvocadoClusterControl.isCluster()) {
    return [];
  }
  return global.AvocadoClusterInfo.getCoordinators();
}
...
function isFoxxmaster () {
  ...
  return global.AvocadoServerState.isFoxxmaster();
}
...
function parallelClusterRequests (requests) {
  ...
  for (const [coordId, method, url, body, headers] of requests) {
    if (!options) {
      options = {coordTransactionID: global.AvocadoClusterComm.getId()};
    }
    options.clientTransactionID = global.AvocadoClusterInfo.uniqid();
    ...
    global.AvocadoClusterComm.asyncRequest(
      method,
      `server:${coordId}`,
      db._name(),
      url,
  ...
  const results = AvocadoClusterControl.wait(options, pending, true);
  return results.sort(
...
function startup () {
  ...
  if (global.AvocadoServerState.role() === 'SINGLE') {
...
function _prepareService (serviceInfo, options = {}) {
  ...
  try {
    ...
    if (options.legacy) {
      patchManifestFile(tempServicePath, {engines: {avocadodb: '^2.8.0'}});
...
function runTests (mount, options = {}) {
  ...
  ensureServiceLoaded(mount);
  return require('@avocadodb/foxx/mocha').run(service, options.reporter);
}
