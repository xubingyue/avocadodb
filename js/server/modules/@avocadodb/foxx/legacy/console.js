...
var avocadoConsole = require('console');
var ErrorStackParser = require('error-stack-parser');
var AssertionError = require('assert').AssertionError;
var exists = require('@avocadodb/is').existy;
...
function nativeLogger (level, levelNum, mount) {
  ...
  if (NATIVE_LOG_LEVELS.indexOf(logLevel) !== -1) {
    ...
    return function (message) {
      avocadoConsole._log(logLevel, `${mount} ${message}`);
    };
  }
  if (levelNum >= 200) {
    logLevel = 'error';
  ...
  return function (message) {
    avocadoConsole._log(logLevel, `(${level}) ${mount} ${message}`);
  };
}
