...
var runInDatabase = function () {
          ...
          jobs.forEach(function (job) {
            ...
            tasks.register({
              command: function (cfg) {
                ...
                try {
                  require('@avocadodb/foxx/queues/worker').work(cfg.job);
                } catch (e) {}
  ...
  if (!busy) {
    require('@avocadodb/foxx/queues')._updateQueueDelay();
  }
};

exports.manage = function () {
  if (!global.AvocadoServerState.isFoxxmaster()) {
    return;
  }

  if (isCluster && global.AvocadoServerState.getFoxxmasterQueueupdate()) {
    var foxxQueues = require('@avocadodb/foxx/queues');
    foxxQueues._updateQueueDelay();
    global.AvocadoAgency.set('Current/FoxxmasterQueueupdate', false);
    //global.AvocadoServerState.setFoxxmasterQueueupdate(false);
  }
...
exports.run = function () {
  ...
  var queues = require('@avocadodb/foxx/queues');
  ...
  if (tasks.register !== undefined) {
    tasks.register({
      command: function () {
        require('@avocadodb/foxx/queues/manager').manage();
      },
      period: period,
      isSystem: true
    });
