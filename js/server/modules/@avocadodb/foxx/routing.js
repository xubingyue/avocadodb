...
const dd = require('@avocadodb/util').dedent;
...
const routeLegacyService = require('@avocadodb/foxx/legacy/routing').routeService;
...
exports.routeService = function (service, throwOnErrors) {
  ...
  if (service.manifest.files) {
    ...
    _.each(files, function (file, path) {
      ...
      const normalized = avocadodb.normalizeURL(`/${path}`);
      const route = {
        url: {match: `${normalized}/*`},
        action: {
          do: '@avocadodb/actions/pathHandler',
          options: {
            root: service.root,
