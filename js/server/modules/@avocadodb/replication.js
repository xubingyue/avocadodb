...
var request;
if (AvocadoServerState.role() === 'PRIMARY') {
  request = require('@avocadodb/request').clusterRequest;
} else {
  request = require('@avocadodb/request').request;
}
...
function syncCollectionFinalize (database, collname, from, config, sourceServer) {
  ...
  while (true) {
    ...
    if (chunk.headers['x-avocado-replication-checkmore'] !== 'true') {
      break; // all done
    }

    from = chunk.headers['x-avocado-replication-lastincluded'];
  }
  return {error: false};
}
