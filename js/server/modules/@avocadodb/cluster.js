/* global AvocadoServerState, AvocadoClusterInfo */
'use strict';
...
var request = require('@avocadodb/request').clusterRequest;
var wait = require('internal').wait;
var isEnterprise = require('internal').isEnterprise();
var _ = require('lodash');

const curDatabases = '/avocado/Current/Databases/';
const curCollections = '/avocado/Current/Collections/';
const curVersion = '/avocado/Current/Version';
...
function startReadLockOnLeader (endpoint, database, collName, timeout) {
  ...
  request({ url: url + '/_api/replication/holdReadLockCollection',
            ...
            method: 'POST', headers: {'x-avocado-async': true} });
...
function addShardFollower (endpoint, database, shard, lockJobId) {
  ...
  var body = {followerId: AvocadoServerState.id(), shard, checksum: db._collection(shard).count() + '', readLockId: lockJobId};
...
function removeShardFollower (endpoint, database, shard) {
  ...
  var body = {followerId: AvocadoServerState.id(), shard};
  var r = request({url, body: JSON.stringify(body), method: 'PUT'});
  if (r.status !== 200) {
          ...
          try {
            registerTask({
              ...
              command: function (params) {
                require('@avocadodb/cluster').synchronizeOneShard(
...
function synchronizeOneShard (database, shard, planId, leader) {
  ...
  var isStopping = require('internal').isStopping;
  var ourselves = global.AvocadoServerState.id();
  ...
  while (true) {
    ...
    var planned = [];
    try {
      planned = global.AvocadoClusterInfo.getCollectionInfo(database, planId)
        .shards[shard];
    } catch (e) {}
    ...
    var current = [];
    try {
      current = global.AvocadoClusterInfo.getCollectionInfoCurrent(
  ...
  // we give it a try:
  var ok = false;
  const rep = require('@avocadodb/replication');
  ...
  try {
    var ep = AvocadoClusterInfo.getServerEndpoint(leader);
...
function executePlanForCollections(plannedCollections) {
  let ourselves = global.AvocadoServerState.id();
  let localErrors = {};
          ...
          localErrors = Object.keys(shards).reduce((errors, shardName) => {
            ...
            if (plannedServers.indexOf(ourselves) >= 0) {
              ...
              if (!localCollections.hasOwnProperty(shardName)) {
                ...
                try {
                  if (collectionInfo.type === AvocadoCollection.TYPE_EDGE) {
                ...
                collectionStatus = AvocadoCollection.STATUS_LOADED;
              } else {
                collection = db._collection(shardName);
              ...
              if (collectionStatus !== collectionInfo.status) {
                ...
                if (collectionInfo.status === AvocadoCollection.STATUS_UNLOADED) {
                  console.topic('heartbeat=debug', "unloading local shard '%s/%s'",
                    database,
                    shardName);

                  collection.unload();
                } else if (collectionInfo.status === AvocadoCollection.STATUS_LOADED) {
          ...
          if (shardMap.hasOwnProperty(collection) &&
            ...
            if (collections[collection].theLeader !== "") {
              ...
              if (servers !== undefined) {
                let endpoint = AvocadoClusterInfo.getServerEndpoint(servers[0]);
                try {
...
function migratePrimary(plan, current) {
  ...
  if (Object.keys(trx).length > 0) {
    ...
    try {
      let res = global.AvocadoAgency.write([trx]);
...
function executePlanForDatabases(plannedDatabases) {
  ...
  Object.keys(localDatabases).forEach(name => {
    if (!plannedDatabases.hasOwnProperty(name) && name.substr(0, 1) !== '_') {
      ...
      if (AvocadoServerState.role() === 'SECONDARY') {
        try {
          db._useDatabase(name);
          var rep = require('@avocadodb/replication');
  ...
  for (i = 0; i < dbs.length; i++) {
    ...
    try {
      ...
      if (state.state.running === false) {
        var endpoint = AvocadoClusterInfo.getServerEndpoint(
          AvocadoServerState.idOfPrimary());
...
function handleChanges (plan, current) {
  ...
  if (role === 'PRIMARY') {
    ...
    if (plan.DBServers[myId]) {
      ...
      if (found !== AvocadoServerState.idOfPrimary()) {
...
var shardList = function (dbName, collectionName) {
  let ci = global.AvocadoClusterInfo.getCollectionInfo(dbName, collectionName);
...
var waitForDistributedResponse = function (data, numberOfRequests, ignoreHttpErrors) {
  ...
  try {
    while (received.length < numberOfRequests) {
      var result = global.AvocadoClusterComm.wait(data);
      var status = result.status;

      if (status === 'ERROR') {
        raiseError(avocadodb.errors.ERROR_INTERNAL.code,
          'received an error from a DB server: ' + JSON.stringify(result));
      } else if (status === 'TIMEOUT') {
        raiseError(avocadodb.errors.ERROR_CLUSTER_TIMEOUT.code,
          avocadodb.errors.ERROR_CLUSTER_TIMEOUT.message);
        ...
        if (result.headers && result.headers.hasOwnProperty('x-avocado-response-code')) {
          var code = parseInt(result.headers['x-avocado-response-code'].substr(0, 3), 10);
    ...
    global.AvocadoClusterComm.drop(data);
  }
  return received;
};
...
var isCluster = function () {
  var role = global.AvocadoServerState.role();
...
var isCoordinator = function () {
  return global.AvocadoServerState.isCoordinator();
};
...
var status = function () {
  if (!isCluster() || !global.AvocadoServerState.initialized()) {
    return undefined;
  }

  return global.AvocadoServerState.status();
};
...
var isCoordinatorRequest = function (req) {
  ...
  return req.headers.hasOwnProperty('x-avocado-coordinator');
};
...
var handlePlanChange = function (plan, current) {
  ...
  if (!isCluster() || isCoordinator() || !global.AvocadoServerState.initialized()) {
    return true;
  }
...
var coordinatorId = function () {
  ...
  return global.AvocadoServerState.id();
};
...
function shardDistribution () {
  ...
  for (var i = 0; i < colls.length; ++i) {
    ...
    var collInfo = global.AvocadoClusterInfo.getCollectionInfo(dbName, collName);
    var shards = collInfo.shards;
    var collInfoCurrent = {};
    var shardNames = Object.keys(shards);
    for (var j = 0; j < shardNames.length; ++j) {
      collInfoCurrent[shardNames[j]] =
        global.AvocadoClusterInfo.getCollectionInfoCurrent(
...
function moveShard (info) {
  ...
  try {
    ...
    collInfo = global.AvocadoClusterInfo.getCollectionInfo(info.database,
      info.collection);
  ...
  var id;
  try {
    id = global.AvocadoClusterInfo.uniqid();
    var todo = { 'type': 'moveShard',
      ...
      'isLeader': isLeader };
    global.AvocadoAgency.set('Target/ToDo/' + id, todo);
  } catch (e1) {
...
function rebalanceShards () {
  var dbServers = global.AvocadoClusterInfo.getDBServers();
  var dbTab = {};
  var i, j, k, l;
  ...
  for (j = 0; j < colls.length; ++j) {
    ...
    var collInfo = global.AvocadoClusterInfo.getCollectionInfo(
      db._name(), collName);
    if (collInfo.distributeShardsLike === undefined) {
...
function supervisionState () {
  try {
    var result = global.AvocadoAgency.get('Target');
    result = result.avocado.Target;
...
function checkForSyncReplOneCollection (dbName, collName) {
  try {
    let cinfo = global.AvocadoClusterInfo.getCollectionInfo(dbName, collName);
    let shards = Object.keys(cinfo.shards);
    let ccinfo = shards.map(function (s) {
      return global.AvocadoClusterInfo.getCollectionInfoCurrent(dbName,
        collName, s).servers;
    });
...
function endpoints() {
  try {
    let coords = global.AvocadoClusterInfo.getCoordinators();
    let endpoints = coords.map(c => global.AvocadoClusterInfo.getServerEndpoint(c));
