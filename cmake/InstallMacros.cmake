...
if (NOT CMAKE_INSTALL_SYSCONFDIR_AVOCADO
    OR NOT CMAKE_INSTALL_FULL_SYSCONFDIR_AVOCADO
    OR NOT CMAKE_INSTALL_DATAROOTDIR_AVOCADO
    OR NOT CMAKE_INSTALL_FULL_DATAROOTDIR_AVOCADO)
...
macro (install_config name)
  if (MSVC OR (DARWIN AND NOT HOMEBREW))
    set(PKGDATADIR "@ROOTDIR@/${CMAKE_INSTALL_DATAROOTDIR_AVOCADO}")
    if (DARWIN)
      # var will be redirected to ~ for the macos bundle
      set(LOCALSTATEDIR "@HOME@${INC_CPACK_AVOCADO_STATE_DIR}")
    else ()
      set(LOCALSTATEDIR "@ROOTDIR@${CMAKE_INSTALL_LOCALSTATEDIR}")
    endif ()
    set(SBINDIR "@ROOTDIR@/${CMAKE_INSTALL_SBINDIR}")  
    set(SYSCONFDIR "@ROOTDIR@/${CMAKE_INSTALL_SYSCONFDIR_AVOCADO}")
  else ()
    set(PKGDATADIR "${CMAKE_INSTALL_FULL_DATAROOTDIR_AVOCADO}")
  ...
  configure_file(
    "${PROJECT_SOURCE_DIR}/etc/avocadodb3/${name}.conf.in"
    "${PROJECT_BINARY_DIR}/${CMAKE_INSTALL_SYSCONFDIR_AVOCADO}/${name}.conf"
    NEWLINE_STYLE ${CRLFSTYLE}
    @ONLY)

  install(
    FILES ${PROJECT_BINARY_DIR}/${CMAKE_INSTALL_SYSCONFDIR_AVOCADO}/${name}.conf
    DESTINATION ${CMAKE_INSTALL_SYSCONFDIR_AVOCADO})

  set(INSTALL_CONFIGFILES_LIST
    "${INSTALL_CONFIGFILES_LIST};${CMAKE_INSTALL_SYSCONFDIR_AVOCADO}/${name}.conf"
...
macro (install_readme input output)
  ...
  if (${USE_VERSION_IN_LICENSEDIR})
    set(PKG_VERSION "-${AVOCADODB_VERSION}")
  endif ()
  set(CRLFSTYLE "UNIX")
  if (MSVC)
    set(CRLFSTYLE "CRLF")
